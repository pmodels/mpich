...
MPL_STATIC_INLINE_PREFIX const MPIDI_POSIX_coll_algo_container_t
    * MPIDI_POSIX_coll_select(MPIDU_SELECTON_coll_signature_t * coll_sig,
                              const void *ch4_algo_parameters_container_in ATTRIBUTE((unused)))
{
    const MPIDI_POSIX_coll_algo_container_t *container = NULL;
    ...
    if ((ch4_algo_parameters_container_in != NULL) &&
        ...
        container = (const MPIDI_POSIX_coll_algo_container_t *) ch4_algo_parameters_container_in;
        is_valid = (container != NULL);
        if (!is_valid) {
            container = (MPIDI_POSIX_coll_algo_container_t *)
                MPIDI_POSIX_get_default_container(coll_sig->coll_id);
        }
    ...
    if (!is_valid) {
        container = (const MPIDI_POSIX_coll_algo_container_t *)
            MPIDI_POSIX_get_default_container(coll_sig->coll_id);
    }
