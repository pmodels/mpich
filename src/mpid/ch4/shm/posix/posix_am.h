                     ...
                     MPI_Count count, MPI_Datatype datatype, MPIR_Request * sreq, void *shm_context)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_POSIX_SEND_AM);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_POSIX_SEND_AM);
    ...
    MPIDI_POSIX_am_header_t msg_hdr = {.handler_id = handler_id,
        .am_hdr_sz = am_hdr_sz,
        .data_sz = data_sz
    };
    ...
    POSIX_TRACE
        ("Direct OUT HDR [ POSIX AM [handler_id %llu, am_hdr_sz %llu, data_sz %llu, seq_num = %d], "
         "tag = %08x, src_rank = %d ] to %d\n", msg_hdr_p->handler_id, msg_hdr_p->am_hdr_sz,
         msg_hdr_p->data_sz,
#ifdef POSIX_AM_DEBUG
         msg_hdr_p->seq_num,
#else /* POSIX_AM_DEBUG */
  ...
  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_POSIX_SEND_AM);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
  ...
  enqueue_request:
    ...
    MPL_DL_APPEND(MPIDI_POSIX_global.postponed_queue, &sreq->dev.ch4.am.req->rreq);
...
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int
MPIDI_POSIX_am_isendv(int rank,
                      MPIR_Comm * comm,
                      int handler_id,
                      struct iovec *am_hdr,
                      size_t iov_len,
...
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int
MPIDI_POSIX_am_isend_reply(MPIR_Context_id_t context_id,
                           int src_rank,
                           int handler_id,
                           const void *am_hdr,
                           size_t am_hdr_sz,
...
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int
MPIDI_POSIX_am_send_hdr_reply(MPIR_Context_id_t context_id,
...
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_am_recv(MPIR_Request * req)
{
    ...
    mpi_errno = MPIDI_POSIX_am_send_hdr_reply(MPIDI_CH4U_get_context(MPIDI_CH4U_REQUEST(req, tag)),
