                                                  ...
                                                  MPI_Datatype datatype, MPIR_Request * sreq)
{
    ...
    POSIX_TRACE("Direct OUT HDR [ POSIX AM [handler_id %" PRIu64 ", am_hdr_sz %" PRIu64
                ", data_sz %" PRIu64 ", seq_num = %d], " "tag = %d, src_rank = %d, endpoint = %d ] to %d\n",
                ...
                -1,
#endif /* POSIX_AM_DEBUG */
                ((MPIDIG_hdr_t *) am_hdr)->tag, ((MPIDIG_hdr_t *) am_hdr)->src_rank,
                ((MPIDIG_hdr_t *) am_hdr)->endpoint, grank);
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_am_send_hdr_reply(MPIR_Context_id_t context_id, int endpoint,
                                                           ...
                                                           const void *am_hdr, size_t am_hdr_sz)
{
    ...
    mpi_errno = MPIDI_POSIX_am_send_hdr(src_rank,
                                        MPIDIG_context_id_to_comm(context_id, endpoint),
...
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_am_recv(MPIR_Request * req)
{
    ...
    mpi_errno = MPIDI_POSIX_am_send_hdr_reply(MPIDIG_REQUEST(req, context_id), MPIDIG_REQUEST(req, endpoint),
