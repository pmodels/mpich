...
int MPIDI_POSIX_fbox_init(int rank, int size)
{
    ...
    MPIR_CHKPMEM_MALLOC(MPIDI_POSIX_eager_fbox_transport_global.first_poll_local_ranks,
                        int16_t *,
                        sizeof(*MPIDI_POSIX_eager_fbox_transport_global.first_poll_local_ranks) *
    ...
    for (i = 0; i < MPIR_CVAR_CH4_POSIX_EAGER_FBOX_POLL_CACHE_SIZE; i++) {
        MPIDI_POSIX_eager_fbox_transport_global.first_poll_local_ranks[i] = -1;
    }
    ...
    MPIDI_POSIX_eager_fbox_transport_global.first_poll_local_ranks[i] = 0;
    ...
    mpi_errno = MPIDU_Init_shm_alloc(len, &MPIDI_POSIX_eager_fbox_transport_global.shm_ptr);
    MPIR_ERR_CHECK(mpi_errno);

    /* Allocate table of pointers to fastboxes */
    MPIR_CHKPMEM_MALLOC(MPIDI_POSIX_eager_fbox_transport_global.mailboxes.in,
                        MPIDI_POSIX_fastbox_t **,
                        MPIDI_POSIX_global.num_local * sizeof(MPIDI_POSIX_fastbox_t *), mpi_errno,
                        "fastboxes", MPL_MEM_SHM);
    MPIR_CHKPMEM_MALLOC(MPIDI_POSIX_eager_fbox_transport_global.mailboxes.out,
                        MPIDI_POSIX_fastbox_t **,
                        MPIDI_POSIX_global.num_local * sizeof(MPIDI_POSIX_fastbox_t *), mpi_errno,
                        "fastboxes", MPL_MEM_SHM);

    /* Fill in fbox tables */
    char *fastboxes_p = (char *) MPIDI_POSIX_eager_fbox_transport_global.shm_ptr;
    for (i = 0; i < MPIDI_POSIX_global.num_local; i++) {
        MPIDI_POSIX_eager_fbox_transport_global.mailboxes.in[i] =
            (void *) (fastboxes_p + FBOX_OFFSET(i, MPIDI_POSIX_global.my_local_rank));
        MPIDI_POSIX_eager_fbox_transport_global.mailboxes.out[i] =
            (void *) (fastboxes_p + FBOX_OFFSET(MPIDI_POSIX_global.my_local_rank, i));

        memset(MPIDI_POSIX_eager_fbox_transport_global.mailboxes.in[i], 0, MPIDI_POSIX_FBOX_SIZE);
    }
...
int MPIDI_POSIX_fbox_finalize(void)
{
    ...
    MPL_free(MPIDI_POSIX_eager_fbox_transport_global.mailboxes.in);
    MPL_free(MPIDI_POSIX_eager_fbox_transport_global.mailboxes.out);
    MPL_free(MPIDI_POSIX_eager_fbox_transport_global.first_poll_local_ranks);

    mpi_errno = MPIDU_Init_shm_free(MPIDI_POSIX_eager_fbox_transport_global.shm_ptr);
