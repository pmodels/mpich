...
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_eager_init(int rank, int size)
{
    ...
    size_of_terminals = (size_t)transport->num_local
        * sizeof(MPIDI_POSIX_EAGER_IQUEUE_terminal_t);
    ...
    transport->terminals = (MPIDI_POSIX_EAGER_IQUEUE_terminal_t*)(
        (char*)transport->pointer_to_shared_memory);

    transport->cells = (char*)transport->pointer_to_shared_memory
        + size_of_terminals
        + (size_t)transport->local_rank * (size_t)transport->num_cells * (size_t)transport->size_of_cell;
