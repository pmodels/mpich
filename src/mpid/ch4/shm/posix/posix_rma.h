                                                                       ...
                                                                       int is_dev2, int dev2)
{
    ...
    MPIR_Assert(MPIR_CVAR_CH4_GPU_RMA_ENGINE_TYPE != MPIR_CVAR_CH4_GPU_RMA_ENGINE_TYPE_yaksa);
    if (MPIR_CVAR_CH4_GPU_RMA_ENGINE_TYPE != MPIR_CVAR_CH4_GPU_RMA_ENGINE_TYPE_auto) {
        return MPIR_CVAR_CH4_GPU_RMA_ENGINE_TYPE - MPIR_CVAR_CH4_GPU_RMA_ENGINE_TYPE_0;
    }

    /* auto select */
    engine = MPL_GPU_ENGINE_TYPE_COPY_LOW_LATENCY;
    /* host buffer involved */
    if (!is_dev1 || !is_dev2) {
        return MPL_GPU_ENGINE_TYPE_COPY_HIGH_BANDWIDTH;
    }
    /* a device is not visible */
    if (dev1 == -1 || dev2 == -1) {
        return MPL_GPU_ENGINE_TYPE_COPY_LOW_LATENCY;
    }
    MPIR_Assert(dev1 != -1 && dev2 != -1);
...
#ifdef MPL_HAVE_GPU
    if (MPIR_CVAR_CH4_GPU_RMA_ENGINE_TYPE != MPIR_CVAR_CH4_GPU_RMA_ENGINE_TYPE_yaksa) {
        int engine_type =
