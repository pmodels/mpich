...
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_progress(int blocking)
{
    ...
    MPIDI_POSIX_am_header_t *msg_hdr = transaction.msg_hdr;
  ...
  send_progress:
    ...
    if (MPIDI_POSIX_global.postponed_queue) {
        ...
        POSIX_TRACE
            ("Queue OUT HDR [ POSIX AM [handler_id %llu, am_hdr_sz %llu, data_sz %llu, seq_num = %d], "
        ...
        MPL_DL_DELETE(MPIDI_POSIX_global.postponed_queue, &sreq->dev.ch4.am.req->rreq);
