                                             ...
                                             const MPIDI_POSIX_release_gather_opcode_t operation)
{
    ...
    if (initialize_flags || initialize_bcast_buf || initialize_reduce_buf) {
        ...
        if (rank == 0) {
            ...
            if ((tmp_shm_counter + memory_to_be_allocated) >
                ...
                fallback = 1;
                MPIR_Bcast_impl(&fallback, 1, MPI_INT, 0, comm_ptr, coll_attr);
                ...
                fallback = 0;
                mpi_errno = MPIR_Bcast_impl(&fallback, 1, MPI_INT, 0, comm_ptr, coll_attr);
    ...
    if (initialize_flags) {
        ...
        MPIR_ERR_COLL_CHECKANDCONT(mpi_errno_ret, coll_attr, mpi_errno_ret);
