                                             ...
                                             const MPIDI_POSIX_release_gather_opcode_t operation)
{
    ...
    if (initialize_flags || initialize_bcast_buf || initialize_reduce_buf) {
        ...
        if (rank == 0) {
            ...
            if ((tmp_shm_counter + memory_to_be_allocated) >
                ...
                fallback = 1;
                MPIR_Bcast_fallback(&fallback, 1, MPIR_INT_INTERNAL, 0, comm_ptr, coll_attr);
    ...
    if (initialize_flags) {
        ...
        if (MPIR_CVAR_ENABLE_INTRANODE_TOPOLOGY_AWARE_TREES &&
            ...
            if (MPIR_hwtopo_is_initialized()) {
                ...
                mpi_errno =
                    MPIR_Treealgo_tree_create(rank, num_ranks,
                                              RELEASE_GATHER_FIELD(comm_ptr,
                                                                   reduce_tree_type_large),
                                              RELEASE_GATHER_FIELD(comm_ptr,
                                                                   reduce_tree_kval_large), 0,
            ...
            mpi_errno = MPIR_Allreduce_fallback(MPI_IN_PLACE, topotree_fail, 2, MPIR_INT_INTERNAL,
        ...
        mpi_errno = MPIR_Barrier_fallback(comm_ptr, coll_attr);
        MPIR_ERR_CHECK(mpi_errno);
    }
