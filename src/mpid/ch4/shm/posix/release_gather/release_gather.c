                                             ...
                                             const MPIDI_POSIX_release_gather_opcode_t operation)
{
    ...
    if (initialize_flags || initialize_bcast_buf || initialize_reduce_buf) {
        ...
        if (rank == 0) {
            ...
            tmp_shm_counter = MPL_inter_atomic_acquire_load_uint64(MPIDI_POSIX_shm_limit_counter);
            ...
            if ((tmp_shm_counter + memory_to_be_allocated) >
                ...
                MPL_inter_atomic_fetch_add_uint64(MPIDI_POSIX_shm_limit_counter,
                                                  memory_to_be_allocated);
                fallback = 0;
