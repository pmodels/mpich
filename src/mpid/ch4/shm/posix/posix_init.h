...
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_mpi_init_hook(int rank, int size, int *n_vnis_provided)
{
    ...
    *n_vnis_provided = 1;
    ...
    grank = MPIDI_CH4U_rank_to_lpid(rank, MPIR_Process.comm_world);
...
#undef FUNCNAME
#define FUNCNAME MPIDI_POSIX_mpi_finalize_hook
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_get_node_id(MPIR_Comm * comm, int rank,
                                                     MPID_Node_id_t * id_p)
{
