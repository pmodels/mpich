...
static int gpu_ipc_async_poll(MPIR_Async_thing * thing)
{
    ...
    if (p->typerep_req) {
        MPIR_Typerep_test(*(p->typerep_req), &is_done);
    } else {
    ...
    if (is_done) {
        ...
        mpi_errno = MPIDI_GPU_ipc_handle_unmap(p->src_buf, p->gpu_handle, 0);
        MPIR_Assertp(mpi_errno == MPI_SUCCESS);
        mpi_errno = MPIDI_IPC_complete(p->rreq, MPIDI_IPCI_TYPE__GPU);
        MPIR_Assertp(mpi_errno == MPI_SUCCESS);
...
int MPIDI_GPU_ipc_async_start(MPIR_Request * rreq, void *src_buf, MPIDI_GPU_ipc_handle_t gpu_handle)
{
    int mpi_errno = MPI_SUCCESS;
