...
MPL_STATIC_INLINE_PREFIX int MPIDI_IPC_xpmem_lmt_coop_copy(const void *src_buf,
                                                           ...
                                                           uint64_t req_ptr,
                                                           MPIR_Request * local_req,
                                                           MPIDI_IPC_xpmem_fin_type_t * fin_type,
                                                           MPIDI_IPC_xpmem_copy_type_t * copy_type)
{
    ...
    if (cur_chunk == total_chunk)
        *fin_type = MPIDI_IPC_XPMEM_LOCAL_FIN;  /* copy is only locally complete */
    else
        *fin_type = MPIDI_IPC_XPMEM_BOTH_FIN;   /* copy is done by both sides */

    if (num_local_copy == total_chunk)
        *copy_type = MPIDI_IPC_XPMEM_COPY_ALL;  /* the process copies all chunks */
    else if (num_local_copy == 0)
        *copy_type = MPIDI_IPC_XPMEM_COPY_ZERO; /* the process copies zero chunk */
    else
                                                                  ...
                                                                  MPIR_Request * rreq)
{
    ...
    MPIDI_IPC_XPMEM_REQUEST(rreq, counter_ptr) =
        (MPIDI_IPC_xpmem_cnt_t *) MPIR_Handle_obj_alloc(&MPIDI_IPC_xpmem_cnt_mem);
    MPL_atomic_store_int(&MPIDI_IPC_XPMEM_REQUEST(rreq, counter_ptr)->obj.counter, 0);
    if (HANDLE_GET_KIND(MPIDI_IPC_XPMEM_REQUEST(rreq, counter_ptr)->obj.handle) ==
        HANDLE_KIND_DIRECT) {
        slmt_cts_hdr->coop_counter_direct_flag = 1;
        slmt_cts_hdr->coop_counter_offset =
            (uint64_t) MPIDI_IPC_XPMEM_REQUEST(rreq, counter_ptr) -
            (uint64_t) (&MPIDI_IPC_xpmem_cnt_mem_direct);
    } else {
        slmt_cts_hdr->coop_counter_direct_flag = 0;
        slmt_cts_hdr->coop_counter_offset = (uint64_t) MPIDI_IPC_XPMEM_REQUEST(rreq, counter_ptr);
    }
    ...
    if ((fin_type == MPIDI_IPC_XPMEM_LOCAL_FIN && copy_type == MPIDI_IPC_XPMEM_COPY_ALL) ||
        ...
        if (fin_type == MPIDI_IPC_XPMEM_BOTH_FIN) {
            ...
            MPIR_Handle_obj_free(&MPIDI_IPC_xpmem_cnt_mem,
                                 MPIDI_IPC_XPMEM_REQUEST(rreq, counter_ptr));
        } else {
            int c;
            MPIR_cc_incr(&MPIDI_IPC_xpmem_global.num_pending_cnt, &c);
        }
  ...
  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_IPC_XPMEM_HANDLE_LMT_COOP_RECV);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
