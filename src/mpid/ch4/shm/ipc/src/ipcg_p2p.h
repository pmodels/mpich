                                                        ...
                                                        MPIR_Request ** request)
{
    ...
    MPIDI_SHM_ctrl_ipc_send_contig_lmt_rts_t *slmt_req_hdr = &ctrl_hdr.ipc_slmt_contig_rts;
...
/* Sender-initialized LMT routine with noncontig send buffer and flatten datatype
 *
 * The sender provides the flatten datatype structure and the receiver
 * performs direct unpack. Compared to the pack-based protocol, it may benefit
 * performance if transfer of flatten datatype is less than that of data
 * pack. E.g., XPMEM can prepare pre-mapped buffer for direct sharing of
 * such metadata.
 */
MPL_STATIC_INLINE_PREFIX int MPIDI_IPCG_send_flatten_dtype_lmt(const void *buf, MPI_Aint count,
                                                               MPI_Datatype datatype, int rank,
                                                               int tag, MPIR_Comm * comm,
                                                               int context_offset,
                                                               MPIDI_av_entry_t * addr,
                                                               void *flatten_dtype,
                                                               size_t flatten_dtype_size,
                                                               MPIDI_SHM_IPC_type_t ipc_type,
                                                               MPIR_Request ** request)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    MPIR_Request *sreq = NULL;
    MPIDI_SHM_ctrl_hdr_t ctrl_hdr;
    MPIDI_SHM_ctrl_ipc_send_flatten_dtype_lmt_rts_t *slmt_req_hdr = &ctrl_hdr.ipc_slmt_flatten_rts;

    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_IPCG_SEND_NONCONTIG_DTYPE_LMT);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_IPCG_SEND_NONCONTIG_DTYPE_LMT);
    ...
    *request = sreq;
    ...
    slmt_req_hdr->sreq_ptr = (uint64_t) sreq;
    slmt_req_hdr->ipc_type = ipc_type;
    slmt_req_hdr->flatten_dtype = flatten_dtype;
    slmt_req_hdr->flatten_dtype_size = flatten_dtype_size;
    ...
    IPC_TRACE("send_flatten_dtype_lmt: shm ctrl_id %d, data_sz 0x%lx, sreq_ptr 0x%lx, "
              "src_lrank %d, match info[dest %d, src_rank %d, tag %d, context_id 0x%x]\n",
              MPIDI_SHM_IPC_SEND_CONTIG_LMT_RTS, slmt_req_hdr->data_sz, slmt_req_hdr->sreq_ptr,
              slmt_req_hdr->src_lrank, rank, slmt_req_hdr->src_rank, slmt_req_hdr->tag,
              slmt_req_hdr->context_id);
    ...
    mpi_errno =
        MPIDI_SHM_do_ctrl_send(rank, comm, MPIDI_SHM_IPC_SEND_FLATTEN_DTYPE_LMT_RTS, &ctrl_hdr);
    MPIR_ERR_CHECK(mpi_errno);

  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_IPCG_SEND_NONCONTIG_DTYPE_LMT);
    return mpi_errno;
  fn_fail:
    goto fn_exit;
}
                                                               ...
                                                               int src_lrank, MPIR_Request * rreq)
{
    ...
    ack_ctrl_hdr.ipc_slmt_contig_fin.req_ptr = sreq_ptr;
