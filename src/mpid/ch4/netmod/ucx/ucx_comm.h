...
MPL_STATIC_INLINE_PREFIX int MPIDI_NM_mpi_comm_create_hook(MPIR_Comm * comm)
{
    ...
    // TODO: extend this to accommodate for multiple VCIs in ucx_comm.h
    if (comm == MPIR_Process.comm_world && MPIR_CVAR_CH4_ROOTS_ONLY_PMI) {
        ...
        MPIDU_bc_allgather(comm, MPIDI_CH4_Global.node_map[0],
                           MPIDI_UCX_global.if_address[0 /*WRONG*/],
                           (int) MPIDI_UCX_global.addrname_len[0 /*WRONG*/], FALSE,
        ...
        for (i = 1, curr = 0, node = 0; i < MPIR_Comm_size(comm); i++) {
            ...
            ucx_status =
                ucp_ep_create(MPIDI_UCX_global.worker[0 /*WRONG*/], &ep_params,
