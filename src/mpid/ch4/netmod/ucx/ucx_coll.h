...
#ifdef HAVE_HCOLL
    mpi_errno = hcoll_Barrier(comm_ptr);
...
#ifdef HAVE_HCOLL
    mpi_errno = hcoll_Bcast(buffer, count, datatype, root, comm_ptr);
    if (mpi_errno != MPI_SUCCESS)
#endif
    {
...
#ifdef HAVE_HCOLL
    mpi_errno = hcoll_Allreduce(sendbuf, recvbuf, count, datatype, op, comm_ptr);
    if (mpi_errno != MPI_SUCCESS)
#endif
    {
...
#ifdef HAVE_HCOLL
    mpi_errno = hcoll_Reduce(sendbuf, recvbuf, count, datatype, op, root, comm_ptr);
    if (mpi_errno != MPI_SUCCESS)
#endif
    {
