...
MPL_STATIC_INLINE_PREFIX int MPIDI_NM_progress(int vci, int blocking)
{
    ...
    message_handle =
        ucp_tag_probe_nb(MPIDI_UCX_global.worker[vci], MPIDI_UCX_AM_TAG, MPIDI_UCX_AM_TAG, 1,
                         &info);
    while (message_handle) {
        am_buf = MPL_malloc(info.length, MPL_MEM_BUFFER);
        ucp_request = (MPIDI_UCX_ucp_request_t *) ucp_tag_msg_recv_nb(MPIDI_UCX_global.worker[vci],
                                                                      am_buf,
                                                                      info.length,
