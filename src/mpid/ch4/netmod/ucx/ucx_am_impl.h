...
MPL_STATIC_INLINE_PREFIX void MPIDI_UCX_am_isend_pipeline_req_cmpl(MPIDI_UCX_am_request_t * req,
                                                                   int handler_id)
{
                                                                   ...
                                                                   ucs_status_t status)
{
    ...
    MPIDI_UCX_am_isend_pipeline_req_cmpl(req, req->handler_id);
    ucp_request->req = NULL;
...
MPL_STATIC_INLINE_PREFIX MPIDI_UCX_ucp_request_t
    * MPIDI_UCX_ucp_am_send_iov(ucp_ep_h ep, const void *ucx_hdr, MPI_Aint ucx_hdr_sz,
                                const void *am_hdr, MPI_Aint am_hdr_sz, void *data,
                                MPI_Aint data_sz, MPIDI_UCX_am_request_t * send_req,
                                int ucx_am_handler_id, ucp_send_callback_t cb)
{
    MPIDI_UCX_ucp_request_t *ucp_request;
    char *pack_buf = NULL;
    MPI_Aint header_sz = 0;

    header_sz = ucx_hdr_sz + am_hdr_sz;
    pack_buf = (char *) MPL_malloc(header_sz, MPL_MEM_OTHER);

    MPIR_Memcpy(pack_buf, ucx_hdr, ucx_hdr_sz);
    if (am_hdr_sz) {
        MPIR_Memcpy(pack_buf + ucx_hdr_sz, am_hdr, am_hdr_sz);
    }

    send_req->iov[0].buffer = pack_buf;
    send_req->iov[0].length = header_sz;
    send_req->iov[1].buffer = data;
    send_req->iov[1].length = data_sz;

    send_req->pack_buffer = pack_buf;
    send_req->is_gpu_pack_buffer = false;

    ucp_request =
        (MPIDI_UCX_ucp_request_t *) ucp_am_send_nb(ep, ucx_am_handler_id, send_req->iov, 2,
                                                   ucp_dt_make_iov(), cb, 0);
                                                            ...
                                                            bool issue_deferred)
{
    ...
    if (!issue_deferred) {
        ...
        /* if we are not issuing the first segment, skip am_hdr */
    ...
    /* STEP2: getting pack buffer if needed and send */
    if (need_packing) {
        ...
        char *data_buf = (char *) send_buf + sizeof(ucx_hdr) + local_am_hdr_sz;
        ...
        send_req->is_gpu_pack_buffer = true;

        ucp_request = MPIDI_UCX_ucp_am_send_buffered(ep, &ucx_hdr, sizeof(ucx_hdr), am_hdr,
                                                     local_am_hdr_sz, data_buf,
                                                     send_size, send_req,
                                                     MPIDI_UCX_AM_HANDLER_ID__PIPELINE,
                                                     &MPIDI_UCX_am_isend_pipeline_callback);
    } else {
        MPIDI_Datatype_check_lb(datatype, dt_true_lb);
        mpi_errno = MPIDU_genq_private_pool_alloc_cell(MPIDI_UCX_global.am_hdr_buf_pool,
                                                       (void **) &send_req);
        ...
        ucp_request = MPIDI_UCX_ucp_am_send_iov(ep, &ucx_hdr, sizeof(ucx_hdr), am_hdr,
                                                local_am_hdr_sz, (char *) buf + dt_true_lb + offset,
                                                send_size, send_req,
                                                MPIDI_UCX_AM_HANDLER_ID__PIPELINE,
                                                &MPIDI_UCX_am_isend_pipeline_callback);
    }
    ...
    /* STEP3: handles immediate send completion or preseve info for callback */
    if (ucp_request == NULL) {
        MPIDI_UCX_am_isend_pipeline_req_cmpl(send_req, handler_id);
    } else {
    ...
    /* STEP4: cleanup deferred req if needed */
  ...
  fn_deferred:
    ...
    MPIR_Memcpy(MPIDI_UCX_AMREQUEST(sreq, deferred_req)->am_hdr, am_hdr, am_hdr_sz);
    DL_APPEND(MPIDI_UCX_global.deferred_am_isend_q, MPIDI_UCX_AMREQUEST(sreq, deferred_req));
    /* NOTE: need to hold a ref to the sreq for the deferred_req */
    MPIR_cc_incr(sreq->cc_ptr, &c);
    goto fn_exit;
}
