...
static int win_allgather(MPIR_Win * win, size_t length, uint32_t disp_unit, void **base_ptr)
{
    ...
    mpi_errno =
        MPIR_Allgather(MPI_IN_PLACE, 1, MPIR_AINT_INTERNAL,
                       rkey_sizes, 1, MPIR_AINT_INTERNAL, comm_ptr, MPIR_COLL_ATTR_INIT);
    ...
    mpi_errno =
        MPIR_Allgather(MPI_IN_PLACE, sizeof(struct ucx_share), MPIR_BYTE_INTERNAL, share_data,
                       sizeof(struct ucx_share), MPIR_BYTE_INTERNAL, comm_ptr, MPIR_COLL_ATTR_INIT);
    MPIR_ERR_CHECK(mpi_errno);
