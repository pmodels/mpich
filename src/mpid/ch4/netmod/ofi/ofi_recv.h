    ...
    *request = rreq;
    ...
    if (!dt_contig || force_gpu_pack) {
        ...
        if (force_gpu_pack && MPIR_CVAR_CH4_OFI_ENABLE_GPU_PIPELINE &&
            data_sz > MPIR_CVAR_CH4_OFI_GPU_PIPELINE_BUFFER_SZ) {
            /* Pipeline path */
            MPL_atomic_relaxed_store_int(&MPIDI_OFI_REQUEST(rreq, util_id), context_id);
            ...
            if (MPI_ANY_SOURCE == rank)
                ...
                remote_addr = MPIDI_OFI_av_to_phys(addr, sender_nic, vni_local, vni_remote);
            ...
            MPIDI_OFI_REQUEST(rreq, pipeline_info.vni_local) = vni_local;
        ...
        MPIDI_OFI_REQUEST(rreq, noncontig.pack.buf) = buf;
#ifdef MPL_HAVE_ZE
        //if (dt_contig && attr.type == MPL_GPU_POINTER_DEV && data_sz <= MPIR_CVAR_CH4_IPC_GPU_FAST_COPY_MAX_SIZE) {
