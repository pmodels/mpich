                                                ...
                                                MPIR_Datatype * dt_ptr, uint64_t flags)
{
    ...
    msg.data = 0;
    msg.addr = (MPI_ANY_SOURCE == rank) ? FI_ADDR_UNSPEC : MPIDI_OFI_av_to_phys(addr, comm);
                                                ...
                                                MPIR_Request ** request, int mode, uint64_t flags)
{
    ...
    *request = rreq;
    ...
    MPIDI_OFI_REQUEST(rreq, event_id) = -1;

    match_bits = MPIDI_OFI_init_recvtag(&mask_bits, context_id, rank, comm->dev.endpoint, tag);
    ...
    if (!flags) /* Branch should compile out */
        MPIDI_OFI_CALL_RETRY(fi_trecv(MPIDI_Global.ctx[MPIDI_hash_comm_to_vci(comm)].rx,
                                      ...
                                      NULL,
                                      (MPI_ANY_SOURCE ==
                                       rank) ? FI_ADDR_UNSPEC : MPIDI_OFI_av_to_phys(addr, comm),
