                                      ...
                                      MPIR_Request ** message, uint64_t peek_flags)
{
    ...
    match_bits = MPIDI_OFI_init_recvtag(&mask_bits, comm->context_id + context_offset, source, comm->dev.endpoint, tag);
    ...
    MPIDI_OFI_CALL_RETURN(fi_trecvmsg(MPIDI_Global.ctx[MPIDI_hash_comm_to_vci(comm)].rx, &msg,
