...
MPL_STATIC_INLINE_PREFIX int MPIDI_OFI_win_init(MPIR_Win * win)
{
    ...
    MPIR_ERR_CHKANDSTMT(window_instance >= (((uint64_t) 1) << (MPIDI_Global.max_mr_key_size * 8)),
                        mpi_errno, MPI_ERR_OTHER, goto fn_fail, "**ofid_mr_reg");
    MPIDI_OFI_WIN(win).win_id = window_instance;
...
MPL_STATIC_INLINE_PREFIX int MPIDI_NM_mpi_win_free_hook(MPIR_Win * win)
{
    ...
    if (MPIDI_OFI_ENABLE_RMA) {
        MPIDI_OFI_index_allocator_free(MPIDI_Global.mr_key_allocator, MPIDI_OFI_WIN(win).win_id);
