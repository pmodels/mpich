...
MPL_STATIC_INLINE_PREFIX const
MPIDI_OFI_coll_algo_container_t *MPIDI_OFI_Ibcast_select(void *buffer,
                                                         int count,
                                                         MPI_Datatype datatype,
                                                         int root,
                                                         MPIR_Comm * comm,
                                                         ...
                                                         ATTRIBUTE((unused)))
{
    ...
    if (comm->hierarchy_kind == MPIR_COMM_HIERARCHY_KIND__PARENT &&
        ...
        return &MPIDI_OFI_Ibcast_intra_nbc_smp_cnt;
    }

    if ((nbytes < MPIR_CVAR_BCAST_SHORT_MSG_SIZE) || (comm_size < MPIR_CVAR_BCAST_MIN_PROCS)) {
        return &MPIDI_OFI_Ibcast_intra_nbc_binomial_cnt;
    } else {
        if ((nbytes < MPIR_CVAR_BCAST_LONG_MSG_SIZE) && (MPL_is_pof2(comm_size, NULL))) {
            return &MPIDI_OFI_Ibcast_intra_nbc_scatter_recursive_doubling_allgather_cnt;
        } else {
            return &MPIDI_OFI_Ibcast_intra_nbc_scatter_ring_allgather_cnt;
        }
    }
}
