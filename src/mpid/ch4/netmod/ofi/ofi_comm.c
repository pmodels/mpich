...
int MPIDI_OFI_mpi_comm_parent_commit_hook(MPIR_Comm * parent_comm, MPIR_Comm * comm)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDI_OFI_MPI_COMM_PARENT_COMMIT_HOOK);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDI_OFI_MPI_COMM_PARENT_COMMIT_HOOK);

  fn_exit:
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDI_OFI_MPI_COMM_PARENT_COMMIT_HOOK);
    return mpi_errno;
  fn_fail:
    goto fn_fail;
}
