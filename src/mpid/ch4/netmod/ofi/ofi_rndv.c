...
static int get_rndv_protocol(bool send_need_pack, bool recv_need_pack)
{
...
int MPIDI_OFI_recv_rndv_event(int vci, struct fi_cq_tagged_entry *wc, MPIR_Request * rreq)
{
    ...
    switch (get_rndv_protocol(send_need_pack, recv_need_pack)) {
    ...
    if (!cts_is_probe(hdr->flag)) {
        ...
        switch (get_rndv_protocol(send_need_pack, recv_need_pack)) {
            ...
            case MPIR_CVAR_CH4_OFI_RNDV_PROTOCOL_direct:
                /* fall through */
            default:
                mpi_errno = MPIDI_NM_am_tag_send(p->remote_rank, sreq->comm,
                                                 -1, hdr->am_tag,
