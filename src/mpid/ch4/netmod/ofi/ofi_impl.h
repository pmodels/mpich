...
#define MPIDI_OFI_AV_ADDR_ROOT(av, vci, nic) \
    MPIDI_OFI_AV(av).root_dest
#define MPIDI_OFI_AV_ADDR_NONROOT(av, vci, nic) \
    MPIDI_OFI_AV(av).all_dest[nic]
#endif
#define MPIDI_OFI_AV_ADDR(av, vci, nic) \
    ((vci==0 && nic==0) ? MPIDI_OFI_AV_ADDR_ROOT(av) : MPIDI_OFI_AV_ADDR_NONROOT(av, vci, nic))
                                                        ...
                                                        int vci, int nic)
{
    ...
    if (MPIDI_OFI_ENABLE_SCALABLE_ENDPOINTS) {
        return fi_rx_addr(MPIDI_OFI_AV_ADDR(av, vci, nic), 0, MPIDI_OFI_MAX_ENDPOINTS_BITS);
    } else {
        return MPIDI_OFI_AV_ADDR(av, vci, nic);
    }
