                                                  ...
                                                  MPIR_Request * rreq, int event_id)
{
    ...
    if (unlikely(MPIDI_OFI_is_tag_sync(wc->tag))) {
        ...
        mpi_errno = MPIDI_OFI_send_handler(MPIDI_Global.ctx[MPIDI_hash_comm_to_vci(c)].tx, NULL, 0, NULL,
                                           MPIDI_OFI_REQUEST(rreq, util_comm->rank),
                                           MPIDI_OFI_comm_to_phys(c, r),
                                           ss_bits, NULL, MPIDI_OFI_DO_INJECT,
                                           MPIDI_OFI_CALL_NO_LOCK, FALSE, MPIDI_hash_comm_to_vci(c));
        if (mpi_errno)
                                                      ...
                                                      MPIR_Request * req)
{
    ...
    if (recv->localreq && recv->cur_offset != 0) {      /* If this is true, then the message has a posted
        ...
        MPIDI_OFI_cntr_incr();
        MPIDI_OFI_CALL_RETRY(fi_read(MPIDI_Global.ctx[MPIDI_hash_comm_to_vci(recv->comm_ptr)].tx,    /* endpoint     */
                             ...
                             MPIDI_OFI_CALL_NO_LOCK, FALSE, MPIDI_hash_comm_to_vci(recv->comm_ptr));
