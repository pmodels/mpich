                                              ...
                                              int port_id, fi_addr_t * conn, MPIR_Comm * comm_ptr)
{
    ...
    if (phase == 0) {
        ...
        match_bits = MPIDI_OFI_init_recvtag(&mask_bits, port_id, MPI_ANY_SOURCE, comm_ptr->dev.endpoint, MPI_ANY_TAG);
        ...
        MPIDI_OFI_CALL_RETRY(fi_trecv(MPIDI_Global.ctx[0].rx,
                                      ...
                                      *conn,
                                      match_bits,
                                      mask_bits, &req.context), trecv, MPIDI_OFI_CALL_LOCK, FALSE, 0 /*vci*/);
        time_gap = 0.0;
        MPID_Wtime(&time_sta);
        do {
            mpi_errno = MPID_Progress_test();
    ...
    if (phase == 1) {
        ...
        match_bits = MPIDI_OFI_init_sendtag(port_id, comm_ptr->rank, comm_ptr->dev.endpoint, tag, MPIDI_OFI_DYNPROC_SEND);
        ...
        mpi_errno = MPIDI_OFI_send_handler(MPIDI_Global.ctx[0].tx,
                                           ...
                                           *conn,
                                           match_bits,
                                           (void *) &req.context,
                                           MPIDI_OFI_DO_SEND, MPIDI_OFI_CALL_LOCK, FALSE, 0 /*vci*/);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
