...
static inline void COLL_sched_init_nb(COLL_sched_t **sched,
                                      COLL_req_t         *request)
{
    COLL_sched_t *s;
    s = request->phases = (COLL_sched_t *) TSP_allocate_mem(sizeof(*s));
    memset(s,0, sizeof(*s));
    TSP_sched_init(&s->tsp_sched);
...
static inline void COLL_sched_kick(COLL_sched_t *s)
{
    ...
    while(!rc) {
        ...
        if(rc) TSP_sched_finalize(&s->tsp_sched);
        else
            COLL_progress_global.progress_fn();
    }
}
