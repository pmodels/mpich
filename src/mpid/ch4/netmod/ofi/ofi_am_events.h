...
MPL_STATIC_INLINE_PREFIX int do_long_am_recv(MPIR_Request * rreq);
                                                        ...
                                                        MPIDI_OFI_lmt_msg_payload_t * lmt_msg)
{
    ...
    if (MPIDIG_recv_initialized(rreq)) {
        mpi_errno = do_long_am_recv(rreq);
        MPIR_ERR_CHECK(mpi_errno);
                                                    ...
                                                    MPIR_Request * rreq)
{
    ...
    mpi_errno = MPIDI_OFI_do_rdma_read(p_data, src_offset, data_sz, src_rank, rreq);
    MPIR_ERR_CHECK(mpi_errno);
                                                 ...
                                                 MPIR_Request * rreq)
{
    ...
    for (int i = 0; i < iov_len && rem > 0; i++) {
        curr_len = MPL_MIN(rem, iov[i].iov_len);
        mpi_errno = MPIDI_OFI_do_rdma_read(iov[i].iov_base, src_offset + done,
                                           curr_len, src_rank, rreq);
        MPIR_ERR_CHECK(mpi_errno);
        rem -= curr_len;
        done += curr_len;
    }
...
MPL_STATIC_INLINE_PREFIX int do_long_am_recv_unpack(MPIR_Request * rreq)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_FUNC_ENTER;
    ...
    mpi_errno = MPIDI_OFI_do_rdma_read(unpack_buffer, src_offset, pack_size, src_rank, rreq);
    MPIR_ERR_CHECK(mpi_errno);
...
MPL_STATIC_INLINE_PREFIX int MPIDI_OFI_am_lmt_unpack_event(MPIR_Request * rreq)
{
    ...
    if (!ret && remain) {
        ...
        MPIDI_OFI_AM_RDMA_READ_HDR(rreq, lmt_u.unpack).pack_size = pack_size;
        MPIDI_OFI_do_rdma_read(unpack_buffer, src_offset + offset, pack_size, src_rank, rreq);
        return FALSE;
    } else {
        /* all done. */
