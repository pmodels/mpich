...
int MPIDIG_comm_revoke(MPIR_Comm *comm_ptr, int is_remote)
{
    ...
    if (0 == comm_ptr->revoked) {
         ...
         * other ranks */
        MPIDI_CH4U_COMM(comm_ptr, waiting_for_revoke) = comm_ptr->local_size - 1 - is_remote; /* Subtract the processes who already know about the revoke */
        MPL_DBG_MSG_FMT(MPIDI_CH4_DBG_COMM, VERBOSE,
                        (MPL_DBG_FDEST, "Comm %08x waiting_for_revoke: %d",
                         comm_ptr->handle, MPIDI_CH4U_COMM(comm_ptr, waiting_for_revoke)));
        ...
        for (dest = 0; dest < comm_ptr->remote_size; dest++) {
            ...
            mpi_errno = MPIDI_NM_am_isend(dest, comm_ptr, MPIDI_CH4U_COMM_REVOKE, &am_hdr,
                                          sizeof(am_hdr), NULL, 0, MPI_INT, sreq);
            if (mpi_errno)
                MPIDI_CH4U_COMM(comm_ptr, waiting_for_revoke)--;
            else
                MPIR_Wait_impl(sreq, MPI_STATUSES_IGNORE);
        ...
        if (MPIDI_CH4U_COMM(comm_ptr, waiting_for_revoke) == 0)
        ...
        MPIDI_CH4U_recvq_clean(comm_ptr);
