...
MPL_STATIC_INLINE_PREFIX bool MPIDIG_check_eager_limit(int is_local, MPI_Aint am_hdr_sz,
                                                       MPI_Aint data_sz, const void *buf,
                                                       MPI_Count count, MPI_Datatype datatype,
                                                       MPIR_Request * sreq)
{
#ifdef MPIDI_CH4_DIRECT_NETMOD
    return MPIDI_NM_am_check_eager_limit(am_hdr_sz, data_sz, buf, count, datatype, sreq);
#else
    if (is_local) {
        return MPIDI_SHM_am_check_eager_limit(am_hdr_sz, data_sz, buf, count, datatype, sreq);
    } else {
        return MPIDI_NM_am_check_eager_limit(am_hdr_sz, data_sz, buf, count, datatype, sreq);
    }
#endif
                                               ...
                                               MPIR_Request ** request, MPIR_Errflag_t errflag)
{
    ...
    if (MPIDIG_check_eager_limit(is_local, sizeof(am_hdr), data_sz, buf, count, datatype, sreq)) {
        /* EAGER send */
