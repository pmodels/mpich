...
static inline int MPIDIG_do_put_new(const void *origin_addr, int origin_count,
                                    ...
                                    void *flattened_dt, size_t flattened_sz, int protocol)
{
...
static inline int MPIDIG_do_accumulate_new(const void *origin_addr, int origin_count,
                                           ...
                                           MPIR_Request ** sreq_ptr, void *flattened_dt,
                                           size_t flattened_sz, int protocol)
{
    int mpi_errno = MPI_SUCCESS, c;
    MPIR_Request *sreq = NULL;
    ...
    if (flattened_sz) {
        ...
        memcpy(tmp_am_hdr, &am_hdr, sizeof(am_hdr));
        memcpy(tmp_am_hdr + sizeof(am_hdr), flattened_dt, flattened_sz);
        MPIDIG_REQUEST(sreq, req->areq).flattened_dt = flattened_dt;
        mpi_errno = MPIDIG_isend_impl_new(origin_addr, origin_count, origin_datatype, target_rank,
                                          0, win->comm_ptr, 0, av, &sreq, MPIR_ERR_NONE,
        ...
        MPIDIG_REQUEST(sreq, req->areq).flattened_dt = NULL;
        mpi_errno = MPIDIG_isend_impl_new(origin_addr, origin_count, origin_datatype, target_rank,
                                          0, win->comm_ptr, 0, av, &sreq, MPIR_ERR_NONE,
...
static inline int MPIDIG_do_get_accumulate_new(const void *origin_addr,
                                               int origin_count_,
                                               ...
                                               size_t flattened_sz, int protocol)
{
    ...
    if (flattened_sz) {
        ...
        mpi_errno = MPIDIG_isend_impl_new(origin_addr, origin_count, origin_datatype, target_rank,
                                          0, win->comm_ptr, 0, av, &sreq, MPIR_ERR_NONE,
                                          MPIDIG_GET_ACC_REQ, tmp_am_hdr,
                                          sizeof(am_hdr) + flattened_sz, protocol);
        MPL_free(tmp_am_hdr);
    } else {
        MPIDIG_REQUEST(sreq, req->areq).flattened_dt = NULL;
        mpi_errno = MPIDIG_isend_impl_new(origin_addr, origin_count, origin_datatype, target_rank,
                                          0, win->comm_ptr, 0, av, &sreq, MPIR_ERR_NONE,
                                          MPIDIG_GET_ACC_REQ, &am_hdr, sizeof(am_hdr), protocol);
    }
    MPIR_ERR_CHECK(mpi_errno);
...
static inline int MPIDIG_do_get_accumulate_dt(const void *origin_addr,
                                              int origin_count_,
                                              MPI_Datatype origin_datatype_,
                                              void *result_addr,
                                              int result_count,
                                              ...
                                              size_t flattened_sz, int protocol)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDIG_DO_GET_ACCUMULATE_DT);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDIG_DO_GET_ACCUMULATE_DT);
    ...
    mpi_errno = MPIDIG_isend_impl_new(flattened_dt, flattened_sz, MPI_BYTE, target_rank,
                                      0, win->comm_ptr, 0, av, &sreq, MPIR_ERR_NONE,
                                      MPIDIG_GET_ACC_DT_REQ, &am_hdr, sizeof(am_hdr), protocol);
    MPIR_ERR_CHECK(mpi_errno);
  ...
  fn_exit:
    ...
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDIG_DO_GET_ACCUMULATE_DT);
    return mpi_errno;
...
MPL_STATIC_INLINE_PREFIX int MPIDIG_mpi_rget_accumulate_new(const void *origin_addr,
                                                            int origin_count,
                                                            MPI_Datatype origin_datatype,
                                                            ...
                                                            int protocol)
{
    ...
    if (!flattened_sz) {
        mpi_errno = MPIDIG_do_get_accumulate_new(origin_addr, origin_count, origin_datatype,
        ...
        if (!is_long_dt) {
            mpi_errno = MPIDIG_do_get_accumulate_new(origin_addr, origin_count, origin_datatype,
                                                     result_addr, result_count, result_datatype,
                                                     target_rank, target_disp, target_count,
                                                     target_datatype, op, av, win, &sreq,
                                                     flattened_dt, flattened_sz, protocol);
        } else {
            mpi_errno = MPIDIG_do_get_accumulate_dt(origin_addr, origin_count, origin_datatype,
                                                    result_addr, result_count, result_datatype,
                                                    target_rank, target_disp, target_count,
                                                    target_datatype, op, av, win, &sreq,
                                                    flattened_dt, flattened_sz, protocol);
...
MPL_STATIC_INLINE_PREFIX int MPIDIG_mpi_get_accumulate_new(const void *origin_addr,
                                                           int origin_count,
                                                           MPI_Datatype origin_datatype,
                                                           void *result_addr, int result_count,
                                                           MPI_Datatype result_datatype,
                                                           ...
                                                           int protocol)
{
    ...
    if (!flattened_sz) {
        mpi_errno = MPIDIG_do_get_accumulate_new(origin_addr, origin_count, origin_datatype,
                                                 result_addr, result_count, result_datatype,
                                                 target_rank, target_disp, target_count,
                                                 target_datatype, op, av, win, NULL, NULL, 0,
                                                 protocol);
    } else {
        void *flattened_dt = NULL;
        int size = 0;
        ...
        if (!is_long_dt) {
            mpi_errno = MPIDIG_do_get_accumulate_new(origin_addr, origin_count, origin_datatype,
                                                     result_addr, result_count, result_datatype,
                                                     target_rank, target_disp, target_count,
                                                     target_datatype, op, av, win, NULL,
                                                     flattened_dt, flattened_sz, protocol);
        } else {
            mpi_errno = MPIDIG_do_get_accumulate_dt(origin_addr, origin_count, origin_datatype,
                                                    result_addr, result_count, result_datatype,
...
MPL_STATIC_INLINE_PREFIX int MPIDIG_mpi_fetch_and_op_new(const void *origin_addr, void *result_addr,
                                                         ...
                                                         int protocol)
{
    ...
    mpi_errno = MPIDIG_mpi_get_accumulate_new(origin_addr, 1, datatype, result_addr, 1, datatype,
                                              target_rank, target_disp, 1, datatype, op, av, win,
                                              0, 0, protocol);
    MPIR_ERR_CHECK(mpi_errno);
  fn_exit:
