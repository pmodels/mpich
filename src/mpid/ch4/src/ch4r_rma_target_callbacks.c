...
static void win_unlock_proc(const MPIDIG_win_cntrl_msg_t * info, MPIR_Win * win);
...
{
    int mpi_errno = MPI_SUCCESS;
    MPIDI_win_lock_recvd_t *lock_recvd_q = &MPIDIU_WIN(win, sync).lock_recvd;
    ...
    if ((lock_recvd_q->head != NULL) && ((lock_recvd_q->count == 0) ||
        ...
        MPIDI_win_lock_t *lock = lock_recvd_q->head;
...
static void win_lock_req_proc(int handler_id, const MPIDIG_win_cntrl_msg_t * info, MPIR_Win * win)
{
    ...
    MPIDI_win_lock_t *lock = (MPIDI_win_lock_t *)
        MPL_calloc(1, sizeof(MPIDI_win_lock_t), MPL_MEM_RMA);
...
static void win_lock_ack_proc(int handler_id, const MPIDIG_win_cntrl_msg_t * info, MPIR_Win * win)
{
    ...
    if (handler_id == MPIDIG_WIN_LOCK_ACK) {
        MPIDI_win_target_t *target_ptr = MPIDIU_win_target_find(win, info->origin_rank);
        ...
        MPIDIU_WIN(win, sync).lockall.allLocked += 1;
    } else {
        MPIR_Assert(0);
    }
...
#undef FUNCNAME
#define FUNCNAME win_unlock_proc
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static void win_unlock_proc(const MPIDIG_win_cntrl_msg_t * info, MPIR_Win * win)
{
    ...
    --MPIDIU_WIN(win, sync).lock_recvd.count;
    MPIR_Assert((int) MPIDIU_WIN(win, sync).lock_recvd.count >= 0);
    win_lock_advance(win);
...
static void win_post_proc(const MPIDIG_win_cntrl_msg_t * info, MPIR_Win * win)
{
    ...
    ++MPIDIU_WIN(win, sync).pw.count;

    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDIG_WIN_POST_PROC);
}

#undef FUNCNAME
#define FUNCNAME win_unlock_done
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static void win_unlock_done(const MPIDIG_win_cntrl_msg_t * info, MPIR_Win * win)
{
    ...
    } else if (MPIDIU_WIN(win, sync).access_epoch_type == MPIDIU_EPOTYPE_LOCK_ALL) {
        MPIR_Assert((int) MPIDIU_WIN(win, sync).lockall.allLocked > 0);
        MPIDIU_WIN(win, sync).lockall.allLocked -= 1;
    } else {
        MPIR_Assert(0);
    }
...
#ifndef MPIDI_CH4_DIRECT_NETMOD
    if (MPIDIU_WIN(win, shm_allocated)) {
...
#ifndef MPIDI_CH4_DIRECT_NETMOD
        if (MPIDI_CH4_rank_is_local(MPIDIG_REQUEST(req, rank), win->comm_ptr))
...
int MPIDI_get_ack_target_cmpl_cb(MPIR_Request * rreq)
{
    ...
    MPIDIU_win_remote_cmpl_cnt_decr(win, MPIDIG_REQUEST(greq, rank));
...
int MPIDI_get_acc_ack_target_cmpl_cb(MPIR_Request * areq)
{
    ...
    MPIDIU_win_remote_cmpl_cnt_decr(win, MPIDIG_REQUEST(areq, rank));
    MPIDIU_win_remote_acc_cmpl_cnt_decr(win, MPIDIG_REQUEST(areq, rank));
...
int MPIDI_cswap_ack_target_cmpl_cb(MPIR_Request * rreq)
{
    ...
    win = MPIDIG_REQUEST(rreq, req->creq.win_ptr);
    MPIDIU_win_remote_cmpl_cnt_decr(win, MPIDIG_REQUEST(rreq, rank));
    MPIDIU_win_remote_acc_cmpl_cnt_decr(win, MPIDIG_REQUEST(rreq, rank));
                                ...
                                MPIR_Request ** req)
{
    ...
    MPIDIU_win_remote_cmpl_cnt_decr(win, MPIDIG_REQUEST(preq, rank));
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIDI_get_acc_ack_target_msg_cb(int handler_id, void *am_hdr, void **data,
                                    ...
                                    MPIDIG_am_target_cmpl_cb * target_cmpl_cb, MPIR_Request ** req)
{
    ...
    *is_contig = dt_contig;
    ...
    if (dt_contig) {
        ...
        *data = (char *) MPIDIG_REQUEST(areq, req->areq.result_addr) + dt_true_lb;
        ...
        MPIR_Segment_init(MPIDIG_REQUEST(areq, req->areq.result_addr),
                          MPIDIG_REQUEST(areq, req->areq.result_count),
                          MPIDIG_REQUEST(areq, req->areq.result_datatype), segment_ptr);
        ...
        last = data_sz;
        MPIR_Segment_pack_vector(segment_ptr, 0, &last, MPIDIG_REQUEST(areq, req->iov), &n_iov);
    ...
    *target_cmpl_cb = MPIDI_get_acc_ack_target_cmpl_cb;
    MPIDIG_REQUEST(areq, req->seq_no) = OPA_fetch_and_add_int(&MPIDI_CH4_Global.nxt_seq_no, 1);
                                  ...
                                  MPIR_Request ** req)
{
    ...
    *target_cmpl_cb = MPIDI_cswap_ack_target_cmpl_cb;
    MPIDIG_REQUEST(creq, req->seq_no) = OPA_fetch_and_add_int(&MPIDI_CH4_Global.nxt_seq_no, 1);
                            ...
                            MPIR_Request ** req)
{
    ...
    rreq = MPIDIG_am_request_create(MPIR_REQUEST_KIND__RMA, 1);
...
#ifndef MPIDI_CH4_DIRECT_NETMOD
    if (MPIDI_CH4_rank_is_local(MPIDIG_REQUEST(origin_req, rank), win->comm_ptr))
                                    ...
                                    MPIR_Request ** req)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_Request *rreq, *origin_req;
    MPIDIG_acc_iov_msg_t *msg_hdr = (MPIDIG_acc_iov_msg_t *) am_hdr;
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIDI_get_acc_data_target_msg_cb(int handler_id, void *am_hdr, void **data,
                                     size_t * p_data_sz, int *is_contig,
                                     MPIDIG_am_target_cmpl_cb * target_cmpl_cb, MPIR_Request ** req)
{
    int mpi_errno = MPI_SUCCESS;
                            ...
                            MPIR_Request ** req)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_Request *rreq = NULL;
    MPIDIG_get_req_msg_t *msg_hdr = (MPIDIG_get_req_msg_t *) am_hdr;
    struct iovec *iov;
    MPIR_Win *win;
    uintptr_t base;
    size_t offset;
                                ...
                                MPIR_Request ** req)
{
    ...
    greq = MPIDIG_am_request_create(MPIR_REQUEST_KIND__RMA, 1);
    ...
    *target_cmpl_cb = MPIDI_get_ack_target_cmpl_cb;
    MPIDIG_REQUEST(greq, req->seq_no) = OPA_fetch_and_add_int(&MPIDI_CH4_Global.nxt_seq_no, 1);
    ...
    *is_contig = dt_contig;
    ...
    if (dt_contig) {
        ...
        *data = (char *) (MPIDIG_REQUEST(rreq, req->greq.addr) + dt_true_lb);
        ...
        MPIR_Segment_init((void *) MPIDIG_REQUEST(rreq, req->greq.addr),
                          MPIDIG_REQUEST(rreq, req->greq.count),
                          MPIDIG_REQUEST(rreq, req->greq.datatype), segment_ptr);
        last = data_sz;
        MPIR_Segment_count_contig_blocks(segment_ptr, 0, &last, &num_iov);
        n_iov = (int) num_iov;
        MPIR_Assert(n_iov > 0);
