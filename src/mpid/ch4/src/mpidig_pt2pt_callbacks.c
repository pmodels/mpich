...
int MPIDIG_do_cts(MPIR_Request * rreq)
{
    ...
    if (can_do_tag(rreq)) {
        am_hdr.tag = rreq->comm->next_am_tag++;
        if (rreq->comm->next_am_tag >= MPIR_Process.attrs.tag_ub) {
            rreq->comm->next_am_tag = 0;
        }
        CH4_CALL(am_tag_recv(MPIDIG_REQUEST(rreq, u.recv.source), rreq->comm,
