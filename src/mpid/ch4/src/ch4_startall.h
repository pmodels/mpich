    ...
    for (i = 0; i < count; i++) {
        ...
        if (MPIDIU_REQUEST_ANYSOURCE_PARTNER(req)) {
            ...
            if (mpi_errno == MPI_SUCCESS) {
                mpi_errno = MPIDI_NM_mpi_startall(1, &MPIDIU_REQUEST_ANYSOURCE_PARTNER(req));
                MPIDIU_REQUEST_ANYSOURCE_PARTNER(req->u.persist.real_request) =
                    MPIDIU_REQUEST_ANYSOURCE_PARTNER(req)->u.persist.real_request;
                MPIDIU_REQUEST_ANYSOURCE_PARTNER(MPIDIU_REQUEST_ANYSOURCE_PARTNER
                                                 (req)->u.persist.real_request) =
                    req->u.persist.real_request;
            }
        } else if (MPIDIU_REQUEST(req, is_local))
            mpi_errno = MPIDI_SHM_mpi_startall(1, &req);
        else
