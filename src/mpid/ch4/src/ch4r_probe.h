...
MPL_STATIC_INLINE_PREFIX int MPIDIG_mpi_iprobe(int source,
                                               int tag,
                                               MPIR_Comm * comm,
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDIG_mpi_improbe(int source,
                                                int tag,
                                                MPIR_Comm * comm,
                                                int context_offset,
...
#undef FUNCNAME
#define FUNCNAME MPIDIG_mprobe
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIDIG_mprobe(int source,
                                           int tag,
                                           MPIR_Comm * comm,
                                           int context_offset,
                                           MPIR_Request ** message, MPI_Status * status)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL(MPID_STATE_MPIDIG_MPROBE);
    MPIR_FUNC_VERBOSE_ENTER(MPID_STATE_MPIDIG_MPROBE);
    while (!flag) {
        mpi_errno = MPIDIG_mpi_improbe(source, tag, comm, context_offset, &flag, message, status);
    }
    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDIG_MPROBE);
    return mpi_errno;
}
