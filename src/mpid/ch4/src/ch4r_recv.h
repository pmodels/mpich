...
static inline int MPIDIG_reply_ssend(MPIR_Request * rreq)
{
    ...
    if (MPIDIG_REQUEST(rreq, req->peer_msg_req_ptr)) {
        ack_msg.sreq_ptr = MPIDIG_REQUEST(rreq, req->peer_msg_req_ptr);
    } else {
        ack_msg.sreq_ptr = MPIDIG_REQUEST(rreq, req->peer_req_ptr);
    }
