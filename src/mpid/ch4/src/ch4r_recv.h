...
static inline int MPIDI_CH4I_reply_ssend(MPIR_Request * rreq)
{
...
#undef FUNCNAME
#define FUNCNAME MPIDI_CH4I_handle_unexp_mrecv
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int MPIDI_CH4I_handle_unexp_mrecv(MPIR_Request * rreq)
{
    int mpi_errno = MPI_SUCCESS;
    uint64_t msg_tag;
    size_t message_sz;
                                                   ...
                                                   MPIR_Request * message, MPIR_Request ** rreqp)
{
    ...
    *rreqp = message;
    ...
    if (MPIDI_CH4U_REQUEST(message, req->status) & MPIDI_CH4U_REQ_BUSY) {
        ...
        mpi_errno = MPIDI_CH4I_handle_unexp_mrecv(message);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
    /* MPIDI_CS_EXIT(); */
