        ...
        if (MPIDIU_LUPID_IS_NEW_AVT((*remote_lupids)[i])) {
            ...
            if (_avtid != 0) {
                ...
                if (remote_node_ids[i] > MPIDI_CH4_Global.max_node_id) {
                    MPIDI_CH4_Global.max_node_id = remote_node_ids[i];
                }
                                                              ...
                                                              int *remote_node_ids)
{
    ...
    if (local_comm->rank == local_leader) {
        ...
        mpi_errno = MPIR_Bcast__intra__auto(map_info, 4, MPI_INT, local_leader, local_comm, &errflag );
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);

        if (!pure_intracomm) {
            mpi_errno = MPIR_Bcast__intra__auto(remote_upid_size, *remote_size, MPI_UNSIGNED_LONG,
                                         local_leader, local_comm, &errflag );
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
            mpi_errno = MPIR_Bcast__intra__auto(remote_upids, upid_recv_size, MPI_BYTE,
                                         local_leader, local_comm, &errflag );
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
            mpi_errno =
                MPIR_Bcast__intra__auto(remote_node_ids, (*remote_size) * sizeof(int), MPI_BYTE,
                                 local_leader, local_comm, &errflag);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        }
        else {
            mpi_errno = MPIR_Bcast__intra__auto(*remote_lupids, *remote_size, MPI_INT,
                                         local_leader, local_comm, &errflag);
        }
    }
    else {
        ...
        *is_low_group = map_info[2];
        ...
        if (!pure_intracomm) {
            ...
            mpi_errno = MPIR_Bcast__intra__auto(_remote_upid_size, *remote_size, MPI_UNSIGNED_LONG,
                                         local_leader, local_comm, &errflag);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
            MPIR_CHKLMEM_MALLOC(_remote_upids, char*, upid_recv_size * sizeof(char),
                                mpi_errno, "_remote_upids", MPL_MEM_COMM);
            mpi_errno = MPIR_Bcast__intra__auto(_remote_upids, upid_recv_size, MPI_BYTE,
                                         local_leader, local_comm, &errflag);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
            ...
            mpi_errno =
                MPIR_Bcast__intra__auto(_remote_node_ids, (*remote_size) * sizeof(int), MPI_BYTE,
                                 local_leader, local_comm, &errflag);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
