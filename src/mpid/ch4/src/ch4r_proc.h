...
static inline int MPIDIU_get_node_id(MPIR_Comm * comm, int rank, int *id_p)
{
...
static inline int MPIDIU_get_max_node_id(MPIR_Comm * comm, int *max_id_p)
{
    int mpi_errno = MPI_SUCCESS;
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int MPIDIU_build_nodemap(int myrank, MPIR_Comm * comm, int sz, int *out_nodemap,
                                       int *sz_out)
{
    int ret;
...
static inline int MPIDIU_build_nodemap_avtid(int myrank, MPIR_Comm * comm, int sz, int avtid)
{
    ...
    ret = MPIDIU_build_nodemap(myrank, comm, sz, MPIDI_CH4_Global.node_map[avtid],
