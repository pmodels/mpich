                    ...
                    MPIR_Comm * comm_ptr, int create_flavor, int model)
{
    ...
    *win_ptr = win;
    ...
    mpi_errno = MPIR_Allreduce(&no_local, &all_no_local, 1, MPIR_C_BOOL_INTERNAL,
                               MPI_LAND, comm_ptr, MPIR_COLL_ATTR_INIT);
    MPIR_ERR_CHECK(mpi_errno);
    if (all_no_local)
        MPIDI_WIN(win, winattr) |= MPIDI_WINATTR_ACCU_NO_SHM;
    ...
    *base_ptr = NULL;
    ...
    if (shm_comm_ptr != NULL) {
        ...
        mpi_errno = MPIR_Allgather(MPI_IN_PLACE,
                                   ...
                                   shm_comm_ptr, MPIR_COLL_ATTR_INIT);
...
int MPIDIG_mpi_win_set_info(MPIR_Win * win, MPIR_Info * info)
{
    ...
    mpi_errno = MPIR_Barrier(win->comm_ptr, MPIR_COLL_ATTR_INIT);
  fn_exit:
    MPIR_FUNC_EXIT;
    return mpi_errno;
  fn_fail:
                                   ...
                                   MPIR_Comm * comm_ptr, void **base_ptr, MPIR_Win ** win_ptr)
{
    ...
    mpi_errno = MPIR_Barrier(comm_ptr, MPIR_COLL_ATTR_INIT);
                            ...
                            void *baseptr, MPIR_Win ** win_ptr)
{
    ...
    mpi_errno = MPIR_Barrier(comm, MPIR_COLL_ATTR_INIT);
