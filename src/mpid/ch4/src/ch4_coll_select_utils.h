...
MPL_STATIC_INLINE_PREFIX MPIU_COLL_SELECTION_storage_handler
MPIDI_get_container_storage(MPIU_COLL_SELECTON_coll_signature_t * coll_sig)
{
    return (MPIDI_CH4_COMM(coll_sig->comm).coll_tuning)[coll_sig->coll_id];
}

MPL_STATIC_INLINE_PREFIX int
MPIDI_check_container(MPIU_COLL_SELECTON_coll_signature_t * coll_sig, void *container)
{
    if (container != NULL) {
        return 1;
    }
...
MPL_STATIC_INLINE_PREFIX const MPIDI_coll_algo_container_t
    * MPIDI_get_container_default_value(MPIU_COLL_SELECTON_coll_signature_t * coll_sig)
{
...
MPL_STATIC_INLINE_PREFIX const MPIDI_coll_algo_container_t
    * MPIDI_coll_select(MPIU_COLL_SELECTON_coll_signature_t * coll_sig)
{
    void *container = NULL;
    int is_valid = 0;
    MPIU_COLL_SELECTION_storage_handler storage = MPIU_COLL_SELECTION_NULL_ENTRY;
    MPIU_COLL_SELECTION_storage_handler entry = MPIU_COLL_SELECTION_NULL_ENTRY;
    MPIU_COLL_SELECTION_match_pattern_t match_pattern;

    storage = MPIDI_get_container_storage(coll_sig);

    if (storage != MPIU_COLL_SELECTION_NULL_ENTRY) {
        MPIU_COLL_SELECTION_init_coll_match_pattern(coll_sig, &match_pattern,
                                                    MPIU_COLL_SELECTION_CONTAINER);

        entry = MPIU_COLL_SELECTION_find_entry(storage, &match_pattern);

        if (entry != MPIU_COLL_SELECTION_NULL_ENTRY) {
            container = MPIU_COLL_SELECTION_get_container(entry);
