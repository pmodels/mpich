    ...
    while (curr_ptr) {
        if (!strcmp(curr_ptr->key, "no_locks")) {
            if (!strcmp(curr_ptr->value, "true"))
                MPIDIU_WIN(win, info_args).no_locks = 1;
            else if (!strcmp(curr_ptr->value, "false"))
                MPIDIU_WIN(win, info_args).no_locks = 0;
        } else if (!strcmp(curr_ptr->key, "accumulate_ordering")) {
            save_ordering = MPIDIU_WIN(win, info_args).accumulate_ordering;
            MPIDIU_WIN(win, info_args).accumulate_ordering = 0;
            ...
            while (token) {
                if (!memcmp(token, "rar", 3))
                    MPIDIU_WIN(win, info_args).accumulate_ordering =
                        (MPIDIU_WIN(win, info_args).accumulate_ordering | MPIDI_ACCU_ORDER_RAR);
                else if (!memcmp(token, "raw", 3))
                    MPIDIU_WIN(win, info_args).accumulate_ordering =
                        (MPIDIU_WIN(win, info_args).accumulate_ordering | MPIDI_ACCU_ORDER_RAW);
                else if (!memcmp(token, "war", 3))
                    MPIDIU_WIN(win, info_args).accumulate_ordering =
                        (MPIDIU_WIN(win, info_args).accumulate_ordering | MPIDI_ACCU_ORDER_WAR);
                else if (!memcmp(token, "waw", 3))
                    MPIDIU_WIN(win, info_args).accumulate_ordering =
                        (MPIDIU_WIN(win, info_args).accumulate_ordering | MPIDI_ACCU_ORDER_WAW);
                else
            ...
            if (MPIDIU_WIN(win, info_args).accumulate_ordering == 0)
                MPIDIU_WIN(win, info_args).accumulate_ordering = save_ordering;
        } else if (!strcmp(curr_ptr->key, "accumulate_ops")) {
            if (!strcmp(curr_ptr->value, "same_op"))
                MPIDIU_WIN(win, info_args).accumulate_ops = MPIDI_ACCU_SAME_OP;
            else if (!strcmp(curr_ptr->value, "same_op_no_op"))
                MPIDIU_WIN(win, info_args).accumulate_ops = MPIDI_ACCU_SAME_OP_NO_OP;
        } else if (!strcmp(curr_ptr->key, "same_disp_unit")) {
            if (!strcmp(curr_ptr->value, "true"))
                MPIDIU_WIN(win, info_args).same_disp_unit = 1;
            else if (!strcmp(curr_ptr->value, "false"))
                MPIDIU_WIN(win, info_args).same_disp_unit = 0;
        } else if (!strcmp(curr_ptr->key, "same_size")) {
            if (!strcmp(curr_ptr->value, "true"))
                MPIDIU_WIN(win, info_args).same_size = 1;
            else if (!strcmp(curr_ptr->value, "false"))
                MPIDIU_WIN(win, info_args).same_size = 0;
        } else if (!strcmp(curr_ptr->key, "alloc_shared_noncontig")) {
            if (!strcmp(curr_ptr->value, "true"))
                MPIDIU_WIN(win, info_args).alloc_shared_noncontig = 1;
            else if (!strcmp(curr_ptr->value, "false"))
                MPIDIU_WIN(win, info_args).alloc_shared_noncontig = 0;
        } else if (!strcmp(curr_ptr->key, "alloc_shm")) {
            if (!strcmp(curr_ptr->value, "true"))
                MPIDIU_WIN(win, info_args).alloc_shm = 1;
            else if (!strcmp(curr_ptr->value, "false"))
                MPIDIU_WIN(win, info_args).alloc_shm = 0;
        }
      next:
...
static inline int MPIDIG_mpi_win_start(MPIR_Group * group, int assert, MPIR_Win * win)
{
    ...
    MPIDIU_ACCESS_EPOCH_CHECK_NONE(win, mpi_errno, goto fn_fail);
    ...
    MPIDIU_PROGRESS_WHILE(group->size != (int) MPIDIU_WIN(win, sync).pw.count);
  no_check:
    MPIDIU_WIN(win, sync).pw.count = 0;

    MPIR_ERR_CHKANDJUMP((MPIDIU_WIN(win, sync).sc.group != NULL),
                        mpi_errno, MPI_ERR_GROUP, "**group");
    MPIDIU_WIN(win, sync).sc.group = group;
    MPIDIU_WIN(win, sync).access_epoch_type = MPIDIU_EPOTYPE_START;
...
static inline int MPIDIG_mpi_win_complete(MPIR_Win * win)
{
    ...
    MPIDIU_ACCESS_EPOCH_CHECK(win, MPIDIU_EPOTYPE_START, mpi_errno, return mpi_errno);

    group = MPIDIU_WIN(win, sync).sc.group;
    ...
    msg.win_id = MPIDIU_WIN(win, win_id);
    ...
    do {
        MPIDIU_PROGRESS();
        MPIDIU_win_check_group_local_completed(win, ranks_in_win_grp, group->size,
                                               &all_local_completed);
    ...
    if (MPIR_CVAR_CH4_RMA_MEM_EFFICIENT)
        MPIDIU_win_target_cleanall(win);
    MPIDIU_WIN(win, sync).access_epoch_type = MPIDIU_EPOTYPE_NONE;
    MPIR_Group_release(MPIDIU_WIN(win, sync).sc.group);
    MPIDIU_WIN(win, sync).sc.group = NULL;
...
static inline int MPIDIG_mpi_win_post(MPIR_Group * group, int assert, MPIR_Win * win)
{
    ...
    MPIDIU_EXPOSURE_EPOCH_CHECK_NONE(win, mpi_errno, goto fn_fail);

    MPIR_Group_add_ref(group);
    MPIR_ERR_CHKANDJUMP((MPIDIU_WIN(win, sync).pw.group != NULL),
                        mpi_errno, MPI_ERR_GROUP, "**group");

    MPIDIU_WIN(win, sync).pw.group = group;
    MPIR_Assert(group != NULL);
    if (assert & MPI_MODE_NOCHECK) {
        goto no_check;
    }
  ...
  no_check:
    MPIDIU_WIN(win, sync).exposure_epoch_type = MPIDIU_EPOTYPE_POST;
  fn_exit:
    MPL_free(ranks_in_win_grp);
...
static inline int MPIDIG_mpi_win_wait(MPIR_Win * win)
{
    ...
    MPIDIU_EXPOSURE_EPOCH_CHECK(win, MPIDIU_EPOTYPE_POST, mpi_errno, goto fn_fail);
    group = MPIDIU_WIN(win, sync).pw.group;
    MPIDIU_PROGRESS_WHILE(group->size != (int) MPIDIU_WIN(win, sync).sc.count);

    MPIDIU_WIN(win, sync).sc.count = 0;
    MPIDIU_WIN(win, sync).pw.group = NULL;
...
static inline int MPIDIG_mpi_win_test(MPIR_Win * win, int *flag)
{
    ...
    if (group->size == (int) MPIDIU_WIN(win, sync).sc.count) {
        MPIDIU_WIN(win, sync).sc.count = 0;
        MPIDIU_WIN(win, sync).pw.group = NULL;
        *flag = 1;
        MPIR_Group_release(group);
...
static inline int MPIDIG_mpi_win_lock(int lock_type, int rank, int assert, MPIR_Win * win)
{
    ...
    MPIDIU_LOCK_EPOCH_CHECK_NONE(win, rank, mpi_errno, goto fn_fail);

    MPIDI_win_target_t *target_ptr = MPIDIU_win_target_get(win, rank);

    MPIDI_win_target_sync_lock_t *slock = &target_ptr->sync.lock;
  ...
  fn_exit0:
    ...
    MPIDIU_WIN(win, sync).lock.count++;
...
static inline int MPIDIG_mpi_win_unlock(int rank, MPIR_Win * win)
{
    ...
    MPIDIU_ACCESS_EPOCH_CHECK(win, MPIDIU_EPOTYPE_LOCK, mpi_errno, return mpi_errno);
    if (rank == MPI_PROC_NULL)
        goto fn_exit0;
    ...
    MPIDIU_EPOCH_CHECK_TARGET_LOCK(target_ptr, mpi_errno, return mpi_errno);
  ...
  no_check:
    /* In performance-efficient mode, all allocated targets are freed at win_finalize. */
    if (MPIR_CVAR_CH4_RMA_MEM_EFFICIENT)
        MPIDIU_win_target_delete(win, target_ptr);

  fn_exit0:
    MPIR_Assert(MPIDIU_WIN(win, sync).lock.count > 0);
    MPIDIU_WIN(win, sync).lock.count--;

    /* Reset window epoch only when all per-target lock epochs are closed. */
    if (MPIDIU_WIN(win, sync).lock.count == 0) {
        MPIDIU_WIN(win, sync).access_epoch_type = MPIDIU_EPOTYPE_NONE;
    }
...
static inline int MPIDIG_mpi_win_get_info(MPIR_Win * win, MPIR_Info ** info_p_p)
{
    ...
    if (MPIDIU_WIN(win, info_args).no_locks)
...
#define BUFSIZE 32
        ...
        if (MPIDIU_WIN(win, info_args).accumulate_ordering & MPIDI_ACCU_ORDER_RAR)
            c += snprintf(buf, BUFSIZE, "rar");

        if (MPIDIU_WIN(win, info_args).accumulate_ordering & MPIDI_ACCU_ORDER_RAW)
            c += snprintf(buf + c, BUFSIZE - c, "%sraw", (c > 0) ? "," : "");

        if (MPIDIU_WIN(win, info_args).accumulate_ordering & MPIDI_ACCU_ORDER_WAR)
            c += snprintf(buf + c, BUFSIZE - c, "%swar", (c > 0) ? "," : "");

        if (MPIDIU_WIN(win, info_args).accumulate_ordering & MPIDI_ACCU_ORDER_WAW)
...
#undef BUFSIZE
    ...
    if (MPIDIU_WIN(win, info_args).accumulate_ops == MPIDI_ACCU_SAME_OP)
    ...
    if (MPIDIU_WIN(win, info_args).alloc_shared_noncontig)
    ...
    if (MPIDIU_WIN(win, info_args).same_size)
    ...
    if (MPIDIU_WIN(win, info_args).same_disp_unit)
    ...
    if (MPIDIU_WIN(win, info_args).alloc_shm)
...
static inline int MPIDIG_mpi_win_fence(int massert, MPIR_Win * win)
{
    ...
    MPIDIU_FENCE_EPOCH_CHECK(win, mpi_errno, goto fn_fail);
    ...
    do {
        MPIDIU_PROGRESS();
    } while (MPIR_cc_get(MPIDIU_WIN(win, local_cmpl_cnts)) != 0);
    MPIDIU_EPOCH_FENCE_EVENT(win, massert);
...
static inline int MPIDIG_mpi_win_flush(int rank, MPIR_Win * win)
{
    ...
    /* Check window lock epoch.
     * PROC_NULL does not update per-target epoch. */
    MPIDIU_EPOCH_CHECK_PASSIVE(win, mpi_errno, return mpi_errno);
    if (rank == MPI_PROC_NULL)
        goto fn_exit;
    ...
    if (target_ptr) {
        if (MPIDIU_WIN(win, sync).access_epoch_type == MPIDIU_EPOTYPE_LOCK)
            MPIDIU_EPOCH_CHECK_TARGET_LOCK(target_ptr, mpi_errno, goto fn_fail);
    }
...
static inline int MPIDIG_mpi_win_flush_local_all(MPIR_Win * win)
{
    ...
    MPIDIU_EPOCH_CHECK_PASSIVE(win, mpi_errno, goto fn_fail);
    ...
    do {
        MPIDIU_PROGRESS();
        MPIDIU_win_check_all_targets_local_completed(win, &all_local_completed);
    } while (all_local_completed != 1);
...
static inline int MPIDIG_mpi_win_unlock_all(MPIR_Win * win)
{
    ...
    MPIDIU_ACCESS_EPOCH_CHECK(win, MPIDIU_EPOTYPE_LOCK_ALL, mpi_errno, return mpi_errno);
    /* NOTE: lockall blocking waits till all locks granted */
    MPIR_Assert(MPIDIU_WIN(win, sync).lockall.allLocked == win->comm_ptr->local_size);
    ...
    if (MPIDIU_WIN(win, sync).assert_mode & MPI_MODE_NOCHECK) {
        MPIDIU_WIN(win, sync).lockall.allLocked = 0;
        goto no_check;
    }
    ...
    MPIDIU_PROGRESS_WHILE(MPIDIU_WIN(win, sync).lockall.allLocked);
  no_check:
    /* In performance-efficient mode, all allocated targets are freed at win_finalize. */
    if (MPIR_CVAR_CH4_RMA_MEM_EFFICIENT)
        MPIDIU_win_target_cleanall(win);
    MPIDIU_WIN(win, sync).access_epoch_type = MPIDIU_EPOTYPE_NONE;
    MPIDIU_WIN(win, sync).assert_mode = 0;
...
static inline int MPIDIG_mpi_win_lock_all(int assert, MPIR_Win * win)
{
    ...
    MPIR_Assert(MPIDIU_WIN(win, sync).lockall.allLocked == 0);

    int size;
    size = win->comm_ptr->local_size;
    if (assert & MPI_MODE_NOCHECK) {
        MPIDIU_WIN(win, sync).assert_mode |= MPI_MODE_NOCHECK;
        MPIDIU_WIN(win, sync).lockall.allLocked = size;
        goto no_check;
    }
    ...
    MPIDIU_PROGRESS_WHILE(size != (int) MPIDIU_WIN(win, sync).lockall.allLocked);
  no_check:
