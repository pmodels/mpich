...
enum {
    MPIDIU_EPOTYPE_NONE = 0,          /**< No epoch in affect */
    MPIDIU_EPOTYPE_LOCK = 1,          /**< MPI_Win_lock access epoch */
    MPIDIU_EPOTYPE_START = 2,         /**< MPI_Win_start access epoch */
    MPIDIU_EPOTYPE_POST = 3,          /**< MPI_Win_post exposure epoch */
    MPIDIU_EPOTYPE_FENCE = 4,         /**< MPI_Win_fence access/exposure epoch */
    MPIDIU_EPOTYPE_REFENCE = 5,       /**< MPI_Win_fence possible access/exposure epoch */
    MPIDIU_EPOTYPE_LOCK_ALL = 6       /**< MPI_Win_lock_all access epoch */
};
...
typedef struct MPIDIG_hdr {
    int src_rank;
    int tag;
    MPIR_Context_id_t context_id;
} MPIDIG_hdr_t;

typedef struct MPIDIG_send_long_req_msg {
    MPIDIG_hdr_t hdr;
...
typedef struct MPIDIG_send_long_ack_msg {
    uint64_t sreq_ptr;
    uint64_t rreq_ptr;
} MPIDIG_send_long_ack_msg_t;

typedef struct MPIDIG_send_long_lmt_msg {
    uint64_t rreq_ptr;
} MPIDIG_send_long_lmt_msg_t;

typedef struct MPIDIG_ssend_req_msg {
    MPIDIG_hdr_t hdr;
    uint64_t sreq_ptr;
} MPIDIG_ssend_req_msg_t;

typedef struct MPIDIG_ssend_ack_msg {
    uint64_t sreq_ptr;
} MPIDIG_ssend_ack_msg_t;

typedef struct MPIDIG_win_cntrl_msg {
    uint64_t win_id;
    uint32_t origin_rank;
    int16_t lock_type;
} MPIDIG_win_cntrl_msg_t;

typedef struct MPIDIG_put_msg {
    int src_rank;
    uint64_t win_id;
    uint64_t preq_ptr;
...
typedef struct MPIDIG_put_ack_msg {
    uint64_t preq_ptr;
} MPIDIG_put_ack_msg_t;

typedef struct MPIDIG_get_req_msg {
    int src_rank;
    uint64_t win_id;
    uint64_t greq_ptr;
    MPI_Aint target_disp;
...
typedef struct MPIDIG_get_ack_msg {
    uint64_t greq_ptr;
} MPIDIG_get_ack_msg_t;

typedef struct MPIDIG_cswap_req_msg {
    int src_rank;
    uint64_t win_id;
    uint64_t req_ptr;
    MPI_Aint target_disp;
...
typedef struct MPIDIG_cswap_ack_msg {
    uint64_t req_ptr;
} MPIDIG_cswap_ack_msg_t;

typedef struct MPIDIG_acc_req_msg {
    int src_rank;
    uint64_t win_id;
    uint64_t req_ptr;
    int origin_count;
...
typedef MPIDIG_acc_req_msg_t MPIDIG_get_acc_req_msg_t;

typedef struct MPIDIG_acc_ack_msg {
    uint64_t req_ptr;
} MPIDIG_acc_ack_msg_t;

typedef MPIDIG_acc_ack_msg_t MPIDIG_get_acc_ack_msg_t;

typedef struct MPIDIU_comm_req_list {
    MPIR_Comm *comm[2][4];
    MPIDIG_rreq_t *uelist[2][4];
} MPIDIU_comm_req_list_t;
...
typedef struct MPIDIU_map_entry {
    uint64_t key;
    void *value;
    UT_hash_handle hh;          /* makes this structure hashable */
} MPIDIU_map_entry_t;

typedef struct MPIDIU_map {
    MPIDIU_map_entry_t *head;
} MPIDIU_map_t;

typedef struct MPIDI_CH4_Global {
    ...
    MPIDIU_comm_req_list_t *comm_req_lists;
...
#endif
#define MPIDI_THREAD_PROGRESS_MUTEX  MPIDI_CH4_Global.m[0]
#define MPIDI_THREAD_PROGRESS_HOOK_MUTEX  MPIDI_CH4_Global.m[1]
#define MPIDI_THREAD_UTIL_MUTEX  MPIDI_CH4_Global.m[2]
