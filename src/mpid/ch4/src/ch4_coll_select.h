    ...
    if (MPIR_CVAR_ENABLE_SMP_COLLECTIVES && MPIR_CVAR_ENABLE_SMP_BCAST &&
        ...
        if ((nbytes < MPIR_CVAR_BCAST_SHORT_MSG_SIZE) ||
            ...
            if (nbytes < MPIR_CVAR_BCAST_LONG_MSG_SIZE && MPL_is_pof2(comm->local_size, NULL)) {
                return &MPIDI_Bcast_intra_local_then_nodes_cnt;
            }
        }
    }
                                                    ...
                                                    MPIR_Comm * comm, MPIR_Errflag_t * errflag)
{
    ...
    if (comm->comm_kind == MPIR_COMM_KIND__INTERCOMM) {
        return &MPIDI_Allreduce_inter_fallback_cnt;
    }
...
{
    if (comm->comm_kind == MPIR_COMM_KIND__INTERCOMM) {
        return &MPIDI_Gather_inter_fallback_cnt;
    }

    return &MPIDI_Gather_intra_netmod_cnt;
}
...
{
    if (comm->comm_kind == MPIR_COMM_KIND__INTERCOMM) {
        return &MPIDI_Gatherv_inter_fallback_cnt;
    }
...
{
    if (comm->comm_kind == MPIR_COMM_KIND__INTERCOMM) {
        return &MPIDI_Alltoall_inter_fallback_cnt;
    }

    return &MPIDI_Alltoall_intra_netmod_cnt;
}
...
{
    if (comm->comm_kind == MPIR_COMM_KIND__INTERCOMM) {
        return &MPIDI_Alltoallw_inter_fallback_cnt;
    }

    return &MPIDI_Alltoallw_intra_netmod_cnt;
}
...
{
    if (comm->comm_kind == MPIR_COMM_KIND__INTERCOMM) {
        return &MPIDI_Allgatherv_inter_fallback_cnt;
    }

    return &MPIDI_Allgatherv_intra_netmod_cnt;
}
                                                               ...
                                                               MPIR_Errflag_t * errflag)
{
    ...
    if (comm->comm_kind == MPIR_COMM_KIND__INTERCOMM) {
        return &MPIDI_Reduce_scatter_block_inter_fallback_cnt;
    }
