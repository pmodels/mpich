...
static int src_rmap_to_lut(MPIDI_rank_map_t * src, MPIDI_rank_map_t * dest,
                           MPIR_Comm_map_t * mapper, int total_mapper_size, int mapper_offset);
...
#undef FUNCNAME
#define FUNCNAME src_rmap_to_lut
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static int src_rmap_to_lut(MPIDI_rank_map_t * src, MPIDI_rank_map_t * dest,
                           MPIR_Comm_map_t * mapper, int total_mapper_size, int mapper_offset)
                               ...
                               MPIR_Comm_map_t * mapper, int offset)
{
    ...
    switch (src->mode) {
        ...
        case MPIDI_RANK_MAP_STRIDE_BLOCK:
        case MPIDI_RANK_MAP_STRIDE_BLOCK_INTRA:
            src_rmap_to_lut(src, dest, mapper, mapper->src_mapping_size, 0);
            break;
        case MPIDI_RANK_MAP_LUT:
        case MPIDI_RANK_MAP_LUT_INTRA:
            dest->mode = src->mode;
        ...
        switch (mode_detected) {
            ...
            default:
                if (src_rmap->mode == MPIDI_RANK_MAP_MLUT) {
                    ...
                    src_rmap_to_lut(src_rmap, dest_rmap, mapper, mapper->src_mapping_size,
                                    mapper_offset);
                }
        }
        goto fn_exit;
