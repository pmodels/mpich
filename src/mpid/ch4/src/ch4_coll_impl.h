                                                                   ...
                                                                   MPIR_Errflag_t errflag)
{
    ...
    if (comm->node_comm != NULL) {
        ...
        mpi_errno = MPIDI_SHM_mpi_barrier(comm->node_comm);
        MPIR_ERR_CHECK(mpi_errno);
#else
        mpi_errno = MPIDI_NM_mpi_barrier(comm->node_comm);
        MPIR_ERR_CHECK(mpi_errno);
    ...
    if (comm->node_roots_comm != NULL) {
        mpi_errno = MPIDI_NM_mpi_barrier(comm->node_roots_comm);
        MPIR_ERR_CHECK(mpi_errno);
    }
    ...
    if (comm->node_comm != NULL) {
        int i = 0;
#ifndef MPIDI_CH4_DIRECT_NETMOD
        mpi_errno = MPIDI_SHM_mpi_bcast(&i, 1, MPI_BYTE, 0, comm->node_comm);
        MPIR_ERR_CHECK(mpi_errno);
#else
        mpi_errno = MPIDI_NM_mpi_bcast(&i, 1, MPI_BYTE, 0, comm->node_comm);
        MPIR_ERR_CHECK(mpi_errno);
#endif /* MPIDI_CH4_DIRECT_NETMOD */
    }
                                                                  ...
                                                                  MPIR_Errflag_t errflag)
{
    ...
    mpi_errno = MPIDI_NM_mpi_barrier(comm);
    MPIR_ERR_CHECK(mpi_errno);
                                                                 ...
                                                                 MPIR_Errflag_t errflag)
{
    ...
    if (intra_root != -1 && intra_root != 0) {
        ...
        if (comm->rank == root) {
            mpi_errno = MPIC_Send(buffer, count, datatype, 0, MPIR_BCAST_TAG, comm->node_comm);
            MPIR_ERR_CHECK(mpi_errno);
        }
                                                                 ...
                                                                 MPIR_Errflag_t errflag)
{
    ...
    mpi_errno = MPIDI_NM_mpi_bcast(buffer, count, datatype, root, comm);
    MPIR_ERR_CHECK(mpi_errno);
                                                                     ...
                                                                     MPIR_Errflag_t errflag)
{
    ...
    if (comm->node_comm != NULL) {
        if ((sendbuf == MPI_IN_PLACE) && (comm->node_comm->rank != 0)) {
            ...
            MPIR_ERR_CHECK(mpi_errno);
#else
            mpi_errno = MPIDI_NM_mpi_reduce(recvbuf, NULL, count, datatype, op, 0, comm->node_comm);
            MPIR_ERR_CHECK(mpi_errno);
#endif /* MPIDI_CH4_DIRECT_NETMOD */
        } else {
#ifndef MPIDI_CH4_DIRECT_NETMOD
                                                                    ...
                                                                    MPIR_Errflag_t errflag)
{
    ...
    mpi_errno = MPIDI_NM_mpi_allreduce(sendbuf, recvbuf, count, datatype, op, comm);
    MPIR_ERR_CHECK(mpi_errno);
...
#ifndef MPIDI_CH4_DIRECT_NETMOD
    mpi_errno = MPIDI_SHM_mpi_allreduce(sendbuf, recvbuf, count, datatype, op, comm);
#else
    mpi_errno = MPIDI_NM_mpi_allreduce(sendbuf, recvbuf, count, datatype, op, comm);
#endif
    MPIR_ERR_CHECK(mpi_errno);
...
#ifndef MPIDI_CH4_DIRECT_NETMOD
        mpi_errno = MPIDI_SHM_mpi_barrier(comm_ptr->node_comm);
#else
        mpi_errno = MPIDI_NM_mpi_barrier(comm_ptr->node_comm);
#endif
        MPIR_ERR_CHECK(mpi_errno);
                                                                  ...
                                                                  MPIR_Errflag_t errflag)
{
    ...
    if (root != 0) {
        if (comm->rank == 0) {
            MPIC_Send(recvbuf, count, datatype, root, MPIR_REDUCE_TAG, comm);
...
#ifndef MPIDI_CH4_DIRECT_NETMOD
        mpi_errno = MPIDI_SHM_mpi_reduce(sendbuf, tmp_buf, count, datatype, op, 0, comm->node_comm);
#else
        ...
        if (comm->node_roots_comm->rank != MPIR_Get_internode_rank(comm, root)) {
            ...
            mpi_errno =
                MPIDI_NM_mpi_reduce(buf, NULL, count, datatype,
                                    op, MPIR_Get_internode_rank(comm, root), comm->node_roots_comm);
            MPIR_ERR_CHECK(mpi_errno);
                                                                  ...
                                                                  MPIR_Errflag_t errflag)
{
    ...
    mpi_errno = MPIDI_NM_mpi_reduce(sendbuf, recvbuf, count, datatype, op, root, comm);
    MPIR_ERR_CHECK(mpi_errno);
                                                                  ...
                                                                  MPIR_Errflag_t errflag)
{
    ...
    mpi_errno =
        MPIDI_NM_mpi_gather(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm);
    MPIR_ERR_CHECK(mpi_errno);
                                                                          ...
                                                                          MPIR_Errflag_t errflag)
{
    ...
    mpi_errno = MPIDI_NM_mpi_reduce_scatter(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr);
    MPIR_ERR_CHECK(mpi_errno);
                                                                                ...
                                                                                comm_ptr)
{
    ...
    mpi_errno =
        MPIDI_NM_mpi_reduce_scatter_block(sendbuf, recvbuf, recvcount, datatype, op, comm_ptr);
    MPIR_ERR_CHECK(mpi_errno);
                                                                ...
                                                                MPIR_Errflag_t errflag)
{
    ...
    if (comm_ptr->node_roots_comm != NULL && comm_ptr->node_comm != NULL) {
        ...
        mpi_errno = MPIC_Send(recvbuf, count, datatype, 0, MPIR_SCAN_TAG, comm_ptr->node_comm);
        MPIR_ERR_CHECK(mpi_errno);
    } else if (comm_ptr->node_roots_comm != NULL) {
    ...
    if (comm_ptr->node_roots_comm != NULL) {
        ...
        if (MPIR_Get_internode_rank(comm_ptr, rank) != comm_ptr->node_roots_comm->local_size - 1) {
            mpi_errno = MPIC_Send(prefulldata, count, datatype,
                                  ...
                                  MPIR_SCAN_TAG, comm_ptr->node_roots_comm);
            MPIR_ERR_CHECK(mpi_errno);
        }
                                                               ...
                                                               MPIR_Errflag_t errflag)
{
    ...
    mpi_errno = MPIDI_NM_mpi_scan(sendbuf, recvbuf, count, datatype, op, comm_ptr);
    MPIR_ERR_CHECK(mpi_errno);
                                                                  ...
                                                                  MPIR_Errflag_t errflag)
{
    ...
    mpi_errno = MPIDI_NM_mpi_exscan(sendbuf, recvbuf, count, datatype, op, comm_ptr);
    MPIR_ERR_CHECK(mpi_errno);
