                                                              ...
                                                              ch4_algo_parameters_container)
{
    ...
    void * bcast_container = MPIDI_coll_get_next_container(reduce_container);

#ifndef MPIDI_BUILD_CH4_SHM
    mpi_errno = MPIDI_NM_mpi_reduce(sendbuf, recvbuf, count, datatype,
                                    op, 0, comm, errflag, reduce_container);
    if (mpi_errno) {
    ...
    mpi_errno = MPIDI_NM_mpi_bcast(recvbuf, count, datatype, 0, comm, errflag, bcast_container);
    if (mpi_errno) {
        *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
        MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
        MPIR_ERR_ADD(mpi_errno_ret, mpi_errno);
...
#else
    if(comm->is_single_node){
      mpi_errno = MPIDI_SHM_mpi_reduce(sendbuf, recvbuf, count, datatype,
                                       op, 0, comm, errflag, reduce_container);
      if (mpi_errno) {
          *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
          MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
      ...
      mpi_errno = MPIDI_SHM_mpi_bcast(recvbuf, count, datatype, 0, comm, errflag, bcast_container);
      if (mpi_errno) {
          *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
          MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
          MPIR_ERR_ADD(mpi_errno_ret, mpi_errno);
