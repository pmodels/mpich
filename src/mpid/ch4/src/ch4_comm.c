 ...
 */
static int leader_exchange(MPIR_Comm * local_comm, MPIDI_av_entry_t * dynamic_av, int tag,
...
int MPID_Intercomm_exchange(MPIR_Comm * local_comm, int local_leader,
                            ...
                            int *remote_size, MPIR_Lpid ** remote_lpids_out)
{
    MPIR_Lpid remote_lpid = 0;
    if (local_comm->rank == local_leader) {
        remote_lpid = MPIDIU_comm_rank_to_lpid(peer_comm, remote_leader);
    }
    return MPIDI_Intercomm_exchange(local_comm, local_leader, remote_lpid, tag, context_id,
                                    remote_context_id, remote_size, remote_lpids_out);
}
                             ...
                             int *remote_size, MPIR_Lpid ** remote_lpids_out)
{
    ...
    void *remote_data;
    if (is_local_leader) {
        /* Stage 1: UPID exchange and LPID conversion in leaders */
        mpi_errno = leader_exchange(local_comm, remote_lpid, tag, context_id,
                                    &remote_data_size, &remote_data);
    }
  ...
  fn_bcast:
    ...
    mpi_errno = MPIR_Bcast_impl(&bcast_data, 2, MPI_INT, local_leader, local_comm, MPIR_ERR_NONE);
    MPIR_ERR_CHECK(mpi_errno);
    ...
    if (is_local_leader) {
        ...
        MPIR_ERR_CHKANDJUMP(bcast_data.mpi_errno, mpi_errno, MPI_ERR_OTHER, "**spawn");
    ...
    mpi_errno = MPIR_Bcast_impl(remote_data, remote_data_size, MPI_BYTE, local_leader, local_comm,
                                MPIR_ERR_NONE);
    MPIR_ERR_CHECK(mpi_errno);
    ...
    char *remote_upids;
    mpi_errno = extract_remote_data(remote_data, remote_size, remote_context_id, &remote_lpids,
                                    &remote_upid_sizes, &remote_upids);
    MPIR_ERR_CHECK(mpi_errno);

    /* insert upids */
    char *upid = remote_upids;
    for (int i = 0; i < *remote_size; i++) {
        mpi_errno = MPIDI_NM_av_insert(remote_lpids[i], upid, remote_upid_sizes[i]);
        MPIR_ERR_CHECK(mpi_errno);
    }

    /* make a copy of remote_lpids */
    *remote_lpids_out = MPL_malloc((*remote_size) * sizeof(MPIR_Lpid), MPL_MEM_GROUP);
    MPIR_ERR_CHKANDJUMP(!(*remote_lpids_out), mpi_errno, MPI_ERR_OTHER, "**nomem");
    memcpy(*remote_lpids_out, remote_lpids, (*remote_size) * sizeof(MPIR_Lpid));
...
#define MAX_WORLDS 100
    static int world_hash[MAX_WORLDS];
    int num_worlds;
                               ...
                               int **remote_upid_sizes_out, char **remote_upids_out)
{
    ...
    int remote_size = *(int *) s;
    s += sizeof(int);
    int remote_context_id = *(int *) s;
    s += sizeof(int);
...
int leader_exchange(MPIR_Comm * local_comm, MPIDI_av_entry_t * dynamic_av, int tag, int context_id,
                    int *remote_data_size_out, void **remote_data_out)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_FUNC_ENTER;
    MPID_THREAD_CS_ENTER(VCI, MPIDI_VCI(0).lock);

    /* I am the leader of local_comm, dynamic_av is the remote leader of remote_comm.
     *
     * 1. Send data sizes
     * 2. Send data
     *
    ...
    int remote_data_size;
    void *remote_data;
    mpi_errno = MPIDI_NM_dynamic_sendrecv(dynamic_av, tag, &local_data_size, sizeof(int),
                                          &remote_data_size, sizeof(int));
    MPIR_ERR_CHECK(mpi_errno);

    remote_data = MPL_malloc(remote_data_size, MPL_MEM_OTHER);
    MPIR_ERR_CHKANDJUMP(!remote_data, mpi_errno, MPI_ERR_OTHER, "**nomem");

    mpi_errno = MPIDI_NM_dynamic_sendrecv(dynamic_av, tag, &local_data, local_data_size,
                                          remote_data, remote_data_size);
    MPIR_ERR_CHECK(mpi_errno);

    MPL_free(local_lpids);
    MPL_free(local_upid_sizes);
