    ...
    if (local_comm->rank == local_leader) {
        ...
        *remote_context_id = remote_ints[1];
        ...
        MPIR_CHKPMEM_MALLOC((*remote_lpids), MPIR_Lpid *, (*remote_size) * sizeof(MPIR_Lpid),
                            mpi_errno, "remote_lpids", MPL_MEM_ADDRESS);
        MPIR_CHKLMEM_MALLOC(local_lpids, MPIR_Lpid *, local_size * sizeof(MPIR_Lpid),
                            mpi_errno, "local_lpids", MPL_MEM_ADDRESS);
        for (i = 0; i < local_size; i++) {
