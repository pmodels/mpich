                             ...
                             MPIR_Comm ** intercomm, int errcodes[])
{
    ...
    mpi_errno =
        MPIR_Bcast(bcast_ints, 2, MPI_INT, root, comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
    ...
    if (errcodes != MPI_ERRCODES_IGNORE) {
        mpi_errno =
            MPIR_Bcast(pmi_errcodes, total_num_processes, MPI_INT, root, comm_ptr,
                                    ...
                                    MPIR_Comm ** newcomm)
{
    ...
    if (comm_ptr->rank == root) {
      ...
      bcast_tag_and_errno:
        ...
        mpi_errno =
            MPIR_Bcast_allcomm_auto(bcast_ints, 2, MPI_INT, root, comm_ptr, MPIR_SUBGROUP_NONE,
                                    MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
