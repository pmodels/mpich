                             ...
                             MPIR_Comm ** intercomm, int errcodes[])
{
    ...
    mpi_errno = MPIR_Bcast(bcast_ints, 2, MPIR_INT_INTERNAL, root, comm_ptr, MPIR_COLL_ATTR_INIT);
    ...
    if (errcodes != MPI_ERRCODES_IGNORE) {
        mpi_errno = MPIR_Bcast(pmi_errcodes, total_num_processes, MPIR_INT_INTERNAL,
                               root, comm_ptr, MPIR_COLL_ATTR_INIT);
        MPIR_ERR_CHECK(mpi_errno);
                                    ...
                                    MPIR_Comm ** newcomm)
{
    ...
    if (comm_ptr->rank == root) {
      ...
      fn_fail:
        ...
        MPIR_Bcast_impl(&mpi_errno, 1, MPIR_INT_INTERNAL, root, comm_ptr, MPIR_COLL_ATTR_INIT);
    } else {
        int root_errno;
        MPIR_Bcast_impl(&root_errno, 1, MPIR_INT_INTERNAL, root, comm_ptr, MPIR_COLL_ATTR_INIT);
        if (root_errno) {
