                                    ...
                                    MPIR_Comm ** newcomm)
{
    ...
    if (comm_ptr->rank == root) {
        ...
        MPIR_Group_create_stride(1, 0, NULL, remote_lpid, 1, 1, &newcomm->remote_group);

      fn_fail:
        /* In case root fails, we bcast mpi_errno so other ranks will abort too */
        MPIR_Bcast_impl(&mpi_errno, 1, MPI_INT, root, comm_ptr, MPIR_ERR_NONE);
    } else {
        int root_errno;
        MPIR_Bcast_impl(&root_errno, 1, MPI_INT, root, comm_ptr, MPIR_ERR_NONE);
        if (root_errno) {
    ...
    if (mpi_errno == MPI_SUCCESS) {
        mpi_errno = MPIR_Intercomm_create_impl(comm_ptr, root, peer_comm, 0, tag, newcomm);
    }
