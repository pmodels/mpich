                    ...
                    MPIR_Comm * comm, MPIR_Info * info, MPIR_Request ** request)
{
    ...
    mpi_errno =
        MPIDIG_mpi_psend_init(buf, partitions, count, datatype, dest, tag, comm, info,
                              MPIDI_av_is_local(av), request);
                    ...
                    MPIR_Comm * comm, MPIR_Info * info, MPIR_Request ** request)
{
    ...
    mpi_errno =
        MPIDIG_mpi_precv_init(buf, partitions, count, datatype, dest, tag, comm, info,
                              MPIDI_av_is_local(av), request);
    MPIR_ERR_CHECK(mpi_errno);
