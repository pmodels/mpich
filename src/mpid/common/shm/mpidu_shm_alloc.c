        ...
        if (shm_comm_ptr) {
            if (shm_comm_ptr->rank == 0) {
              ...
              root_sync:
                ...
                mpi_errno =
                    MPIR_Bcast(map_result_ptr, 1, MPI_INT, 0, shm_comm_ptr, MPIR_SUBGROUP_NONE,
                               MPIR_ERR_NONE);
                MPIR_ERR_CHECK(mpi_errno);
                ...
                mpi_errno = MPIR_Bcast(serialized_hnd, MPL_SHM_GHND_SZ, MPI_BYTE, 0,
                                       shm_comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
                MPIR_ERR_CHECK(mpi_errno);
          ...
          result_sync:
            ...
            mpi_errno = MPIR_Allreduce(map_result_ptr, &all_map_result, 1, MPI_INT,
                                       MPI_MAX, shm_comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
            MPIR_ERR_CHECK(mpi_errno);
...
static int shm_alloc_symm_all(MPIR_Comm * comm_ptr, size_t offset, MPIDU_shm_seg_t * shm_seg)
{
    ...
    while (all_map_result == SYMSHM_MAP_FAIL && iter-- > 0) {
        ...
        mpi_errno =
            MPIR_Bcast(&map_pointer, sizeof(char *), MPI_CHAR, maxsz_loc, comm_ptr,
                       MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
...
static int shm_alloc(MPIR_Comm * shm_comm_ptr, MPIDU_shm_seg_t * shm_seg)
{
    ...
    if (shm_comm_ptr->rank == 0) {
      ...
      hnd_sync:
        ...
        mpi_errno =
            MPIR_Bcast_impl(serialized_hnd, MPL_SHM_GHND_SZ, MPI_BYTE, 0, shm_comm_ptr,
                            MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
        ...
        mpi_errno = MPIR_Allreduce_impl(&shm_fail_flag, &any_shm_fail_flag, 1, MPI_C_BOOL,
                                        MPI_LOR, shm_comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
