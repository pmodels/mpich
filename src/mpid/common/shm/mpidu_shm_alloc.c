        ...
        if (shm_comm_ptr) {
          ...
          result_sync:
            ...
            mpi_errno = MPIR_Allreduce(map_result_ptr, &all_map_result, 1, MPIR_INT_INTERNAL,
                                       MPI_MAX, shm_comm_ptr, MPIR_COLL_ATTR_INIT);
            MPIR_ERR_CHECK(mpi_errno);
...
static int shm_alloc_symm_all(MPIR_Comm * comm_ptr, size_t offset, MPIDU_shm_seg_t * shm_seg)
{
    ...
    while (all_map_result == SYMSHM_MAP_FAIL && iter-- > 0) {
        ...
        mpi_errno = MPIR_Bcast(&map_pointer, sizeof(char *), MPIR_CHAR_INTERNAL,
                               maxsz_loc, comm_ptr, MPIR_COLL_ATTR_INIT);
        MPIR_ERR_CHECK(mpi_errno);
        ...
        mpi_errno = MPIR_Allreduce(&map_result, &all_map_result, 1, MPIR_INT_INTERNAL,
                                   MPI_MAX, comm_ptr, MPIR_COLL_ATTR_INIT);
        MPIR_ERR_CHECK(mpi_errno);
    ...
    if (shm_comm_ptr->rank == 0) {
      ...
      hnd_sync:
        ...
        mpi_errno = MPIR_Allreduce_impl(&shm_fail_flag, &any_shm_fail_flag, 1, MPIR_C_BOOL_INTERNAL,
                                        MPI_LOR, shm_comm_ptr, MPIR_COLL_ATTR_INIT);
        MPIR_ERR_CHECK(mpi_errno);
