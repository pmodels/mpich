...
enum {
    MPIDU_SYMSHM_SUCCESS,
    MPIDU_SYMSHM_MAP_FAIL,      /* mapping failure when specified with a fixed start addr */
    MPIDU_SYMSHM_OTHER_FAIL     /* other failure reported by MPL shm */
};
...
static void *generate_random_addr(size_t size)
{
     ...
     */
#define MPIDU_MAP_POINTER ((random_unsigned&((0x00006FFFFFFFFFFF&(~(page_sz-1)))|0x0000600000000000)))
...
/* try mapping shmem into processes address space symmetrically using address passed in
 * through shm_seg->base_addr. Processes in different nodes coordinate after the function
 * returns. */
static int map_symm_shm(MPIR_Comm * shm_comm_ptr, MPIDU_shm_seg_t * shm_seg, int *map_result)
{
        ...
        if (shm_comm_ptr) {
            if (shm_comm_ptr->rank == 0) {
                ...
                if (mpl_err) {
                    *map_result =
                        (mpl_err ==
                         MPL_SHM_EINVAL) ? MPIDU_SYMSHM_MAP_FAIL : MPIDU_SYMSHM_OTHER_FAIL;
                    goto root_sync;
                }

                mpl_err = MPL_shm_hnd_get_serialized_by_ref(shm_seg->hnd, &serialized_hnd);
                if (mpl_err) {
                    *map_result = MPIDU_SYMSHM_OTHER_FAIL;
                }
              ...
              root_sync:
                ...
                if (*map_result != MPIDU_SYMSHM_SUCCESS)
                    goto fn_exit;
            ...
            *map_result = (shm_seg->base_addr == (void *) MAP_FAILED) ?
                MPIDU_SYMSHM_MAP_FAIL : MPIDU_SYMSHM_SUCCESS;
        }
    }
                              ...
                              bool * fail_flag)
{
    ...
    int all_map_result = MPIDU_SYMSHM_MAP_FAIL;
    ...
    while (all_map_result == MPIDU_SYMSHM_MAP_FAIL && iter-- > 0) {
        int map_result = MPIDU_SYMSHM_SUCCESS;

        /* rank 0 in comm generates random address */
        ...
        if (all_map_result != MPIDU_SYMSHM_SUCCESS && map_result == MPIDU_SYMSHM_SUCCESS &&
    ...
    if (all_map_result != MPIDU_SYMSHM_SUCCESS) {
