...
#if 0
/*
 * <mpi_errno> mpig_comm_dup_hook[IN] orig_comm, [IN/MOD] new_comm)
 *
...
#   if defined(MPIG_VMPI)
	...
	mpi_errno = mpig_cm_vmpi_comm_dup_hook(orig_comm, new_comm);
    }
#   endif /* #if defined(MPIG_VMPI) */
...
/*
 * <mpi_errno> mpig_comm_split_hook[IN] orig_comm, [IN/MOD] new_comm)
 *
 * orig_comm [IN] - communicator being duplicated
 * new_comm [IN/MOD] - communicator resulting from the duplication
 * mpi_errno [OUT] - MPI error code
 */
#undef FUNCNAME
#define FUNCNAME mpig_comm_split_hook
int  mpig_comm_split_hook(MPID_Comm * orig_comm, MPID_Comm * new_comm)
{
    const char fcname[] = MPIG_QUOTE(FUNCNAME);
    int mpi_errno = MPI_SUCCESS;
    MPIG_STATE_DECL(MPID_STATE_mpig_comm_split_hook);

    MPIG_UNUSED_VAR(fcname);
    
    MPIG_FUNC_ENTER(MPID_STATE_mpig_comm_split_hook);
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM,
	"entering: orig_comm=" MPIG_HANDLE_FMT ", orig_commp=" MPIG_PTR_FMT ", new_comm=" MPIG_HANDLE_FMT
	", new_commp=" MPIG_PTR_FMT, orig_comm->handle, MPIG_PTR_CAST(orig_comm), new_comm->handle, MPIG_PTR_CAST(new_comm)));
...
#   if defined(MPIG_VMPI)
    {
	/* call the VMPI CM's hook, allowing it to duplication the associated vendor communicators */
	mpi_errno = mpig_cm_vmpi_comm_split_hook(orig_comm, new_comm);
    }
#   endif /* #if defined(MPIG_VMPI) */

    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM, "exiting: mpi_errno=" MPIG_ERRNO_FMT, mpi_errno));
    MPIG_FUNC_EXIT(MPID_STATE_mpig_comm_split_hook);
    return mpi_errno;
}
/* mpig_comm_split_hook() */

/*
 * <mpi_errno> mpig_intercomm_create_hook[IN] orig_comm, [IN/MOD] new_comm)
 *
 * orig_comm [IN] - communicator being duplicated
 * new_comm [IN/MOD] - communicator resulting from the duplication
 * mpi_errno [OUT] - MPI error code
 */
#undef FUNCNAME
#define FUNCNAME mpig_intercomm_create_hook
int  mpig_intercomm_create_hook(MPID_Comm * orig_comm, MPID_Comm * new_comm)
{
    const char fcname[] = MPIG_QUOTE(FUNCNAME);
    int mpi_errno = MPI_SUCCESS;
    MPIG_STATE_DECL(MPID_STATE_mpig_intercomm_create_hook);

    MPIG_UNUSED_VAR(fcname);
    
    MPIG_FUNC_ENTER(MPID_STATE_mpig_intercomm_create_hook);
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM,
	"entering: orig_comm=" MPIG_HANDLE_FMT ", orig_commp=" MPIG_PTR_FMT ", new_comm=" MPIG_HANDLE_FMT
	", new_commp=" MPIG_PTR_FMT, orig_comm->handle, MPIG_PTR_CAST(orig_comm), new_comm->handle, MPIG_PTR_CAST(new_comm)));
...
#   if defined(MPIG_VMPI)
    {
	/* call the VMPI CM's hook, allowing it to duplication the associated vendor communicators */
	mpi_errno = mpig_cm_vmpi_intercomm_create_hook(orig_comm, new_comm);
    }
#   endif /* #if defined(MPIG_VMPI) */

    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM, "exiting: mpi_errno=" MPIG_ERRNO_FMT, mpi_errno));
    MPIG_FUNC_EXIT(MPID_STATE_mpig_intercomm_create_hook);
    return mpi_errno;
}
/* mpig_intercomm_create_hook() */

/*
 * <mpi_errno> mpig_intercomm_merge_hook[IN] orig_comm, [IN/MOD] new_comm)
 *
 * orig_comm [IN] - communicator being duplicated
 * new_comm [IN/MOD] - communicator resulting from the duplication
 * mpi_errno [OUT] - MPI error code
 */
#undef FUNCNAME
#define FUNCNAME mpig_intercomm_merge_hook
int  mpig_intercomm_merge_hook(MPID_Comm * orig_comm, MPID_Comm * new_comm)
{
    const char fcname[] = MPIG_QUOTE(FUNCNAME);
    int mpi_errno = MPI_SUCCESS;
    MPIG_STATE_DECL(MPID_STATE_mpig_intercomm_merge_hook);

    MPIG_UNUSED_VAR(fcname);
    
    MPIG_FUNC_ENTER(MPID_STATE_mpig_intercomm_merge_hook);
    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM,
	"entering: orig_comm=" MPIG_HANDLE_FMT ", orig_commp=" MPIG_PTR_FMT ", new_comm=" MPIG_HANDLE_FMT
	", new_commp=" MPIG_PTR_FMT, orig_comm->handle, MPIG_PTR_CAST(orig_comm), new_comm->handle, MPIG_PTR_CAST(new_comm)));
...
#   if defined(MPIG_VMPI)
    {
	/* call the VMPI CM's hook, allowing it to duplication the associated vendor communicators */
	mpi_errno = mpig_cm_vmpi_intercomm_merge_hook(orig_comm, new_comm);
    }
#   endif /* #if defined(MPIG_VMPI) */

    MPIG_DEBUG_PRINTF((MPIG_DEBUG_LEVEL_FUNC | MPIG_DEBUG_LEVEL_COMM, "exiting: mpi_errno=" MPIG_ERRNO_FMT, mpi_errno));
    MPIG_FUNC_EXIT(MPID_STATE_mpig_intercomm_merge_hook);
    return mpi_errno;
}
/* mpig_intercomm_merge_hook() */
#endif
