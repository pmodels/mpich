...
int ib_post_write(MPIDI_VC *vc_ptr, MM_Car *car_ptr)
{
    ...
    MPIDI_STATE_DECL(MPID_STATE_IB_POST_WRITE);

    MPIDI_FUNC_ENTER(MPID_STATE_IB_POST_WRITE);

    MPIU_DBG_PRINTF(("ib_post_write\n"));

#ifdef MPICH_DEV_BUILD
    if (!(car_ptr->type & MM_HEAD_CAR))
    {
	err_printf("ib_post_write: only head cars can be posted.\n");
	...
	(car_ptr->msg_header.pkt.u.hdr.size < IB_EAGER_LIMIT))
    {
	...
	ib_setup_packet_car(vc_ptr, MM_WRITE_CAR, car_ptr->dest, rndv_car_ptr);
	...
	if ((unsigned long)car_ptr < 1000)
	    msg_printf("Error: ib_post_write setting invalid send_car_ptr: %u\n", car_ptr);
