...
int ib_merge_with_unexpected(MM_Car *posted_car_ptr, MM_Car *unex_car_ptr)
{
    int num_left, num_updated;
    char *unex_data_ptr;
    MPIDI_STATE_DECL(MPID_STATE_IB_MERGE_WITH_UNEXPECTED);

    MPIDI_FUNC_ENTER(MPID_STATE_IB_MERGE_WITH_UNEXPECTED);
    ...
    MPIDI_FUNC_EXIT(MPID_STATE_IB_MERGE_WITH_UNEXPECTED);
    return MPI_SUCCESS;
}
