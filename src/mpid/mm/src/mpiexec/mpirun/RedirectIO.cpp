...
/*  $Id: RedirectIO.cpp,v 1.4 2003/02/27 01:13:56 David Exp $
 *
...
void RedirectIOThread2(SOCKET abort_sock)
{
	...
	if (n == SOCKET_ERROR)
	{
	    printf("RedirectIOThread2: select failed, error %d\n", WSAGetLastError());fflush(stdout);
	...
	if (n == 0)
	{
	    printf("RedirectIOThread2: select returned zero sockets available\n");fflush(stdout);
	    easy_closesocket(abort_sock);
	    for (i=0; i<nActive; i++)
		easy_closesocket(sockActive[i]);
	    if (sockListen != INVALID_SOCKET)
...
void RedirectIOThread(HANDLE hReadyEvent)
{
	...
	if (n == SOCKET_ERROR)
	{
	    printf("RedirectIOThread: select failed, error %d\n", WSAGetLastError());fflush(stdout);
	    break;
	}
	if (n == 0)
	{
	    printf("RedirectIOThread: select returned zero sockets available\n");fflush(stdout);
	    break;
	}
	else
	{
