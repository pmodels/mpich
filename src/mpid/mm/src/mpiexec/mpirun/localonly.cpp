...
/*  $Id: localonly.cpp,v 1.2 2002/09/27 21:11:13 toonen Exp $
 *
...
{
    DWORD size = 100;
    char pszHost[MAX_HOST_LENGTH], pszCmdLine[MAX_CMD_LENGTH], error_msg[256], pszEnv[MAX_CMD_LENGTH];
    char *pEnv;
    int rootPort=0;
	...
	if (bDoSMP)
	{
	    sprintf(pszEnv, "PMI_KVS=%s|PMI_RANK=%d|PMI_SIZE=%d|PMI_MPD=%s:%d|PMI_SHM_LOW=0|PMI_SHM_HIGH=%d",
	...
	if (strlen(pszEnv) + strlen(g_pszEnv) + 1 >= MAX_CMD_LENGTH)
	{
	    printf("Warning: environment variables truncated.\n");
	}
	strncat(pszEnv, "|", MAX_CMD_LENGTH - 1 - strlen(pszEnv));
	strncat(pszEnv, g_pszEnv, MAX_CMD_LENGTH - 1 - strlen(pszEnv));
