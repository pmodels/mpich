...
int socket_stuff_vector_via(MPID_IOV *, int *, MM_Car *, MM_Segment_buffer *);
#endif
#ifdef WITH_METHOD_VIA_RDMA
int socket_stuff_vector_via_rdma(MPID_IOV *, int *, MM_Car *, MM_Segment_buffer *);
#endif
#ifdef WITH_METHOD_IB
int socket_stuff_vector_ib(MPID_IOV *, int *, MM_Car *, MM_Segment_buffer *);
#endif
#ifdef WITH_METHOD_NEW
int socket_stuff_vector_new(MPID_IOV *, int *, MM_Car *, MM_Segment_buffer *);
#endif
int socket_stuff_vector_vec(MPID_IOV *, int *, MM_Car *, MM_Segment_buffer *);
int socket_stuff_vector_tmp(MPID_IOV *, int *, MM_Car *, MM_Segment_buffer *);
int socket_stuff_vector_simple(MPID_IOV *, int *, MM_Car *, MM_Segment_buffer *);
int socket_update_car_num_written(MM_Car *, int *);
...
#ifdef WITH_METHOD_SHM
int socket_stuff_vector_shm(MPID_IOV *vec, int *cur_pos, MM_Car *car_ptr, MM_Segment_buffer *buf_ptr)
{
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_STUFF_VECTOR_SHM);
    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_STUFF_VECTOR_SHM);
    MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_STUFF_VECTOR_SHM);
    return FALSE;
}
#endif

#ifdef WITH_METHOD_VIA
int socket_stuff_vector_via(MPID_IOV *vec, int *cur_pos, MM_Car *car_ptr, MM_Segment_buffer *buf_ptr)
{
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_STUFF_VECTOR_VIA);
    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_STUFF_VECTOR_VIA);
    MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_STUFF_VECTOR_VIA);
    return FALSE;
}
#endif

#ifdef WITH_METHOD_VIA_RDMA
int socket_stuff_vector_via_rdma(MPID_IOV *vec, int *cur_pos, MM_Car *car_ptr, MM_Segment_buffer *buf_ptr)
{
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_STUFF_VECTOR_VIA_RDMA);
    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_STUFF_VECTOR_VIA_RDMA);
    MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_STUFF_VECTOR_VIA_RDMA);
    return FALSE;
}
#endif

int socket_stuff_vector_vec(MPID_IOV *vec, int *cur_pos_ptr, MM_Car *car_ptr, MM_Segment_buffer *buf_ptr)
{
    int cur_pos, cur_index, num_avail, final_segment;
    MPID_IOV *car_vec, *buf_vec;
    int num_left, i;
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_STUFF_VECTOR_VEC);

    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_STUFF_VECTOR_VEC);
	...
	MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_STUFF_VECTOR_VEC);
	return FALSE;
    }
	...
	*/
    ...
    num_avail = buf_ptr->vec.num_read - car_ptr->data.socket.buf.vec_write.cur_num_written;
    ...
    final_segment = (num_avail + car_ptr->data.socket.buf.vec_write.total_num_written) == buf_ptr->vec.segment_last;

    /*msg_printf("socket_stuff_vector_vec: num_avail = %d\n", num_avail);*/
...
int socket_stuff_vector_tmp(MPID_IOV *vec, int *cur_pos, MM_Car *car_ptr, MM_Segment_buffer *buf_ptr)
{
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_STUFF_VECTOR_TMP);
    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_STUFF_VECTOR_TMP);

    /* check to see that there is data available and space in the vector to put it */
    if ((*cur_pos == MPID_IOV_LIMIT) ||
        (car_ptr->data.socket.buf.tmp.num_written == buf_ptr->tmp.num_read) ||
	...
	MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_STUFF_VECTOR_TMP);
	return FALSE;
    }

    /*msg_printf("socket_stuff_vector_tmp: added %d byte buffer\n", buf_ptr->tmp.num_read - car_ptr->data.socket.buf.tmp.num_written);*/

    /* add the tmp buffer to the vector */
    vec[*cur_pos].MPID_IOV_BUF = (char*)(buf_ptr->tmp.buf) + car_ptr->data.socket.buf.tmp.num_written;
    vec[*cur_pos].MPID_IOV_LEN = buf_ptr->tmp.num_read - car_ptr->data.socket.buf.tmp.num_written;
    (*cur_pos)++;
...
int socket_stuff_vector_simple(MPID_IOV *vec, int *cur_pos, MM_Car *car_ptr, MM_Segment_buffer *buf_ptr)
{
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_STUFF_VECTOR_SIMPLE);
    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_STUFF_VECTOR_SIMPLE);

    /* check to see that there is data available and space in the vector to put it */
    if ((*cur_pos == MPID_IOV_LIMIT) ||
        (car_ptr->data.socket.buf.simple.num_written == buf_ptr->simple.num_read) ||
	(buf_ptr->simple.num_read == 0))
    {
	/* cur_pos has run off the end of the vector OR
	   If num_written is equal to num_read then there is no data available to write. OR
	   No data has been read */
	MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_STUFF_VECTOR_SIMPLE);
	return FALSE;
    }

    /*msg_printf("socket_stuff_vector_simple: added %d byte buffer\n", buf_ptr->simple.num_read - car_ptr->data.socket.buf.simple.num_written);*/

    /* add the simple buffer to the vector */
    vec[*cur_pos].MPID_IOV_BUF = (char*)(buf_ptr->simple.buf) + car_ptr->data.socket.buf.simple.num_written;
    vec[*cur_pos].MPID_IOV_LEN = buf_ptr->simple.num_read - car_ptr->data.socket.buf.simple.num_written;
    (*cur_pos)++;
...
{
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_STUFF_VECTOR_NEW);
    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_STUFF_VECTOR_NEW);
    MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_STUFF_VECTOR_NEW);
    return FALSE;
}
#endif

int socket_update_car_num_written(MM_Car *car_ptr, int *num_written_ptr)
{
    MM_Segment_buffer *buf_ptr;
    int num_written;
    int num_left, i;
    MPIDI_STATE_DECL(MPID_STATE_SOCKET_UPDATE_CAR_NUM_WRITTEN);

    MPIDI_FUNC_ENTER(MPID_STATE_SOCKET_UPDATE_CAR_NUM_WRITTEN);

#ifdef MPICH_DEV_BUILD
    if (car_ptr == NULL)
    {
	err_printf("Error: socket_update_car_num_written called on a NULL car pointer, num_written = %d\n", *num_written_ptr);
    }
#endif
	...
	num_written = min(
	    ...
	    buf_ptr->vec.segment_last - car_ptr->data.socket.buf.vec_write.total_num_written,
	...
	else
	{
		...
		else
		{
			...
			(char*)(car_ptr->data.socket.buf.vec_write.vec[i].MPID_IOV_BUF) +
	...
	if (car_ptr->data.socket.buf.vec_write.total_num_written == buf_ptr->vec.segment_last)
	{
	    ...
	    {
		err_printf("Error: socket_update_car_num_written not dequeueing the head write car.\n");
	    }
#endif
	...
	{
	    /*msg_printf("partial buffer written, total_num_written %d, segment_last %d\n", car_ptr->data.socket.buf.vec_write.total_num_written, buf_ptr->vec.segment_last);*/
	}
	break;
    case MM_TMP_BUFFER:
	num_written = min(
	    /* the amout of buffer space available to have been written */
	    buf_ptr->tmp.len - car_ptr->data.socket.buf.tmp.num_written,
	    /* the actual amount written */
	    num_written);
	...
	num_written = min(
	    /* the amout of buffer space available to have been written */
	    buf_ptr->simple.len - car_ptr->data.socket.buf.simple.num_written,
	    /* the actual amount written */
	    num_written);
...
#endif
    case MM_NULL_BUFFER:
	err_printf("Error: socket_update_car_num_written called on a null buffer\n");
	break;
    default:
	err_printf("Error: socket_update_car_num_written: unknown or unsupported buffer type: %d\n", buf_ptr->type);
	break;
    }
    ...
    MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_UPDATE_CAR_NUM_WRITTEN);
    return MPI_SUCCESS;
}

/*@
   socket_write_aggressive - write as many cars as possible
...
#ifdef WITH_METHOD_SHM
	case MM_SHM_BUFFER:
	    stop = !socket_stuff_vector_shm(vec, &cur_pos, car_ptr, buf_ptr);
	    break;
#endif
#ifdef WITH_METHOD_VIA
	case MM_VIA_BUFFER:
	    stop = !socket_stuff_vector_via(vec, &cur_pos, car_ptr, buf_ptr);
	    break;
#endif
#ifdef WITH_METHOD_VIA_RDMA
	case MM_VIA_RDMA_BUFFER:
	    stop = !socket_stuff_vector_via_rdma(vec, &cur_pos, car_ptr, buf_ptr);
	    break;
#endif
	case MM_VEC_BUFFER:
		...
		stop = !socket_stuff_vector_vec(vec, &cur_pos, car_ptr, buf_ptr);
	    }
	    else
	    {
	    ...
	    break;
	case MM_TMP_BUFFER:
	    stop = !socket_stuff_vector_tmp(vec, &cur_pos, car_ptr, buf_ptr);
	    break;
	case MM_SIMPLE_BUFFER:
...
#endif
	case MM_NULL_BUFFER:
	    err_printf("Error: socket_write_aggressive called on a null buffer\n");
	    break;
	default:
	    err_printf("Error: socket_write_aggressive: unknown or unsupported buffer type: %d\n", buf_ptr->type);
	    break;
	}
	if (stop)
	    break;
...
int socket_handle_written(MPIDI_VC *vc_ptr, int num_written)
{
    ...
    /* update all the cars and buffers affected by the sock_write action */
    while (num_written)
    {
        /* socket_update_car_num_written causes the head to get dequeued and the next car to take its place 
	 * until all the data is accounted for or there is an error. */
	if (socket_update_car_num_written(vc_ptr->writeq_head, &num_written) != MPI_SUCCESS)
	{
	    err_printf("socket_write_aggressive:socket_update_car_num_written failed.\n");
	    MPIDI_FUNC_EXIT(MPID_STATE_SOCKET_HANDLE_WRITTEN);
	    return -1;
	}
    ...
    /* if there are more cars in the queue, post the next sock_write action */
    if (vc_ptr->writeq_head != NULL)
