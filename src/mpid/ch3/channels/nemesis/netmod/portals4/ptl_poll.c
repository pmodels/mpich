...
static int append_overflow(int i)
{
    ...
    ret =
        PtlMEAppend(MPIDI_nem_ptl_ni, MPIDI_nem_ptl_pt, &me, PTL_OVERFLOW_LIST, (void *) (size_t) i,
...
int MPID_nem_ptl_poll(int is_blocking_poll)
{
    ...
    while (1) {
        ...
        if (ret == PTL_EQ_EMPTY) {
            ...
            if (ret == PTL_EQ_EMPTY) {
                ...
                if (ret == PTL_EQ_EMPTY) {
                    ...
                    MPIR_ERR_CHKANDJUMP(ret == PTL_EQ_DROPPED, mpi_errno, MPI_ERR_OTHER,
                                        "**eqdropped");
                } else {
                    ctl_event = TRUE;
                }
        ...
        MPIR_ERR_CHKANDJUMP1(ret, mpi_errno, MPI_ERR_OTHER, "**ptleqget", "**ptleqget %s",
                             MPID_nem_ptl_strerror(ret));
        MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                        (MPL_DBG_FDEST,
                         ...
                         MPID_nem_ptl_strevent(&event), event.pt_index,
                         MPID_nem_ptl_strnifail(event.ni_fail_type),
                         MPID_nem_ptl_strlist(event.ptl_list), event.user_ptr, event.hdr_data,
                         event.mlength, event.rlength));
        MPIR_ERR_CHKANDJUMP2(event.ni_fail_type != PTL_NI_OK &&
                             event.ni_fail_type != PTL_NI_NO_MATCH, mpi_errno, MPI_ERR_OTHER,
                             "**ptlni_fail", "**ptlni_fail %s %s", MPID_nem_ptl_strevent(&event),
                             MPID_nem_ptl_strnifail(event.ni_fail_type));
        ...
        switch (event.type) {
            ...
            case PTL_EVENT_SEARCH:{
                    ...
                    MPL_DBG_MSG_P(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                                  "REQ_PTL(req)->event_handler = %p", REQ_PTL(req)->event_handler);
            ...
            case PTL_EVENT_ACK:
            default:
                MPL_error_printf("Received unexpected event type: %d %s", event.type,
                                 MPID_nem_ptl_strevent(&event));
