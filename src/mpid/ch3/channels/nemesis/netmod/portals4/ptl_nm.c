...
int MPID_nem_ptl_nm_init(void)
{
    ...
    overflow_me.options =
        (PTL_ME_OP_PUT | PTL_ME_MANAGE_LOCAL | PTL_ME_NO_TRUNCATE | PTL_ME_MAY_ALIGN |
                           ...
                           MPIR_Request * sreq)
{
    ...
    const size_t sent_sz =
        data_sz <
        (PAYLOAD_SIZE - sreq->dev.ext_hdr_sz) ? data_sz : (PAYLOAD_SIZE - sreq->dev.ext_hdr_sz -
                                                           sizeof(ptl_size_t));
    const size_t remaining = data_sz - sent_sz;
    const size_t sendbuf_sz =
        SENDBUF_SIZE(sent_sz + sreq->dev.ext_hdr_sz + (remaining ? sizeof(ptl_size_t) : 0));
    ...
    ret =
        MPID_nem_ptl_rptl_put(MPIDI_nem_ptl_global_md, (ptl_size_t) sendbuf, sendbuf_sz,
                              PTL_NO_ACK_REQ, vc_ptl->id, vc_ptl->ptc, match_bits, 0, sreq,
                              NPTL_HEADER(0, remaining));
    MPIR_ERR_CHKANDJUMP1(ret, mpi_errno, MPI_ERR_OTHER, "**ptlput", "**ptlput %s",
                         MPID_nem_ptl_strerror(ret));
    MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                    (MPL_DBG_FDEST, "PtlPut(size=%lu id=(%#x,%#x) pt=%#x)", sendbuf_sz,
                     vc_ptl->id.phys.nid, vc_ptl->id.phys.pid, vc_ptl->ptc));
        ...
        case PTL_EVENT_PUT:
            {
                ...
                if (remaining == 0) {
                    ...
                    do {
                        ...
                        ret = MPID_nem_ptl_rptl_get(MPIDI_nem_ptl_global_md, (ptl_size_t) buf_ptr,
                                                    size, vc_ptl->id, vc_ptl->ptc,
                                                    NPTL_MATCH(sender_req_id, GET_TAG,
                                                               MPIDI_Process.my_pg_rank),
                                                    target_offset, req);
                        MPIR_ERR_CHKANDJUMP1(ret, mpi_errno, MPI_ERR_OTHER, "**ptlget",
                                             "**ptlget %s", MPID_nem_ptl_strerror(ret));
                        MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                                        (MPL_DBG_FDEST,
                ...
                PtlMESearch(MPIDI_nem_ptl_ni, MPIDI_nem_ptl_control_pt, &search_me,
                            PTL_SEARCH_DELETE, NULL);
            }
            break;
