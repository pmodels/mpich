...
static void dequeue_req(const ptl_event_t * e)
{
    ...
    if (s_len > r_len) {
        ...
        MPIR_ERR_SET2(rreq->status.MPI_ERROR, MPI_ERR_TRUNCATE, "**truncate", "**truncate %d %d",
                      s_len, r_len);
    } else {
...
static int handler_recv_complete(const ptl_event_t * e)
{
    ...
    MPIR_Assert(e->type == PTL_EVENT_REPLY || e->type == PTL_EVENT_PUT ||
                e->type == PTL_EVENT_PUT_OVERFLOW);

    if (REQ_PTL(rreq)->md != PTL_INVALID_HANDLE) {
        ret = PtlMDRelease(REQ_PTL(rreq)->md);
        MPIR_ERR_CHKANDJUMP1(ret, mpi_errno, MPI_ERR_OTHER, "**ptlmdrelease", "**ptlmdrelease %s",
...
static int handler_recv_dequeue_complete(const ptl_event_t * e)
{
    ...
    MPIDI_Datatype_get_info(rreq->dev.user_count, rreq->dev.datatype, is_contig, data_sz, dt_ptr,
                            dt_true_lb);
    ...
    if (e->type == PTL_EVENT_PUT_OVERFLOW) {
        ...
        if (dt_basic_type != MPI_DATATYPE_NULL &&
            (e->mlength % MPIR_Datatype_get_basic_size(dt_basic_type)) != 0)
            MPIR_ERR_SET(rreq->status.MPI_ERROR, MPI_ERR_TYPE, "**dtypemismatch");
    }
...
static int handler_recv_big_get(const ptl_event_t * e)
{
    ...
    if (REQ_PTL(rreq)->num_gets == 0) {
        ...
        if (REQ_PTL(rreq)->chunk_buffer[0]) {
            ...
            MPIR_Segment_unpack(rreq->dev.segment_ptr, rreq->dev.segment_first, &last,
                                REQ_PTL(rreq)->chunk_buffer[0]);
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static void big_get(void *buf, ptl_size_t left_to_get, MPIDI_VC_t * vc, ptl_match_bits_t match_bits,
                    MPIR_Request * rreq)
{
    int ret;
    ...
    while (left_to_get > 0) {
        ...
        ret =
            MPID_nem_ptl_rptl_get(MPIDI_nem_ptl_global_md, start, get_sz, vc_ptl->id, vc_ptl->ptg,
                                  match_bits, 0, rreq);
...
static int handler_recv_dequeue_large(const ptl_event_t * e)
{
    ...
    if (dt_contig) {
        big_get((char *) rreq->dev.user_buf + dt_true_lb + PTL_LARGE_THRESHOLD,
                data_sz - PTL_LARGE_THRESHOLD, vc, e->match_bits, rreq);
        goto fn_exit;
    }
    ...
    if (last == rreq->dev.segment_size &&
        rreq->dev.segment_size <= MPIDI_nem_ptl_ni_limits.max_msg_size + PTL_LARGE_THRESHOLD) {
        ...
        ret =
            MPID_nem_ptl_rptl_get(REQ_PTL(rreq)->md, 0,
                                  rreq->dev.segment_size - rreq->dev.segment_first, vc_ptl->id,
                                  vc_ptl->ptg, e->match_bits, 0, rreq);
...
static int handler_recv_dequeue_unpack_large(const ptl_event_t * e)
{
    ...
    MPIR_CHKPMEM_MALLOC(REQ_PTL(rreq)->chunk_buffer[0], void *,
                        rreq->dev.segment_size - rreq->dev.segment_first, mpi_errno, "chunk_buffer",
                        MPL_MEM_BUFFER);
    big_get(REQ_PTL(rreq)->chunk_buffer[0], rreq->dev.segment_size - rreq->dev.segment_first, vc,
            e->match_bits, rreq);
...
int MPID_nem_ptl_recv_posted(MPIDI_VC_t * vc, MPIR_Request * rreq)
{
    ...
    MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                    (MPL_DBG_FDEST, "tag=%#x ctx=%#x rank=%#x", rreq->dev.match.parts.tag,
                     rreq->dev.match.parts.context_id, rreq->dev.match.parts.rank));
    me.match_bits =
        NPTL_MATCH(rreq->dev.match.parts.tag, rreq->dev.match.parts.context_id,
    ...
    MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                    (MPL_DBG_FDEST,
                     "count=" MPI_AINT_FMT_DEC_SPEC " datatype=%#x contig=%d data_sz=%lu",
                     rreq->dev.user_count, rreq->dev.datatype, dt_contig, data_sz));
    ...
    if (data_sz <= PTL_LARGE_THRESHOLD) {
        if (dt_contig) {
            ...
            rreq->dev.segment_ptr =
                MPIR_Segment_alloc(rreq->dev.user_buf, rreq->dev.user_count, rreq->dev.datatype);
            ...
            last = rreq->dev.segment_size;
            rreq->dev.iov_count = MPL_IOV_LIMIT;
            MPIR_Segment_to_iov(rreq->dev.segment_ptr, rreq->dev.segment_first, &last,
            ...
            if (last == rreq->dev.segment_size) {
                ...
                MPL_DBG_MSG(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                            "    IOV too long: using bounce buffer");
                MPIR_CHKPMEM_MALLOC(REQ_PTL(rreq)->chunk_buffer[0], void *, data_sz, mpi_errno,
        ...
        if (dt_contig) {
            ...
            if (last == PTL_LARGE_THRESHOLD) {
                 ...
                 * buffer and unpack later */
                MPL_DBG_MSG(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                            "    IOV too long: using bounce buffer for first chunk");
                MPIR_CHKPMEM_MALLOC(REQ_PTL(rreq)->chunk_buffer[0], void *, PTL_LARGE_THRESHOLD,
                                    mpi_errno, "chunk_buffer", MPL_MEM_BUFFER);
                me.start = REQ_PTL(rreq)->chunk_buffer[0];
                me.length = PTL_LARGE_THRESHOLD;
    ...
    ret =
        PtlMEAppend(MPIDI_nem_ptl_ni, MPIDI_nem_ptl_pt, &me, PTL_PRIORITY_LIST, rreq,
                    &REQ_PTL(rreq)->put_me);
...
int MPID_nem_ptl_lmt_start_recv(MPIDI_VC_t * vc, MPIR_Request * rreq, MPL_IOV s_cookie)
{
    ...
    if (dt_contig) {
        ...
        big_get((char *) real_user_buf + PTL_LARGE_THRESHOLD, data_sz - PTL_LARGE_THRESHOLD, vc,
                match_bits, rreq);
        ...
        if (last == rreq->dev.segment_size &&
            ...
            /* FIXME: For now, allocate a single large buffer to hold entire message */
            MPIR_CHKPMEM_MALLOC(REQ_PTL(rreq)->chunk_buffer[0], void *,
                                rreq->dev.segment_size - rreq->dev.segment_first, mpi_errno,
                                "chunk_buffer", MPL_MEM_BUFFER);
            big_get(REQ_PTL(rreq)->chunk_buffer[0],
                    rreq->dev.segment_size - rreq->dev.segment_first, vc, match_bits, rreq);
        }
    }
