...
static int set_up_listener(void)
{
    ...
    MPIR_ERR_CHKANDJUMP2(ret == -1, mpi_errno, MPI_ERR_OTHER, "**listen", "**listen %s %d",
                                ...
                                MPIDI_CH3I_nem_tcp_ifaddr_t * ifaddr)
{
    ...
    if (MPIR_CVAR_NEMESIS_TCP_NETWORK_IFACE) {
        int len;
        mpi_errno =
            MPIDI_Get_IP_for_iface(MPIR_CVAR_NEMESIS_TCP_NETWORK_IFACE, ifaddr, &ifaddrFound);
        MPIR_ERR_CHKANDJUMP1(mpi_errno ||
                             !ifaddrFound, mpi_errno, MPI_ERR_OTHER, "**iface_notfound",
        ...
        MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CONNECT, VERBOSE, (MPL_DBG_FDEST,
                                                         ...
                                                         ifaddr->type, ifaddr->len,
                                                         ifaddr->ifaddr[0], ifaddr->ifaddr[1],
                                                         ifaddr->ifaddr[2], ifaddr->ifaddr[3]));
    ...
    if (!ifaddrFound) {
        ...
        MPIR_ERR_CHKANDJUMP2(!info ||
                             !info->h_addr_list, mpi_errno, MPI_ERR_OTHER, "**gethostbyname",
...
int MPID_nem_tcp_bind(int sockfd)
{
    ...
    MPIR_ERR_CHKANDJUMP3(ret == -1, mpi_errno, MPI_ERR_OTHER, "**sock|poll|bind",
