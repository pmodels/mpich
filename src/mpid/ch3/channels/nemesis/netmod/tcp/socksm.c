...
static inline int is_same_connection(sockconn_t * sc1, sockconn_t * sc2)
{
    ...
    return !sc1->is_tmpvc && !sc2->is_tmpvc &&
        ...
        ((sc1->is_same_pg && sc2->is_same_pg) ||
         (!sc1->is_same_pg && !sc2->is_same_pg && IS_SAME_PGID(sc1->pg_id, sc2->pg_id)));
}
...
static int alloc_sc_plfd_tbls(void)
{
    ...
    MPIR_Memcpy(&MPID_nem_tcp_plfd_tbl[idx], &MPID_nem_tcp_g_lstn_plfd,
                sizeof(MPID_nem_tcp_g_lstn_plfd));
...
static int expand_sc_plfd_tbls(void)
{
    ...
    MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                    (MPL_DBG_FDEST, "expand_sc_plfd_tbls af g_sc_tbl[0].fd=%d", g_sc_tbl[0].fd));
    for (i = 0; i < g_tbl_capacity; ++i) {
        sockconn_t *sc = &g_sc_tbl[i];
        MPIDI_VC_t *vc = sc->vc;
        ...
        MPIR_Assert(MPID_nem_tcp_plfd_tbl[i].fd == CONN_INVALID_FD ||
                    sc->state.cstate != CONN_STATE_TS_COMMRDY || vc_tcp->sc == sc);
    }
...
static int found_better_sc(sockconn_t * sc, sockconn_t ** fnd_sc)
{
    ...
    for (i = 0; i < g_tbl_size && !found; i++) {
        ...
        if (iter_sc != sc && iter_sc->fd != CONN_INVALID_FD && is_same_connection(iter_sc, sc)) {
            switch (sc->state.cstate) {
                case CONN_STATE_TC_C_CNTD:
                    MPIR_Assert(fnd_sc == NULL);
                    if (istate == CONN_STATE_TS_COMMRDY ||
                        istate == CONN_STATE_TA_C_RANKRCVD || istate == CONN_STATE_TC_C_TMPVCSENT)
                        found = TRUE;
                    break;
  ...
  fn_exit:
    if (found) {
        if (fnd_sc) {
            MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                            (MPL_DBG_FDEST,
                             "found_better_sc(sc=%p (%s), *fnd_sc=%p (%s)) found=TRUE", sc,
                             CONN_STATE_STR[sc->state.cstate], *fnd_sc,
                             (*fnd_sc ? CONN_STATE_STR[(*fnd_sc)->state.cstate] : "N/A")));
        } else {
            MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
...
static int vc_is_in_shutdown(MPIDI_VC_t * vc)
{
    ...
    MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                    (MPL_DBG_FDEST, "vc_is_in_shutdown(%p)=%s", vc, (retval ? "TRUE" : "FALSE")));
    return retval;
}
...
static int send_id_info(const sockconn_t * const sc)
{
    ...
    MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                    (MPL_DBG_FDEST, "my_pg->id=%s my_pg->rank=%d, sc->pg_rank=%d sc->is_same_pg=%s",
...
static int send_tmpvc_info(const sockconn_t * const sc)
{
    ...
    MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                    (MPL_DBG_FDEST, "my->pg_rank=%d, sc->pg_rank=%d", MPIDI_Process.my_pg_rank,
                     sc->pg_rank));
    ...
    *got_sc_eof = 0;
    ...
    if (hdr.pkt_type == MPIDI_NEM_TCP_SOCKSM_PKT_ID_INFO) {
        ...
        {       /* Added this block sp we can declare pointers to tcp private area of vc */
            ...
            MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                            (MPL_DBG_FDEST,
                             "about to incr sc_ref_count sc=%p sc->vc=%p sc_ref_count=%d", sc,
                             sc_vc, sc_vc_tcp->sc_ref_count));
        ...
        MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                        (MPL_DBG_FDEST,
                         "PKT_ID_INFO: sc->pg_rank=%d sc->is_same_pg=%d sc->fd=%d, sc->vc=%p, sc=%p",
                         sc->pg_rank, sc->is_same_pg, sc->fd, sc->vc, sc));
        ...
        if (vc == NULL) {
            mpi_errno =
                MPIR_Err_create_code(MPI_SUCCESS, MPIR_ERR_FATAL, __func__, __LINE__, MPI_ERR_OTHER,
                                     "**nomem", NULL);
            goto fn_fail;
        }
        /* --END ERROR HANDLING-- */
        ...
        MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                        (MPL_DBG_FDEST,
                         "about to incr sc_ref_count sc=%p sc->vc=%p sc_ref_count=%d", sc, sc->vc,
                         vc_tcp->sc_ref_count));
        ++vc_tcp->sc_ref_count;
        ...
        MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                        (MPL_DBG_FDEST, "enqueuing on acceptq vc=%p, sc->fd=%d, tag=%d", vc, sc->fd,
                         sc->vc->port_name_tag));
...
static int send_cmd_pkt(int fd, MPIDI_nem_tcp_socksm_pkt_type_t pkt_type)
{
    ...
    MPIR_ERR_CHKANDJUMP1(offset != pkt_len, mpi_errno, MPI_ERR_OTHER, "**write", "**write %s",
                         MPIR_Strerror(errno, strerrbuf));   /* FIXME-Z1 */
  fn_exit:
    return mpi_errno;
  fn_fail:
    MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
...
int MPID_nem_tcp_connect(struct MPIDI_VC *const vc)
{
    ...
    if (vc_tcp->state == MPID_NEM_TCP_VC_STATE_DISCONNECTED) {
        ...
        if (vc_tcp->connect_retry_count > MPIDI_NEM_TCP_MAX_CONNECT_RETRIES) {
            ...
            MPIR_ERR_SET1(mpi_errno, MPI_ERR_OTHER, "**exceeded_connect_tries",
                          "**exceeded_connect_tries %d", vc->pg_rank);
        ...
        MPIR_ERR_CHKANDJUMP2(sc->fd == -1, mpi_errno, MPI_ERR_OTHER, "**sock_create",
                             "**sock_create %s %d", MPIR_Strerror(errno, strerrbuf), errno);
...
int close_cleanup_and_free_sc_plfd(sockconn_t * const sc)
{
    ...
    MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                    (MPL_DBG_FDEST, "vc=%p, sc=%p, closing fd=%d", sc->vc, sc, sc->fd));
    ...
    if (rc == -1 && errno != EAGAIN && errno != EBADF)
        MPIR_ERR_SET1(mpi_errno, MPI_ERR_OTHER, "**close", "**close %s", MPIR_Strerror(errno, strerrbuf));
...
static int state_tc_c_cntd_handler(struct pollfd *const plfd, sockconn_t * const sc)
{
    ...
    if (IS_WRITEABLE(plfd)) {
        ...
        if (!sc->is_tmpvc) {    /* normal connection */
            ...
            if (mpi_errno) {
                MPL_DBG_MSG(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                            "state_tc_c_cntd_handler(): closing sc");
                mpi_errno = close_cleanup_and_free_sc_plfd(sc); /* QUIESCENT */
    ...
    if (IS_READABLE(plfd)) {
        ...
        switch (pkt_type) {
            case MPIDI_NEM_TCP_SOCKSM_PKT_ID_ACK:
                ...
                MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                                (MPL_DBG_FDEST,
                                 "c_ranksent_handler(): connection established (sc=%p, sc->vc=%p, fd=%d)",
                                 sc, sc->vc, sc->fd));
                break;
    ...
    if (IS_READABLE(plfd)) {
        ...
        if (mpi_errno) {
            ...
            if (pkt_type == MPIDI_NEM_TCP_SOCKSM_PKT_TMPVC_ACK) {
                ...
                MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                                (MPL_DBG_FDEST,
                                 "c_tmpvcsent_handler(): connection established (fd=%d, sc=%p, sc->vc=%p)",
                                 sc->fd, sc, sc_vc));
            } else {    /* pkt_type must be MPIDI_NEM_TCP_SOCKSM_PKT_ID_NAK */
                MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                                (MPL_DBG_FDEST, "state_c_tmpvcsent_handler() 2: closing sc"));
                MPL_DBG_MSG(MPIDI_NEM_TCP_DBG_DET, VERBOSE, "received NAK, closing sc");
                mpi_errno = close_cleanup_and_free_sc_plfd(sc); /* QUIESCENT */
            }
        }
...
static int state_l_cntd_handler(struct pollfd *const plfd, sockconn_t * const sc)
{
    ...
    if (IS_READABLE(plfd)) {
        ...
        MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                        (MPL_DBG_FDEST, "!IS_READABLE(plfd) fd=%d events=%#x revents=%#x", plfd->fd,
                         plfd->events, plfd->revents));
    }
...
static int do_i_win(sockconn_t * rmt_sc)
{
    ...
    MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                    (MPL_DBG_FDEST,
                     "do_i_win(rmt_sc=%p (%s)) win=%s is_same_pg=%s my_pg_rank=%d rmt_pg_rank=%d",
                     rmt_sc, CONN_STATE_STR[rmt_sc->state.cstate], (win ? "TRUE" : "FALSE"),
                     (rmt_sc->is_same_pg ? "TRUE" : "FALSE"), MPIDI_Process.my_pg_rank,
                     rmt_sc->pg_rank));
...
static int state_l_rankrcvd_handler(struct pollfd *const plfd, sockconn_t * const sc)
{
    ...
    if (IS_WRITEABLE(plfd)) {
        ...
        if (snd_nak) {
            ...
            MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                            (MPL_DBG_FDEST,
                             "connection established: sc=%p, sc->vc=%p, sc->fd=%d, is_same_pg=%s, pg_rank=%d",
                             sc, sc_vc, sc->fd, (sc->is_same_pg ? "TRUE" : "FALSE"), sc->pg_rank));
            MPID_nem_tcp_conn_est(sc_vc);
            sc_vc_tcp->connect_retry_count = 0; /* successfully connected, reset connection retry count */
        }
    }
...
static int state_l_tmpvcrcvd_handler(struct pollfd *const plfd, sockconn_t * const sc)
{
    ...
    if (IS_WRITEABLE(plfd)) {
        ...
        sc_vc_tcp->connect_retry_count = 0;     /* successfully connected, reset connection retry count */
        MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                        (MPL_DBG_FDEST, "fd=%d: TMPVC_ACK sent, connection established!", sc->fd));
    }
...
static int MPID_nem_tcp_recv_handler(sockconn_t * const sc)
{
    ...
    if (sc_vc->ch.recv_active == NULL) {
        ...
        if (bytes_recvd <= 0) {
            ...
            if (bytes_recvd == 0) {
                ...
                if (vc_is_in_shutdown(sc_vc)) {
                     ...
                     * blow up here. */
                    MPL_DBG_MSG(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                                "other side closed, but we're shutting down, closing sc");
                    ...
                    MPL_DBG_MSG_D(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                                  "ERROR: sock (fd=%d) is closed: bytes_recvd == 0", sc->fd);
        ...
        MPL_DBG_MSG_FMT(MPIDI_CH3_DBG_CHANNEL, VERBOSE,
                        (MPL_DBG_FDEST, "New recv %" PRIdPTR " (fd=%d, vc=%p, sc=%p)", bytes_recvd,
                         sc_fd, sc_vc, sc));

        mpi_errno = MPID_nem_handle_pkt(sc_vc, recv_buf, bytes_recvd);
        if (mpi_errno)
            MPIR_ERR_POP_LABEL(mpi_errno, fn_noncomm_fail);
    } else {
  ...
  fn_fail:     /* comm related failures jump here */
    {
        ...
        MPIR_ERR_SET1(mpi_errno, MPIX_ERR_PROC_FAILED, "**comm_fail", "**comm_fail %d",
                      sc_vc->pg_rank);
    ...
    for (i = 0; i < num_polled; i++) {
        ...
        if (it_plfd->fd != CONN_INVALID_FD && it_plfd->revents != 0) {
            ...
            if (it_plfd->revents & POLLERR || it_plfd->revents & POLLNVAL) {
                ...
                if (rc < 0)
                    err_str = MPIR_Strerror(errno, strerrbuf);
                ...
                if (it_sc->vc) {
                    MPIR_ERR_SET2(req_errno, MPIX_ERR_PROC_FAILED, "**comm_fail",
                                  "**comm_fail %d %s", it_sc->vc->pg_rank, err_str);
                    mpi_errno = MPID_nem_tcp_cleanup_on_error(it_sc->vc, req_errno);
                    MPIR_ERR_CHKANDJUMP(mpi_errno, mpi_errno, MPI_ERR_OTHER, "**tcp_cleanup_fail");
                } else {
                    MPIR_ERR_SET2(req_errno, MPIX_ERR_PROC_FAILED, "**comm_fail_conn",
                                  "**comm_fail_conn %s %s", CONN_STATE_STR[it_sc->state.cstate],
                                  err_str);
        ...
        if ((connfd = accept(l_sc->fd, (SA *) & rmt_addr, &len)) < 0) {
            MPL_DBG_MSG_FMT(MPIDI_NEM_TCP_DBG_DET, VERBOSE,
                            (MPL_DBG_FDEST,
                             ...
                             l_sc->fd, connfd, errno, MPIR_Strerror(errno, strerrbuf)));
            if (errno == EINTR)
                continue;
