    ...
    *matched_win = NULL;
    ...
    while (elem != NULL) {
        ...
        mpi_errno = MPIR_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
                                        base_shm_offs, 1, MPI_AINT, node_comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
                                      ...
                                      MPIR_Comm * comm_ptr, MPIR_Win ** win_ptr)
{
    ...
    if (node_rank == 0) {
        ...
        mpi_errno =
            MPIR_Bcast(serialized_hnd_ptr, MPL_SHM_GHND_SZ, MPI_CHAR, 0, node_comm_ptr, MPIR_SUBGROUP_NONE,
                       MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
        ...
        mpi_errno =
            MPIR_Bcast(serialized_hnd, MPL_SHM_GHND_SZ, MPI_CHAR, 0, node_comm_ptr, MPIR_SUBGROUP_NONE,
                       MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
    ...
    mpi_errno = MPIR_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, tmp_buf, 4, MPI_AINT,
                                    (*win_ptr)->comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
    MPIR_ERR_CHECK(mpi_errno);
                                       ...
                                       int must)
{
    ...
    mpi_errno = MPIR_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
                               ...
                               node_comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
