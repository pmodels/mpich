...
uint32_t MPID_nem_ib_module_refill_srq(
        ...
        struct ibv_srq *srq, uint32_t max_post)
{
    ...
    MPID_nem_ib_module_cell_elem_t *ce_root;
    MPID_nem_ib_module_cell_elem_t *ce_prev;
    int ret;
    ...
    while( (post_count < max_post) &&
        ...
        MPID_nem_ib_module_get_cell(&ce);

        ce->nem_cell = c;

        MPID_nem_ib_module_prep_cell_recv(ce, 
...
int MPID_nem_ib_module_poll (MPID_nem_poll_dir_t in_or_out)
{
    ...
    int ret;
    MPID_nem_ib_module_queue_elem_t *e, *sqe;
    MPID_nem_ib_module_cell_elem_t *ce;
    MPIDI_VC_t *vc;
    struct ibv_wc wc;

    ret = MPID_nem_ib_module_poll_cq(
    ...
    if(ret) {
        ...
        ce = (MPID_nem_ib_module_cell_elem_t *) wc.wr_id;
    ...
    if(!MPID_nem_ib_module_queue_empty(MPID_nem_ib_module_vc_queue)) {

        /* We got VCs to process, get busy! */

        MPID_nem_ib_module_queue_dequeue(MPID_nem_ib_module_vc_queue, &e);
