...
int MPID_nem_tcp_module_finalize()
{
    ...
    mpi_errno = MPID_nem_tcp_module_poll_finalize();
    if (mpi_errno) MPIU_ERR_POP(mpi_errno);
    mpi_errno = MPID_nem_tcp_module_send_finalize();
    if (mpi_errno) MPIU_ERR_POP(mpi_errno);
    mpi_errno = MPID_nem_tcp_module_sm_finalize();
    if (mpi_errno) MPIU_ERR_POP(mpi_errno);
     
    if (MPID_nem_tcp_module_g_lstn_sc.fd)
    {
        CHECK_EINTR (ret, close(MPID_nem_tcp_module_g_lstn_sc.fd));
...
#undef FCNAME
#define FCNAME MPIDI_QUOTE(FUNCNAME)
int MPID_nem_tcp_module_ckpt_shutdown()
{
    return MPID_nem_tcp_module_finalize();
}
