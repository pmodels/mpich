...
/* globals */
extern pollfd_t *MPID_nem_tcp_module_plfd_tbl;
extern sockconn_t MPID_nem_tcp_module_g_lstn_sc;
extern pollfd_t MPID_nem_tcp_module_g_lstn_plfd;

extern char *MPID_nem_tcp_module_recv_buf;
#define MPID_NEM_TCP_MODULE_RECV_MAX_PKT_LEN 1024


#define MPID_NEM_TCP_MODULE_VC_STATE_DISCONNECTED 0
#define MPID_NEM_TCP_MODULE_VC_STATE_CONNECTED 1
...
typedef struct 
{
    ...
    int sc_ref_count;
} MPID_nem_tcp_module_vc_area;

/* accessor macro to private fields in VC */
#define VC_FIELD(vc, field) (((MPID_nem_tcp_module_vc_area *)((MPIDI_CH3I_VC *)(vc)->channel_private)->netmod_area.padding)->field)
...
int MPID_nem_tcp_module_send_init(void);
int MPID_nem_tcp_module_poll_init(void);
int MPID_nem_tcp_module_connect(struct MPIDI_VC *const vc);
int MPID_nem_tcp_module_connpoll(void);
int MPID_nem_tcp_module_sm_init(void);
int MPID_nem_tcp_module_sm_finalize(void);
int MPID_nem_tcp_module_set_sockopts(int fd);
MPID_NEM_TCP_MODULE_SOCK_STATUS_t MPID_nem_tcp_module_check_sock_status(const pollfd_t *const plfd);
int MPID_nem_tcp_module_poll_finalize(void);
int MPID_nem_tcp_module_send_finalize(void);
int MPID_nem_tcp_module_bind(int sockfd);
int MPID_nem_tcp_module_conn_est(MPIDI_VC_t *vc);
...
int MPID_nem_tcp_module_disconnect(struct MPIDI_VC *const vc);
int MPID_nem_tcp_module_cleanup (struct MPIDI_VC *const vc);
int MPID_nem_tcp_module_state_listening_handler(pollfd_t *const l_plfd, sockconn_t *const l_sc);
int MPID_nem_tcp_module_send_queued(MPIDI_VC_t *vc);
...
int MPID_nem_tcp_module_socksm_finalize(void);
int MPID_nem_tcp_module_socksm_init(void);
...
#define SET_PLFD(ep)   MPID_nem_tcp_module_plfd_tbl[VC_FIELD(ep, sc)->index].events |= POLLOUT
#define UNSET_PLFD(ep) MPID_nem_tcp_module_plfd_tbl[VC_FIELD(ep, sc)->index].events &= ~POLLOUT
