...
{
    int mpi_errno = MPI_SUCCESS;
    MPIDI_STATE_DECL(MPID_STATE_MPID_NEM_GM_MODULE_SEND);

    MPIDI_FUNC_ENTER(MPID_STATE_MPID_NEM_GM_MODULE_SEND);    
 ...
 fn_exit:
    MPIDI_FUNC_EXIT(MPID_STATE_MPID_NEM_GM_MODULE_SEND);
    return mpi_errno;
 fn_fail:
    goto fn_exit;
}
