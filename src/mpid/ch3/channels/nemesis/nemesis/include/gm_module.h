...
int MPID_nem_gm_module_init (MPID_nem_queue_ptr_t proc_recv_queue, MPID_nem_queue_ptr_t proc_free_queue, MPID_nem_cell_ptr_t proc_elements,
...
int MPID_nem_gm_module_finalize (void);
int MPID_nem_gm_module_ckpt_shutdown (void);
int MPID_nem_gm_module_poll (MPID_nem_poll_dir_t in_or_out);
int MPID_nem_gm_module_send (MPIDI_VC_t *vc, MPID_nem_cell_ptr_t cell, int datalen);
int MPID_nem_gm_module_get_business_card (int my_rank, char **bc_val_p, int *val_max_sz_p);
int MPID_nem_gm_module_connect_to_root (const char *business_card, MPIDI_VC_t *new_vc);
int MPID_nem_gm_module_vc_init (MPIDI_VC_t *vc, const char *business_card);
int MPID_nem_gm_module_vc_destroy(MPIDI_VC_t *vc);
int MPID_nem_gm_module_vc_terminate (MPIDI_VC_t *vc);

int MPID_nem_gm_module_test (void);

int MPID_mem_gm_module_register_mem (void *p, int len);
int MPID_nem_gm_module_deregister_mem (void *p, int len);

/* completion counter is atomically decremented when operation completes */
int MPID_nem_gm_module_get (void *target_p, void *source_p, int len, MPIDI_VC_t *source_vc, int *completion_ctr);
