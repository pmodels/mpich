...
static int init_ib_ctxt(void)
{
    ...
    for(i = 0; i < count; i++) {
        ...
        mpi_errno = MPID_nem_ib_open_hca(dev_list[i], 
        ...
        mpi_errno = MPID_nem_ib_alloc_pd
            (MPID_nem_ib_ctxt_ptr->ib_dev[i].context,
        ...
        mpi_errno = MPID_nem_ib_get_dev_attr
            (MPID_nem_ib_ctxt_ptr->ib_dev[i].context,
        ...
        mpi_errno = MPID_nem_ib_create_cq
            (MPID_nem_ib_ctxt_ptr->ib_dev[i].context,
        ...
        mpi_errno = MPID_nem_ib_create_srq
            (MPID_nem_ib_ctxt_ptr->ib_dev[i].context,
        ...
        int *val_max_sz_p)
{
    ...
    mpi_errno = MPID_nem_ib_init_cm_param();
    if(mpi_errno) {
        MPIU_ERR_POP(mpi_errno);
    }

    mpi_errno = MPID_nem_ib_init_dev_param();
    if(mpi_errno) {
        MPIU_ERR_POP(mpi_errno);
    }
    ...
    mpi_errno = MPID_nem_ib_get_business_card
    ...
    mpi_errno = MPID_nem_ib_init_cell_pool(
    ...
    while(!MPID_nem_queue_empty(MPID_nem_module_ib_free_queue)
        ...
        MPID_nem_ib_prep_cell_recv(ce, (void *) MPID_NEM_CELL_TO_PACKET(c));
    ...
    *module_free_queue = MPID_nem_module_ib_free_queue;
    ...
    MPID_nem_ib_queue_init(&MPID_nem_ib_vc_queue);

    for(i = 0; i < MPID_nem_ib_dev_param_ptr->max_vc_queue; i++) {

        MPID_nem_ib_queue_elem_t *e;

        mpi_errno = MPID_nem_ib_queue_new_elem(&e, NULL);
        if(mpi_errno) {
            MPIU_ERR_POP(mpi_errno);
        }
            
        MPID_nem_ib_queue_free(MPID_nem_ib_vc_queue, e);
    }

    NEM_IB_DBG("End of ib_init module elem %d, proc elem %d, "
            "posted SRQ %d\n",
            num_module_elements, 
            num_proc_elements,
            MPID_nem_ib_ctxt_ptr->ib_dev[0].srq_n_posted);
