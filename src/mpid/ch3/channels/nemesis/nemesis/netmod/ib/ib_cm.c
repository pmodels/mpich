...
MPID_nem_ib_queue_ptr_t MPID_nem_ib_qp_queue = 0;
...
static int cm_create_rc_qp(struct ibv_qp **qp)
{
    ...
    MPID_nem_ib_queue_elem_ptr_t e;
    ...
    *qp = ibv_create_qp(MPID_nem_ib_cm_ctxt_ptr->prot_domain,
    ...
    MPID_nem_ib_queue_alloc(MPID_nem_ib_qp_queue, &e);

    e->data = *qp;

    NEM_IB_DBG("Enqueing qp %p", *qp);

    MPID_nem_ib_queue_enqueue(MPID_nem_ib_qp_queue, e);
...
int MPID_nem_ib_cm_init_cm()
{
    ...
    MPID_nem_ib_queue_init(&MPID_nem_ib_qp_queue);

    /* Initialize the Hash table */

    mpi_errno = MPID_nem_ib_init_hash_table(&ctxt_ptr->hash_table,
...
int MPID_nem_ib_cm_finalize_rc_qps()
{
    ...
    while(!MPID_nem_ib_queue_empty(
                MPID_nem_ib_qp_queue)) {

        MPID_nem_ib_queue_dequeue(
                MPID_nem_ib_qp_queue, &e);
    ...
    MPID_nem_ib_queue_finalize(MPID_nem_ib_qp_queue);
