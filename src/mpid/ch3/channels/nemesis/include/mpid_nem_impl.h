...
#define MPID_nem_mpich_release_fbox(cell)                               \
    do {                                                                \
        MPL_inter_atomic_release_store_int(&MPID_nem_mem_region.mailboxes.in[(cell)->pkt.header.source]->mpich.flag.value, 0); \
    } while (0)
...
#define MPID_nem_fbox_is_full(pbox_) (MPL_inter_atomic_acquire_load_int(&(pbox_)->flag.value))
