                                  ...
                                  **intercomm, int *errcodes) 
{
	...
	/* FIXME: info may be needed for port name */
        mpi_errno = MPID_Open_port(NULL, port_name, MPI_MAX_PORT_NAME);
    ...
    if (errcodes != MPI_ERRCODES_IGNORE) {
        mpi_errno = MPIR_Bcast(&should_accept, 1, MPI_INT, root, comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);

        mpi_errno = MPIR_Bcast(&total_num_processes, 1, MPI_INT, root, comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
        
        mpi_errno = MPIR_Bcast(errcodes, total_num_processes, MPI_INT, root, comm_ptr, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
        MPIR_ERR_CHECK(mpi_errno);
    }
