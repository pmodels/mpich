		       ...
		       MPIR_Comm *comm_ptr, MPIR_Comm **newcomm)
{
    ...
    mpi_errno = MPIR_Bcast_allcomm_auto(recv_ints, 3, MPI_INT, root, comm_ptr);
    MPIR_ERR_CHECK(mpi_errno);
    ...
    mpi_errno = MPIR_Barrier_allcomm_auto(comm_ptr);
    MPIR_ERR_CHECK(mpi_errno);
 ...
 no_port:
    {
       ...
       /* broadcast error notification to other processes */
        ...
        mpi_errno2 = MPIR_Bcast_allcomm_auto(recv_ints, 3, MPI_INT, root, comm_ptr);
        if (mpi_errno2) MPIR_ERR_ADD(mpi_errno, mpi_errno2);
        goto fn_fail;
    }
}
				   ...
				   int n_remote_pgs, MPIDI_PG_t *remote_pg[] )
{
	...
	mpi_errno = MPIR_Bcast_allcomm_auto(&j, 1, MPI_INT, root, comm_ptr);
	MPIR_ERR_CHECK(mpi_errno);
	...
	mpi_errno = MPIR_Bcast_allcomm_auto(pg_str, j, MPI_CHAR, root, comm_ptr);
	MPIR_ERR_CHECK(mpi_errno);
...
int MPID_PG_BCast( MPIR_Comm *peercomm_p, MPIR_Comm *comm_p, int root )
{
    ...
    mpi_errno = MPIR_Bcast( &n_local_pgs, 1, MPI_INT, root, comm_p);
    MPIR_ERR_CHECK(mpi_errno);
	...
	mpi_errno = MPIR_Bcast( &len, 1, MPI_INT, root, comm_p);
        MPIR_ERR_CHECK(mpi_errno);
	if (rank != root) {
	...
	mpi_errno = MPIR_Bcast( pg_str, len, MPI_CHAR, root, comm_p);
        if (mpi_errno) {
            if (rank != root)
                MPL_free( pg_str );
            MPIR_ERR_POP(mpi_errno);
				...
				pg_node *pg_list )
{
	...
	mpi_errno = MPIC_Send(&i, 1, MPI_INT, 0, sendtag++, tmp_comm);
	*sendtag_p = sendtag;
	MPIR_ERR_CHECK(mpi_errno);
