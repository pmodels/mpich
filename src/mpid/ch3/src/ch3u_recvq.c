				    ...
				    MPIDI_Message_match * match)
{
	...
	prev_rreq = cur_rreq;
	cur_rreq = cur_rreq->next;
    }

    if (matching_cur_rreq != NULL) {
	if (matching_prev_rreq != NULL) {
	    matching_prev_rreq->next = matching_cur_rreq->next;
	}
	else {
	    recvq_unexpected_head = matching_cur_rreq->next;
	}
	
	if (matching_cur_rreq->next == NULL) {
    ...
    if (rreq) {
        ...
        if (tag != MPI_ANY_TAG && source != MPI_ANY_SOURCE) {
            do {
                ...
                if (MATCH_WITH_LEFT_MASK(rreq->dev.match, match, mask)) {
                    if (prev_rreq != NULL) {
                        prev_rreq->next = rreq->next;
                    }
                    else {
                        recvq_unexpected_head = rreq->next;
                    }
...
int MPIDI_CH3U_Recvq_DP(MPIR_Request * rreq)
{
	...
	if (cur_rreq == rreq) {
	    if (prev_rreq != NULL) {
		prev_rreq->next = cur_rreq->next;
	    }
	    else {
		recvq_posted_head = cur_rreq->next;
	    }
	    if (cur_rreq->next == NULL) {
					   ...
					   int * foundp)
{
	...
	if (recvq_unexpected_tail != NULL) {
	    recvq_unexpected_tail->next = rreq;
	}
	else {
...
int MPIDI_CH3U_Clean_recvq(MPIR_Comm *comm_ptr)
{
    ...
    while (NULL != rreq) {
        ...
        match.parts.context_id =
            (MPIR_Context_id_t) (comm_ptr->recvcontext_id + MPIR_CONTEXT_INTRA_PT2PT);
        ...
        match.parts.context_id =
            (MPIR_Context_id_t) (comm_ptr->recvcontext_id + MPIR_CONTEXT_INTRA_COLL);
        ...
        if (MPIR_Comm_is_parent_comm(comm_ptr)) {
            ...
            match.parts.context_id = (MPIR_Context_id_t) (comm_ptr->recvcontext_id + MPIR_CONTEXT_INTRANODE_OFFSET + offset);
            ...
            offset = (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) ?  MPIR_CONTEXT_INTRA_PT2PT : MPIR_CONTEXT_INTER_PT2PT;
            match.parts.context_id = (MPIR_Context_id_t) (comm_ptr->recvcontext_id + MPIR_CONTEXT_INTERNODE_OFFSET + offset);
    ...
    while (NULL != rreq) {
        /* We'll have to do this matching twice. Once for the pt2pt context id
         * and once for the collective context id */
        match.parts.context_id = (MPIR_Context_id_t) (comm_ptr->recvcontext_id + MPIR_CONTEXT_INTRA_PT2PT);
        ...
        match.parts.context_id = (MPIR_Context_id_t) (comm_ptr->recvcontext_id + MPIR_CONTEXT_INTRA_COLL);
