    ...
    switch (pInput->header.type) {
        case RLOG_EVENT_TYPE:
            ...
            printf(" %3d:%04d, start: %10g, end: %10g, %d\n",
                   ...
                   pInput->record.event.end_time, pInput->record.event.recursion);
            break;
        case RLOG_IARROW_TYPE:
            ...
            printf(" %3d %s %3d, tag: %3d, length: %d\n",
                   ...
                   pInput->record.iarrow.tag, pInput->record.iarrow.length);
            break;
        case RLOG_COMM_TYPE:
            printf("RLOG_COMM   -");
            printf(" %3d, newcomm: %d\n", pInput->record.comm.rank, pInput->record.comm.newcomm);
            break;
        case RLOG_STATE_TYPE:
            printf("RLOG_STATE  -");
            printf(" id: %3d, %12s : %s\n",
                   pInput->record.state.event,
                   pInput->record.state.color, pInput->record.state.description);
            break;
