...
{
    printf("RLOG_STATE -");
    printf(" id: %3d, %12s : %s\n", pState->event, pState->color, pState->description);
}
...
void PrintEvent(RLOG_EVENT * pEvent)
{
    ...
    printf(" %3d:%04d, start: %4.07g, end: %4.07g, %d\n",
           pEvent->rank, pEvent->event, pEvent->start_time, pEvent->end_time, pEvent->recursion);
}
...
void PrintEventAndIndex(RLOG_EVENT * pEvent, int index)
{
    ...
    printf(" %3d:%04d, start: % 10f, end: % 10f, %d, #%d\n",
           pEvent->rank,
           pEvent->event, pEvent->start_time, pEvent->end_time, pEvent->recursion, index);
}
...
void PrintArrow(RLOG_ARROW * pArrow)
{
    ...
    printf(" %2d -> %2d, %s tag:%3d, len: %d, start: %g, end: %g\n",
           ...
           pArrow->tag, pArrow->length, pArrow->start_time, pArrow->end_time);
}

void PrintComm(RLOG_COMM * pComm)
{
    printf("RLOG_COMM  -");
    printf(" %3d, newcomm: %d\n", pComm->rank, pComm->newcomm);
}
