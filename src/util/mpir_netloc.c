                               ...
                               MPIR_Netloc_network_attributes * network_attr)
{
    ...
    network_attr->type = MPIR_HWTOPO_NET_TYPE__INVALID;
    ...
    if (!mpi_errno) {
        ...
        if (!mpi_errno) {
            ...
            if (!out_degree_mismatch_at_level && !edges_go_across_levels) {
                network_attr->type = MPIR_HWTOPO_NET_TYPE__CLOS_NETWORK;
                if (!count_at_level_mismatch && !bandwidth_mismatch && !out_degree_mismatch) {
                    network_attr->type = MPIR_HWTOPO_NET_TYPE__FAT_TREE;
                }
                                ...
                                MPIR_Netloc_network_attributes * network_attr)
{
    ...
    if (network_attr->type == MPIR_HWTOPO_NET_TYPE__INVALID) {
        network_attr->type = MPIR_HWTOPO_NET_TYPE__TORUS;
    ...
    if (start_node != NULL && network_attr->type != MPIR_HWTOPO_NET_TYPE__INVALID) {
