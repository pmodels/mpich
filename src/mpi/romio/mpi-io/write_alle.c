...
int MPI_File_write_all_end(MPI_File fh, ROMIO_CONST void *buf, MPI_Status * status)
{
    ...
    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_FILE_WRITE_ALL_END_T], fh, buf, status);
}
...
.N fortran
@*/
int QMPI_File_write_all_end(QMPI_Context context, int tool_id, MPI_File fh, ROMIO_CONST void *buf,
                            MPI_Status * status)
{
    int error_code;
