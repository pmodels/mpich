         ...
         * else copy into aligned buf and then use (1) */
            ...
            if (!(((long) buf) % fd->d_mem)) {
                ...
                if (newbuf) {
                    ...
                    if (err > 0)
                        memcpy(buf, newbuf, err);
                    nbytes += err;
                    ADIOI_Free(newbuf);
                } else
                    nbytes += pread(fd->fd_sys, buf, size, offset);
            }
            err = nbytes;
        }
...
void ADIOI_XFS_Aligned_Mem_File_Read(ADIO_File fd, void *buf, int len, ADIO_Offset offset, int *err)
{
    ...
    if (!(len % fd->d_miniosz) && (len >= fd->d_miniosz) && (len <= read_chunk_sz))
