...
static void ADIOI_TAM_W_Exchange_data_alltoallv(ADIO_File fd, const void *buf, char *write_buf, char *tmp_buf, int coll_bufsize,        /* 1 */
                                                ...
                                                MPI_Aint * buf_idx, int *error_code);
static void ADIOI_Fill_send_buffer(ADIO_File fd, const void *buf, ADIOI_Flatlist_node * flat_buf,
                                   char **send_buf, ADIO_Offset * offset_list,
                                   ADIO_Offset * len_list, int *send_size, MPI_Request * requests,
                                   ...
                                   ADIO_Offset * fd_end, int *send_buf_idx, int *curr_to_proc,
                                   int *done_to_proc, int iter, MPI_Aint buftype_extent);
static void ADIOI_Fill_send_buffer_nosend(ADIO_File fd, const void *buf,
                                          ADIOI_Flatlist_node * flat_buf, char **send_buf,
                                          ADIO_Offset * offset_list, ADIO_Offset * len_list,
                                          int *send_size, MPI_Request * requests, int *sent_to_proc,
                                          int nprocs, int myrank, int contig_access_count,
                                          ADIO_Offset min_st_offset, ADIO_Offset fd_size,
                                          ADIO_Offset * fd_start, ADIO_Offset * fd_end,
                                          int *send_buf_idx, int *curr_to_proc, int *done_to_proc,
                                          int iter, MPI_Aint buftype_extent);
static void ADIOI_Heap_merge(ADIOI_Access * others_req, int *count, ADIO_Offset * srt_off,
...
*/
    ...
    if (gpfsmpio_tuneblocking) {
        ADIOI_GPFS_TAM_Free_my_req(fd, nprocs, count_my_req_per_proc, my_req, buf_idx);
   ...
   completes all pending nonblocking operations before completing. */
    ...
    for (m = 0; m < ntimes; m++) {
        ...
        if (gpfsmpio_comm == 1)
            ...
            ADIOI_TAM_W_Exchange_data_alltoallv(fd, buf, write_buf, tmp_buf, coll_bufsize, flat_buf,
                                                offset_list, len_list, send_size, recv_size, off,
                                                size, count, start_pos, partial_recv, sent_to_proc,
...
static void ADIOI_TAM_Unpack(char *buf, int *recv_size, int *partial_recv,
                             ADIOI_Access * others_req, int *count, int *start_pos, int i)
{
    int j, k;
    char *to_ptr;
    ADIO_Offset tmp_len;
...
static void ADIOI_TAM_Write_Kernel(ADIO_File fd, int myrank, char *tmp_buf, char **send_buf,
                                   char *send_buf_start, int *send_size, int *recv_size,
                                   int nprocs_recv, size_t send_total_size, int sum_recv,
                                   int coll_bufsize, int *partial_recv, ADIOI_Access * others_req,
                                   int *count, int *start_pos)
{
    ...
    if (send_size[myrank]) {
        ADIOI_TAM_Unpack(send_buf[myrank], recv_size, partial_recv, others_req, count, start_pos,
                         myrank);
    }
    ...
    if (fd->is_local_aggregator) {
        ...
        for (i = 1; i < fd->nprocs_aggregator; ++i) {
            ...
            if (fd->aggregator_local_ranks[i] != myrank) {
                ...
                memcpy(fd->local_buf + fd->local_lens[i * fd->hints->cb_nodes - 1], send_buf_start,
                       (fd->local_lens[(i + 1) * fd->hints->cb_nodes - 1] -
        ...
        for (i = 0; i < fd->hints->cb_nodes; ++i) {
            ...
            if (fd->hints->ranklist[i] != myrank) {
                ...
                for (k = 0; k < fd->nprocs_aggregator; ++k) {
                    ...
                    if (k * fd->hints->cb_nodes + i) {
                        ADIOI_TAM_Unpack(fd->local_buf +
                                         fd->local_lens[k * fd->hints->cb_nodes + i - 1], recv_size,
                                         partial_recv, others_req, count, start_pos,
                                         fd->aggregator_local_ranks[k]);
                    } else {
                        ADIOI_TAM_Unpack(fd->local_buf, recv_size, partial_recv, others_req, count,
                                         start_pos, fd->aggregator_local_ranks[k]);
                    }
                }
            }
        }
            ...
            if (fd->local_aggregators[i] != myrank) {
                ...
                for (k = 0; k < fd->local_aggregator_domain_size[i]; ++k) {
                    ...
                    if (fd->local_aggregator_domain[i][k] != myrank) {
                        ADIOI_TAM_Unpack((char *) buf_ptr, recv_size, partial_recv, others_req,
                                         count, start_pos, fd->local_aggregator_domain[i][k]);
                                                ...
                                                MPI_Aint * buf_idx, int *error_code)
{
    ...
    if (buftype_is_contig) {
        for (i = 0; i < nprocs; i++) {
            if (send_size[i]) {
                memcpy(send_buf[i], buf + buf_idx[i], send_size[i]);
    ...
    ADIOI_TAM_Write_Kernel(fd, myrank, tmp_buf, send_buf, send_buf_start, send_size, recv_size,
                           nprocs_recv, send_total_size, sum_recv, coll_bufsize, partial_recv,
