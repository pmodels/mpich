                                  ...
                                  ADIO_Offset offset, ADIOI_LUSTRE_Access * my_access)
{
    ...
    if (!filetype_size) {
        ...
        my_access->offset_list[0] =
            (file_ptr_type == ADIO_INDIVIDUAL) ? fd->fp_ind : fd->disp + etype_size * offset;
    ...
    if (filetype_is_contig) {
        ...
        my_access->offset_list = (ADIO_Offset *) ADIOI_Malloc((my_access->contig_access_count + 1) *
                                                              sizeof(ADIO_Offset));
        my_access->len_list = (ADIO_Offset *) ADIOI_Malloc((my_access->contig_access_count + 1) *
                                                           sizeof(ADIO_Offset));
                                    ...
                                    ADIOI_LUSTRE_file_domain * file_domain)
{
    ...
    MPI_Allreduce(MPI_IN_PLACE, AAR_st_end_offsets, 2, ADIO_OFFSET, MPI_MAX, fd->comm);
#else
    ADIO_Offset MAR_st_end_offsets[2];
    MAR_st_end_offsets[0] = -my_access->start_offset;
    MAR_st_end_offsets[1] = my_access->end_offset;
    MPI_Allreduce(MAR_st_end_offsets, AAR_st_end_offsets, 2, ADIO_OFFSET, MPI_MAX, fd->comm);
#endif
    ...
    AAR_num_stripes = max_end_offset / striping_unit - min_st_offset / striping_unit + 1;
    ...
    for (i = 1; i < cb_nodes / cyclic_nprocs; i++) {
        for (j = 0; j < cyclic_nprocs; j++) {
            ...
            if (file_domain->num_stripes[me] > 0) {
                ...
                file_domain->starts[me] = prev_off + gap * striping_unit * cyclic_nprocs;
            }
        }
    }
    ...
    for (i = 0; i < cb_nodes; i++)
        if (file_domain->num_stripes[i] > 0) {
            ...
            file_domain->ends[i] =
                FLOOR(file_domain->starts[i],
                      striping_unit) + gap * striping_unit * cyclic_nprocs + striping_unit - 1;
        }

    last_agg = (max_end_offset / striping_unit) % cyclic_nprocs;
    num_groups = AAR_num_stripes / cyclic_nprocs;
    if (AAR_num_stripes % cyclic_nprocs == 0)
        num_groups--;
    ...
    for (i = 0; i < cb_nodes; i++) {
        ...
        if (file_domain->num_stripes[i] == 0) {
            /* some aggregators have zero-size file domains, this happens when the AAR is small */
        ...
        /* fd_size is the size of coalesced file stripes in aggregator i's file domain */
        ...
        for (j = 1; j < ntimes; j++) {
            ...
            while (rem > 0) {
                int stripe_rem = striping_unit - file_domain->offsets[i][j] % striping_unit;
                if (rem > stripe_rem) {
                    ...
                    if (rem == stripe_rem)
                        file_domain->offsets[i][j] += cyclic_gap;
        ...
        file_domain->sizes[i][ntimes - 1] =
            file_domain->ends[i] + 1 - file_domain->offsets[i][ntimes - 1];
    ...
    *my_req_ptr = (ADIOI_Access *) ADIOI_Malloc(nprocs * sizeof(ADIOI_Access));
    ...
    for (i = 0; i < nprocs; i++) {
        if (count_my_req_per_proc[i]) {
            ...
            my_req[i].mem_ptrs =
                (MPI_Aint *) ADIOI_Malloc(count_my_req_per_proc[i] * sizeof(MPI_Aint));
            count_my_req_procs++;
        }
...
#define USE_ALLTOALL
     ...
     */
#ifdef USE_ALLTOALL
    MPI_Alltoall(count_my_req_per_proc, 1, MPI_INT, count_others_req_per_proc, 1, MPI_INT,
                 fd->comm);
#else
    for (i = 0; i < nprocs; i++)
        count_others_req_per_proc[i] = 0;
    ...
    if (fd->is_agg) {
        for (i = 0; i < nprocs; i++)    /* get from proc i on how much this aggregator will receive */
            MPI_Irecv(&count_others_req_per_proc[i], 1, MPI_INT, i, i + myrank, fd->comm,
                      &requests[j++]);
    }
    ...
    for (i = 0; i < fd->hints->cb_nodes; i++) {
        ...
        MPI_Isend(&count_my_req_per_proc[aggRank], 1, MPI_INT, aggRank, aggRank + myrank, fd->comm,
                  &requests[j++]);
    }
...
#define COMBINE_OFFSETS_LENS
    ...
    for (i = 0; i < nprocs; i++) {
        if (i == myrank) {
            ...
            MPI_Irecv(others_req[i].offsets, 1, buf_type, i, i + myrank, fd->comm, &requests[j++]);
    ...
    for (i = 0; i < nprocs; i++) {
        if (i != myrank && my_req[i].count) {
            ...
            MPI_Type_create_hindexed(2, blocklengths, disps, MPI_BYTE, &buf_type);
            MPI_Type_commit(&buf_type);
            MPI_Isend(my_req[i].offsets, 1, buf_type, i, i + myrank, fd->comm, &requests[j++]);
            MPI_Type_free(&buf_type);
#else
