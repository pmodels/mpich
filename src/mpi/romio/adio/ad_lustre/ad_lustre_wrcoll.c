...
static void ADIOI_LUSTRE_W_Exchange_data(ADIO_File fd, const void *buf, char *write_buf,
                                         ...
                                         int *nprocs_recv_ptr,           /* out */
                                         MPI_Request **recv_reqs_ptr,    /* out */
                                         int *sent_to_proc, int nprocs, 
                                         int myrank,
                                         ADIOI_LUSTRE_Access *my_access, 
                                         ADIOI_Access *my_req, 
                                         ...
                                         int *done_to_proc, int iter, int max_ntimes,
                                         int *striping_info,
                                         MPI_Aint buftype_extent,
                                         ADIO_Offset **srt_off_ptr, int **srt_len_ptr, int *srt_num_ptr,
                                         int *error_code);
                                   ...
                                   ADIO_Status * status, int *error_code)
{
    ...
    ADIOI_LUSTRE_Calc_file_domains(fd, 
                                  nprocs, 
                                  myrank, 
                                  &my_access, 
                                  &file_domain); /* OUT */
    ...
    ADIOI_LUSTRE_Calc_others_req(fd, nprocs, myrank, count_my_req_per_proc,
                                 my_req, &others_req);
    ...
    *error_code = MPI_SUCCESS;  /* changed below if error */
    ...
    for(m=0; m<max_ntimes; m++){
        ...
        ADIOI_LUSTRE_W_Exchange_data(fd, buf, write_buf, flat_buf,
              send_size, recv_size_ptr,
              off, size,
              recv_count_ptr, recv_start_pos, partial_recv_size, 
              recv_types,
              ...
              &srt_off,  /* OUT: [srt_num] sorted non-contiguous requests */
              &srt_len,  /* OUT: [srt_num] */
              &srt_num,  /* OUT: */
              error_code); 
        if (*error_code != MPI_SUCCESS)
        ...
        if(m < ntimes){
            ...
            flag = ADIOI_LUSTRE_Wait_recvs(m, 
                                           ntimes,
                                           num_sub_buffers,
                                           num_reqs_to_wait,
                                           nprocs_recv_array,
            ...
            if (flag) { /* now it is time to write to the file */
                if(file_domain->pipelining == 1){
                    ADIOI_LUSTRE_Write_data_pipe(fd,  
                                                 nprocs, myrank,
                                                 m, ntimes,
                                                 num_sub_buffers,
                                                 num_reqs_to_wait,
...
static void ADIOI_LUSTRE_W_Exchange_data(ADIO_File fd, const void *buf,
                                         char *write_buf,
                                         ADIOI_Flatlist_node *flat_buf, /* only pass to ADIOI_LUSTRE_Fill_send_buffer() */
                                         int *send_size, 
                                         int *recv_size,
                                         ADIO_Offset off,
                                         ADIO_Offset size,  /* this is a non-coalesced write size */
                                         int *count, int *start_pos, int *partial_recv,
                                         MPI_Datatype *recv_types,
                                         int *nprocs_recv_ptr,           /* out */
                                         MPI_Request **recv_reqs_ptr,    /* out */
                                         int *sent_to_proc,         /* only pass to ADIOI_LUSTRE_Fill_send_buffer() */
                                         int nprocs, 
                                         int myrank,
                                         ADIOI_LUSTRE_Access *my_access, 
                                         ADIOI_Access *my_req, 
                                         ADIOI_Access *others_req, 
                                         ADIOI_LUSTRE_file_domain *file_domain,
                                         ADIO_Offset *send_buf_idx, /* only pass to ADIOI_LUSTRE_Fill_send_buffer() */
                                         int *curr_to_proc,         /* only pass to ADIOI_LUSTRE_Fill_send_buffer() */
                                         int *done_to_proc,         /* only pass to ADIOI_LUSTRE_Fill_send_buffer() */
                                         int iter, 
                                         int max_ntimes, 
                                         int *striping_info,
                                         MPI_Aint buftype_extent,   /* only pass to ADIOI_LUSTRE_Fill_send_buffer() */
                                         ADIO_Offset **srt_off_ptr,
                                         int         **srt_len_ptr,
                                         int         *srt_num_ptr,
                                         int *error_code)
{
    ...
    *nprocs_recv_ptr = nprocs_recv;
    ...
    if (nprocs_recv) { /* prepare the receiving part (aggregators only) */
        ...
        j = 0;
        for (i=1; i<sum; i++) {
            ADIO_Offset stripe_end = srt_off[j] - srt_off[j] % striping_unit
                                   + striping_unit;
            if (srt_off[i] <  stripe_end && /* check if done with this stripe */
                ...
                if (new_len > srt_len[j]) srt_len[j] = new_len;
            }
    ...
    *recv_reqs_ptr = recv_reqs;
    ...
    if (my_access->buftype_is_contig) {
        ...
        for (i=0; i<nprocs; i++) { /* send to aggregators only */
            ...
            if (send_size[i] == 0) continue;

            rem = send_size[i];
            tail = -1;
            for (k=0; k<my_req[i].count; k++) {
                if (tail == -1 && my_req[i].lens[k] > 0) tail = k;
            ...
            if (k == tail) { /* if tail==k, no need to create send type */
                ...
                MPI_Bsend(((char *) buf)+my_req[i].mem_ptrs[k],
                          my_req[i].lens[k], MPI_BYTE, i,
                          myrank+i+100*iter, fd->comm);
#else
                MPI_Isend(((char *) buf)+my_req[i].mem_ptrs[k],
                          my_req[i].lens[k], MPI_BYTE, i,
                          myrank+i+100*iter, fd->comm, send_reqs+j);
                j++;
#endif
            }
                ...
                ADIOI_Type_create_hindexed_x(k-tail+1,
                                             &(my_req[i].lens[tail]),
                                             &(my_req[i].mem_ptrs[tail]),
                                             MPI_BYTE, send_types+i);
                MPI_Type_commit(send_types+i);

#ifdef _USE_BSEND
                MPI_Bsend(buf, 1, send_types[i], i, myrank+i+100*iter,
                          fd->comm);
#else
                MPI_Isend(buf, 1, send_types[i], i, myrank+i+100*iter,
                          fd->comm, send_reqs+j);
                j++;
#endif
            }
            for (l=tail; l<k; l++) my_req[i].lens[l] = 0;
        ...
        sum = 0;
        for (i=0; i<nprocs; i++) sum += send_size[i];
        send_buf[0] = (char*) ADIOI_Malloc(sum);
        for (i=1; i<nprocs; i++) send_buf[i] = send_buf[i-1] + send_size[i-1];
                                        ...
                                        int *send_size) /* out */
{
    ...
    for (i=0; i<cb_nodes; i++) {
        if (iter >= file_domain->ntimes[i]) continue;

        int dest = fd->hints->ranklist[i];
        int dest_len = 0;
        for (k=0; k<my_req[dest].count; k++) {
            if (my_req[dest].lens[k] == 0) continue;
            if (my_req[dest].offsets[k]+my_req[dest].lens[k] < file_domain->offsets[i][iter] ||
                my_req[dest].offsets[k] >= file_domain->offsets[i][iter] + file_domain->sizes[i][iter])
                continue;

            if (my_req[dest].offsets[k] < file_domain->offsets[i][iter])
                dest_len += my_req[dest].lens[k]
                         - (file_domain->offsets[i][iter] - my_req[dest].offsets[k]);
            else
                dest_len += my_req[dest].lens[k];

            ADIO_Offset extra = (my_req[dest].offsets[k]
                               + my_req[dest].lens[k])
                              - (file_domain->offsets[i][iter]
...
/* [Sunwoo] Looks like Saba's pipelining code assumes that the size of 
 * each request is always 'stripe_size'. So, it directly uses 
 * file_domain->sizes and just perform one time of write. */
        if(write_flag == 1){
    ...
    if (file_domain->pipelining == 1) {
        ...
        if((iter >= num_reqs_to_wait) && (iter < ntimes-1)){
            if(nprocs_recv_array[curr_buf_idx] > 0){
                statuses = (MPI_Status *) ADIOI_Malloc(nprocs_recv_array[curr_buf_idx] * sizeof(MPI_Status)); 
