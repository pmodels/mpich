...
/* Invalidate an external pset in the global list (thread-safe) */
void MPIR_Pset_invalidate(char *pset_name)
{
    ...
    if (pset_find_by_name(pset_name, &p)) {
        p->is_valid = 0;        /* Set to invalid */
    }
...
int MPIR_Pset_add_defaults(MPIR_Session * session_ptr)
{
    ...
    session_ptr->psets[0].is_valid = 1;
    ...
    session_ptr->psets[1].is_valid = 1;
...
int MPIR_Pset_update_list(MPIR_Session * session_ptr)
{
    ...
    /* Iterate over global pset list and check for updates
     * Updates can be: invalidation of pset or new pset for this session */
    for (unsigned i = 0; i < utarray_len(pset_array); i++) {
        pset = (MPIR_Pset *) utarray_eltptr(pset_array, i);
        MPIR_Assert(pset != NULL);

        /* Check if session knows this pset already, append or update accordingly */
        int found = 0;
        for (int k = MPIR_PSET_NUM_DEFAULT_PSETS; k < session_ptr->num_psets; k++) {
            if (strcmp(session_ptr->psets[k].name, pset->name) == 0) {
                found = 1;
                /* update validity - all other pset params must not change */
                if (session_ptr->psets[k].is_valid != pset->is_valid) {
                    session_ptr->psets[k].is_valid = pset->is_valid;
                }
                break;
            }
        }
        if (!found && pset->is_valid) {
            /* Add new valid pset to session */
