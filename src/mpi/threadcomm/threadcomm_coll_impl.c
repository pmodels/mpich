...
int MPIR_Threadcomm_barrier_impl(MPIR_Comm * comm)
{
    ...
    if (comm->local_size == 1) {
        ...
        mpi_errno = MPIR_Barrier_intra_dissemination(comm, MPIR_SUBGROUP_NONE, MPIR_ERR_NONE);
    }
                               ...
                               int root, MPIR_Comm * comm)
{
    ...
    mpi_errno =
        MPIR_Bcast_intra_binomial(buffer, count, datatype, root, comm, MPIR_SUBGROUP_NONE,
                                  MPIR_ERR_NONE);
                                   ...
                                   MPIR_Comm * comm)
{
    ...
    mpi_errno = MPIR_Allgather_intra_brucks(sendbuf, sendcount, sendtype,
                                            recvbuf, recvcount, recvtype, comm, MPIR_SUBGROUP_NONE,
                                            MPIR_ERR_NONE);
                                        ...
                                        MPI_Op op, MPIR_Comm * comm)
{
    ...
    mpi_errno = MPIR_Reduce_scatter_intra_recursive_halving(sendbuf, recvbuf, recvcounts,
                                                            datatype, op, comm, MPIR_SUBGROUP_NONE,
                                                            MPIR_ERR_NONE);
