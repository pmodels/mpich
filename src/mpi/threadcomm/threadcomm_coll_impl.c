...
int MPIR_Threadcomm_barrier_impl(MPIR_Comm * comm)
{
    ...
    if (comm->local_size == 1) {
        ...
        mpi_errno = MPIR_Barrier_intra_dissemination(comm);
    }
                               ...
                               int root, MPIR_Comm * comm)
{
    ...
    mpi_errno = MPIR_Bcast_intra_binomial(buffer, count, datatype, root, comm);
                                ...
                                MPI_Op op, int root, MPIR_Comm * comm)
{
    ...
    mpi_errno = MPIR_Reduce_intra_binomial(sendbuf, recvbuf, count, datatype, op, root, comm);
                              ...
                              MPI_Aint count, MPI_Datatype datatype, MPI_Op op, MPIR_Comm * comm)
{
    ...
    mpi_errno = MPIR_Scan_intra_recursive_doubling(sendbuf, recvbuf, count, datatype, op, comm);
                                ...
                                MPI_Aint count, MPI_Datatype datatype, MPI_Op op, MPIR_Comm * comm)
{
    ...
    mpi_errno = MPIR_Exscan_intra_recursive_doubling(sendbuf, recvbuf, count, datatype, op, comm);
