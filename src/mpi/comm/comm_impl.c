...
/* arbitarily determin which group is the low_group by comparing
...
{
    MPIR_Lpid remote_lpid = MPIR_comm_rank_to_lpid(peer_comm_ptr, remote_leader);
    return MPIR_Intercomm_create(local_comm_ptr, local_leader, remote_lpid, tag, new_intercomm_ptr);
}

int MPIR_Intercomm_create(MPIR_Comm * local_comm_ptr, int local_leader,
                          MPIR_Lpid remote_lpid, int tag, MPIR_Comm ** new_intercomm_ptr)
{
    int mpi_errno = MPI_SUCCESS;
    int remote_size = 0;
