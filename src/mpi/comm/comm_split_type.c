...
static struct shmem_info_table shmem_info[] = {
    ...
    {"pu", HWLOC_OBJ_PU},
	{"cache:l1d",  HWLOC_OBJ_L1CACHE},
	{"cache:l1u",  HWLOC_OBJ_L1CACHE},
	{"cache:l1",  HWLOC_OBJ_L1CACHE},
	{"cache:l2d",  HWLOC_OBJ_L1CACHE},
	{"cache:l2u",  HWLOC_OBJ_L1CACHE},
	{"cache:l2",  HWLOC_OBJ_L1CACHE},
	{"cache:l3d",  HWLOC_OBJ_L1CACHE},
	{"cache:l3u",  HWLOC_OBJ_L1CACHE},
	{"cache:l3",  HWLOC_OBJ_L1CACHE},
	{"cache:l4d",  HWLOC_OBJ_L1CACHE},
	{"cache:l4u",  HWLOC_OBJ_L1CACHE},
	{"cache:l4",  HWLOC_OBJ_L1CACHE},
	{"cache:l5d",  HWLOC_OBJ_L1CACHE},
	{"cache:l5u",  HWLOC_OBJ_L1CACHE},
	{"cache:l5",  HWLOC_OBJ_L1CACHE},
	{"cache:l1i",  HWLOC_OBJ_L1ICACHE},
	{"cache:l2i",  HWLOC_OBJ_L2ICACHE},
	{"cache:l3i",  HWLOC_OBJ_L3ICACHE},
                                   ...
                                   MPIR_Info * info_ptr, MPIR_Comm ** newcomm_ptr)
{
        ...
        if (flag) {
            ...
            for (i = 0; shmem_info[i].val; i++) {
                if (!strcmp(shmem_info[i].val, hintval)) {
                    obj_type = shmem_info[i].obj_type;
                    break;
                }
            }
        }
	...
	nprocs = MPIR_Comm_size(comm_ptr);
    /* even if we did not give an info key, do an allreduce since
     * other processes might have given an info key */
    mpi_errno =
        MPID_Allreduce(&obj_type, &obj_type_global, MPI_INT, MPI_INT, MPI_LXOR, comm_ptr,
                       &errflag);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);


    info_args_are_equal = ((nprocs & 1)&& obj_type == obj_type_global)||((nprocs & 1 == 0) && !obj_type_global);
