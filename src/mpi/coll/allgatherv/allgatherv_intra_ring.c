                               ...
                               MPI_Datatype recvtype, MPIR_Comm * comm_ptr, int coll_attr)
{
    ...
    while (tosend || torecv) {  /* While we have data to send or receive */
        ...
        if (!sendnow && !recvnow) {
            ...
            mpi_errno = MPIC_Recv(rbuf, recvnow, recvtype, left, MPIR_ALLGATHERV_TAG, comm_ptr,
                                  coll_attr, &status);
            ...
            mpi_errno =
                MPIC_Send(sbuf, sendnow, recvtype, right, MPIR_ALLGATHERV_TAG, comm_ptr, coll_attr);
            MPIR_ERR_COLL_CHECKANDCONT(mpi_errno, coll_attr, mpi_errno_ret);
            tosend -= sendnow;
