                                             ...
                                             MPIR_Comm * comm_ptr, int coll_attr)
{
    ...
    MPI_Aint curr_cnt, last_recv_cnt, total_count;
    int dst;
    void *tmp_buf;
    ...
    while (mask < comm_size) {
        ...
        if (dst < comm_size) {
            ...
            mpi_errno = MPIC_Sendrecv(((char *) tmp_buf + send_offset * recvtype_sz),
                                      ...
                                      MPIR_ALLGATHERV_TAG, comm_ptr, &status, coll_attr);
            ...
            while (tmp_mask) {
                ...
                if ((dst > rank) && (rank < tree_root + nprocs_completed)
                    ...
                    mpi_errno = MPIC_Send(((char *) tmp_buf + offset * recvtype_sz),
                                          ...
                                          MPI_BYTE, dst, MPIR_ALLGATHERV_TAG, comm_ptr, coll_attr);
                    MPIR_ERR_COLL_CHECKANDCONT(mpi_errno, coll_attr, mpi_errno_ret);
                 ...
                 * has data */
                    ...
                    mpi_errno = MPIC_Recv(((char *) tmp_buf + offset * recvtype_sz),
                                          ...
                                          dst, MPIR_ALLGATHERV_TAG, comm_ptr, coll_attr, &status);
                    MPIR_ERR_COLL_CHECKANDCONT(mpi_errno, coll_attr, mpi_errno_ret);
                    if (mpi_errno) {
                        last_recv_cnt = 0;
                    } else
