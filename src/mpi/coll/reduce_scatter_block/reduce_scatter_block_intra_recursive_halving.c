    ...
    if (rank < 2 * rem) {
        if (rank % 2 == 0) {    /* even */
            mpi_errno = MPIC_Send(tmp_results, total_count,
                                  datatype, rank + 1,
                                  MPIR_REDUCE_SCATTER_BLOCK_TAG, comm_ptr, coll_group, errflag);
            MPIR_ERR_CHECK(mpi_errno);
            ...
            mpi_errno = MPIC_Recv(tmp_recvbuf, total_count,
                                  datatype, rank - 1,
                                  MPIR_REDUCE_SCATTER_BLOCK_TAG, comm_ptr, coll_group,
                                  MPI_STATUS_IGNORE);
            MPIR_ERR_CHECK(mpi_errno);
...
*/
            ...
            if ((send_cnt != 0) && (recv_cnt != 0))
                ...
                mpi_errno = MPIC_Send((char *) tmp_results +
                                      newdisps[send_idx] * extent,
                                      send_cnt, datatype,
                                      dst, MPIR_REDUCE_SCATTER_BLOCK_TAG, comm_ptr, coll_group,
                                      errflag);
