                                        ...
                                        MPIR_Sched_element_t s)
{
    ...
    while (pof2 <= comm_size / 2) {
        ...
        mpi_errno = MPIR_Sched_element_send(tmp_buf, curr_count, recvtype, dst, comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        /* sendrecv, no barrier */
        mpi_errno = MPIR_Sched_element_recv(((char *) tmp_buf + curr_count * recvtype_extent),
    ...
    if (rem) {
        ...
        mpi_errno = MPIR_Sched_element_send(tmp_buf, send_cnt, recvtype, dst, comm_ptr, s);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
        /* sendrecv, no barrier */
        mpi_errno = MPIR_Sched_element_recv(((char *) tmp_buf + curr_count * recvtype_extent),
    ...
    for (i = 0; i < (comm_size - rank); i++) {
        j = (rank + i) % comm_size;
        mpi_errno = MPIR_Sched_element_copy(((char *) tmp_buf + send_cnt * recvtype_extent),
