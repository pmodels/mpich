    ...
    if (rank < 2 * rem) {
        if (rank % 2 == 0) {    /* even */
            mpi_errno = MPIC_Send(tmp_results, total_count,
                                  datatype, rank + 1, MPIR_REDUCE_SCATTER_TAG, comm_ptr, coll_attr);
            ...
            mpi_errno = MPIC_Recv(tmp_recvbuf, total_count,
                                  datatype, rank - 1,
                                  MPIR_REDUCE_SCATTER_TAG, comm_ptr, coll_attr, MPI_STATUS_IGNORE);
            MPIR_ERR_COLL_CHECKANDCONT(mpi_errno, coll_attr, mpi_errno_ret);
        ...
        if (rank % 2) { /* odd */
            if (recvcounts[rank - 1]) {
                mpi_errno = MPIC_Send((char *) tmp_results +
                                      ...
                                      datatype, rank - 1,
                                      MPIR_REDUCE_SCATTER_TAG, comm_ptr, coll_attr);
                MPIR_ERR_COLL_CHECKANDCONT(mpi_errno, coll_attr, mpi_errno_ret);
            }
        } else {        /* even */
            if (recvcounts[rank]) {
