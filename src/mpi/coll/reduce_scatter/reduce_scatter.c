                              ...
                              MPI_Datatype datatype, MPI_Op op, MPIR_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (is_commutative && (nbytes >= MPIR_CVAR_REDSCAT_COMMUTATIVE_LONG_MSG_SIZE)) {
        ...
        mpi_errno = MPIR_Reduce_scatter_intra_pairwise (sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr, errflag);
        if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    ...
    if (!is_commutative) {
        ...
        if (pof2 == comm_size && is_block_regular) {
            ...
            mpi_errno = MPIR_Reduce_scatter_intra_noncomm(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr, errflag);
            if (mpi_errno) {
                              ...
                              MPIR_Errflag_t *errflag)
{
    ...
    MPIR_Reduce_scatter_inter_generic(sendbuf, recvbuf, recvcounts, datatype,
            op, comm_ptr, errflag);
                        ...
                        MPI_Datatype datatype, MPI_Op op, MPIR_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        ...
        switch (MPIR_Reduce_scatter_intra_algo_choice) {
            case MPIR_REDUCE_SCATTER_INTRA_ALGO_NONCOMM:
                mpi_errno = MPIR_Reduce_scatter_intra_noncomm(sendbuf, recvbuf,
        ...
        /* intercommunicator */
        switch (MPIR_Reduce_scatter_intra_algo_choice) {
            case MPIR_REDUCE_SCATTER_INTER_ALGO_GENERIC:
                mpi_errno = MPIR_Reduce_scatter_inter_generic(sendbuf, recvbuf, recvcounts,
