...
cvars:
    - name        : MPIR_CVAR_USE_GATHERV
      ...
      description : >-
        Gatherv algorithm number to use

=== END_MPI_T_CVAR_INFO_BLOCK ===
*/

enum {
    MPIC_DEFAULT_GATHERV = 0
};
        ...
        MPIR_Errflag_t *errflag )
{
    ...
    int use_coll = MPIR_CVAR_USE_GATHERV;
    if(comm_ptr->comm_kind == MPIR_COMM_KIND__INTERCOMM)
        use_coll = MPIC_DEFAULT_GATHERV;

    switch (use_coll) {
        case MPIC_DEFAULT_GATHERV:
        default:
            mpi_errno = MPIC_DEFAULT_Gatherv(sendbuf,sendcount, sendtype,
