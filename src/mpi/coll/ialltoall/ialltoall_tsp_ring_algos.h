                                        ...
                                        MPIR_Comm * comm, MPIR_TSP_sched_t * sched)
{
    ...
    MPL_DBG_MSG_FMT(MPIR_DBG_COLL, VERBOSE,
                    (MPL_DBG_FDEST,
                     "rank =%d, data_buf=%04x, sendbuf=%04x, buf1=%04x, buf2=%04x, recvbuf=%04x",
                     rank, data_buf, sendbuf, buf1, buf2, recvbuf));

    if (!is_inplace) {  /* copy my part of my sendbuf to recv_buf */
        MPL_DBG_MSG_FMT(MPIR_DBG_COLL, VERBOSE,
                        (MPL_DBG_FDEST,
                         "copying my part from sendbuf address=%04x to my recvbuf address=%04x",
                         sendbuf + rank * sendcount * sendtype_extent,
                         recvbuf + rank * recvcount * recvtype_extent));
    ...
    for (i = 0; i < size - 1; i++) {
        ...
        MPL_DBG_MSG_FMT(MPIR_DBG_COLL, VERBOSE,
                        (MPL_DBG_FDEST, "copying from location=%04x to location=%04x",
