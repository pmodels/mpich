...
{
    size_t type_size, type_extent, type_lb, type_true_extent;
    ...
    MPIR_Datatype_get_size_macro(rtype, type_size);
    ...
    *pupsize = 0;       /* points to the first empty location in pupbuf */
    while (offset < comm_size) {
        if (pack) {
            dtcopy_id[counter++] =
                MPIR_TSP_sched_localcopy(rbuf + offset * count * type_extent, count, rtype,
                                         pupbuf + *pupsize, count, rtype, sched, ninvtcs, invtcs);
            MPL_DBG_MSG_FMT(MPIR_DBG_COLL, VERBOSE,
                            (MPL_DBG_FDEST, "packing rbuf+%d to pupbuf+%d\n",
                             offset * count * type_extent, *pupsize));
        } else {
            dtcopy_id[counter++] =
                MPIR_TSP_sched_localcopy(pupbuf + *pupsize, count, rtype,
                                         rbuf + offset * count * type_extent, count, rtype,
                                         sched, ninvtcs, invtcs);
            MPL_DBG_MSG_FMT(MPIR_DBG_COLL, VERBOSE,
                            (MPL_DBG_FDEST, "unpacking from pupbuf+%d to rbuf+%d\n", *pupsize,
                                      ...
                                      int buffer_per_phase)
{
    ...
    pack_invtcs = (int *) MPL_malloc(sizeof(int) * k, MPL_MEM_COLL);
    recv_invtcs = (int *) MPL_malloc(sizeof(int) * k, MPL_MEM_COLL);
    unpack_invtcs = (int *) MPL_malloc(sizeof(int) * k, MPL_MEM_COLL);
    invtcs = (int *) MPL_malloc(sizeof(int) * 6 * k, MPL_MEM_COLL);
    ...
    MPIR_Datatype_get_size_macro(recvtype, r_type_size);
    MPIR_Datatype_get_extent_macro(recvtype, r_extent);
    MPIR_Type_get_true_extent_impl(recvtype, &r_lb, &r_extent);
    r_extent = MPL_MAX(r_extent, r_true_extent);

    MPL_DBG_MSG_FMT(MPIR_DBG_COLL, VERBOSE,
                    (MPL_DBG_FDEST, "send_type_size: %d, send_type_extent: %d, send_count: %d\n",
    ...
    delta = 1;
    tmp_sbuf = (void ***) MPL_malloc(sizeof(void **) * nphases, MPL_MEM_COLL);
    tmp_rbuf = (void ***) MPL_malloc(sizeof(void **) * nphases, MPL_MEM_COLL);
    ...
    for (i = 0; i < size; i++)
        MPIR_TSP_sched_localcopy(tmp_buf + i * recvcount * r_extent, recvcount, recvtype,
                                 (void *) ((char *) recvbuf +
  ...
  fn_exit:
    ...
    MPL_free(tmp_rbuf);
    MPL_free(invtcs);
    MPL_free(pack_invtcs);
    MPL_free(unpack_invtcs);
    MPL_free(recv_invtcs);
