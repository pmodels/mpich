                                                   ...
                                                   MPIR_Comm * comm_ptr, int coll_attr)
{
    ...
    if (sendcount != 0) {
        mpi_errno = MPIR_Gather_auto(sendbuf, sendcount, sendtype, tmp_buf, sendcount * sendtype_sz,
                                     MPIR_BYTE_INTERNAL, 0, newcomm_ptr, MPIR_CSEL_ENTRY__AUTO);
    ...
    if (comm_ptr->is_low_group) {
        ...
        if (sendcount != 0) {
            ...
            mpi_errno = MPIR_Bcast_auto(tmp_buf, sendcount * sendtype_sz * local_size,
                                        MPIR_BYTE_INTERNAL, root, comm_ptr, MPIR_CSEL_ENTRY__AUTO);
            MPIR_ERR_CHECK(mpi_errno);
        }
        ...
        if (recvcount != 0) {
            root = 0;
            mpi_errno = MPIR_Bcast_auto(recvbuf, recvcount * remote_size,
                                        recvtype, root, comm_ptr, MPIR_CSEL_ENTRY__AUTO);
            MPIR_ERR_CHECK(mpi_errno);
        }
    } else {
