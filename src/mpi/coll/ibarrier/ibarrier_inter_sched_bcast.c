...
int MPIR_Ibarrier_inter_sched_bcast(MPIR_Comm * comm_ptr, int coll_group, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->local_size != 1) {
        mpi_errno = MPIR_Ibarrier_intra_sched_auto(comm_ptr->local_comm, coll_group, s);
        MPIR_ERR_CHECK(mpi_errno);
    ...
    if (comm_ptr->is_low_group) {
        ...
        mpi_errno = MPIR_Ibcast_inter_sched_auto(buf, 1, MPI_BYTE, root, comm_ptr, coll_group, s);
        MPIR_ERR_CHECK(mpi_errno);
