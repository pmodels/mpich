                                                                         ...
                                                                         MPIR_TSP_sched_t sched)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL
        (MPID_STATE_MPIR_TSP_IALLREDUCE_SCHED_INTRA_SINGLE_BUFFER_WITHOUT_DTCOPY_RECEXCH);
    ...
    if (!in_step2) {
        ...
        for (i = 0; i < step1_nrecvs; i++) {    /* participating rank gets data from non-partcipating ranks */
            ...
            recv_id = MPIR_TSP_sched_irecv(step1_recvbuf, count, datatype,
            ...
            reduce_id = MPIR_TSP_sched_reduce_local(step1_recvbuf, recvbuf, count,
                                                                           ...
                                                                           MPIR_TSP_sched_t sched)
{
    ...
    MPIR_FUNC_VERBOSE_STATE_DECL
        (MPID_STATE_MPIR_TSP_IALLREDUCE_SCHED_INTRA_MULTIPLE_BUFFER_WITHOUT_DTCOPY_RECEXCH);
    MPIR_FUNC_VERBOSE_ENTER
        (MPID_STATE_MPIR_TSP_IALLREDUCE_SCHED_INTRA_MULTIPLE_BUFFER_WITHOUT_DTCOPY_RECEXCH);
    ...
    for (phase = 0; phase < step2_nphases && step1_sendto == -1; phase++) {
        ...
        for (i = 0; i < k - 1; i++) {
            if (phase == 0) {
                ...
                MPL_DBG_MSG_FMT(MPIR_DBG_COLL, VERBOSE,
                                (MPL_DBG_FDEST, "Depend on all reduces (%d) from prev phase",
                                 nvtcs));
