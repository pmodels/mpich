...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Gather_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Gather_intra(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Gather_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Gather_inter(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                              void *recvbuf, int recvcount, MPI_Datatype recvtype,
                              int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (nbytes < MPIR_CVAR_GATHER_INTER_SHORT_MSG_SIZE) {
        if (root == MPI_ROOT) {
            ...
            mpi_errno = MPIC_Recv(recvbuf, recvcount * remote_size,
                                  recvtype, 0, MPIR_GATHER_TAG, comm_ptr, &status, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
                MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
            ...
            mpi_errno = MPID_Gather(sendbuf, sendcount, sendtype,
                                    tmp_buf, sendcount, sendtype, 0, newcomm_ptr, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
                MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
            ...
            if (rank == 0) {
                mpi_errno = MPIC_Send(tmp_buf, sendcount * local_size,
                                      sendtype, root, MPIR_GATHER_TAG, comm_ptr, errflag);
                if (mpi_errno) {
                    /* for communication errors, just record the error but continue */
                    *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
                    MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Gather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Gather(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                        void *recvbuf, int recvcount, MPI_Datatype recvtype,
                        int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        mpi_errno = MPIC_DEFAULT_Gather_intra(sendbuf, sendcount, sendtype,
        ...
        /* intercommunicator */
        mpi_errno = MPIC_DEFAULT_Gather_inter(sendbuf, sendcount, sendtype,
                                              recvbuf, recvcount, recvtype, root,
                                              comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
