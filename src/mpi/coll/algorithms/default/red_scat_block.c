 ...
 * the power-of-2 case. */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_scatter_block_noncomm
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce_scatter_block_noncomm(const void *sendbuf,
                                              void *recvbuf,
                                              int recvcount,
                                              ...
                                              MPIR_Errflag_t * errflag)
{
    ...
    for (k = 0; k < log2_comm_size; ++k) {
        ...
        if (rank > peer) {
            ...
            mpi_errno = MPIR_Reduce_local_impl(outgoing_data + recv_offset * true_extent,
                                               incoming_data + recv_offset * true_extent,
                                               size, datatype, op);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_scatter_block_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)

/* not declared static because a machine-specific function may call this one in some cases */
int MPIC_DEFAULT_Reduce_scatter_block_intra(const void *sendbuf,
                                            void *recvbuf,
                                            int recvcount,
                                            MPI_Datatype datatype,
                                            MPI_Op op,
...
*/
            ...
            if (is_commutative || (src < rank)) {
                if (sendbuf != MPI_IN_PLACE) {
                    ...
                    mpi_errno =
                        MPIR_Reduce_local_impl(tmp_recvbuf,
                                               ((char *) recvbuf + disps[rank] * extent), recvcount,
                                               datatype, op);
                ...
                if (sendbuf != MPI_IN_PLACE) {
                    ...
                    mpi_errno = MPIR_Localcopy(tmp_recvbuf, recvcount,
                                               datatype, recvbuf, recvcount, datatype);
                    if (mpi_errno)
                        MPIR_ERR_POP(mpi_errno);
                } else {
                    ...
                    mpi_errno = MPIR_Localcopy(tmp_recvbuf, recvcount,
                                               datatype,
                                               ((char *) recvbuf +
                                                disps[rank] * extent), recvcount, datatype);
                    if (mpi_errno)
                        MPIR_ERR_POP(mpi_errno);
                }
            }
    ...
    if (!is_commutative) {
        ...
        if (!(comm_size & (comm_size - 1))) {
            ...
            mpi_errno =
                MPIC_DEFAULT_Reduce_scatter_block_noncomm(sendbuf, recvbuf, recvcount, datatype, op,
                                                          comm_ptr, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
            ...
            while (mask < comm_size) {
                ...
                if (received) {
                    if (is_commutative || (dst_tree_root < my_tree_root)) {
                        ...
                        mpi_errno = MPIR_Reduce_local_impl(((char *) tmp_recvbuf + dis[1] * extent),
                        ...
                        mpi_errno = MPIR_Reduce_local_impl(((char *) tmp_results + dis[1] * extent),
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_scatter_block_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
/* not declared static because a machine-specific function may call this one in some cases */
int MPIC_DEFAULT_Reduce_scatter_block_inter(const void *sendbuf,
                                            void *recvbuf,
                                            int recvcount,
                                            MPI_Datatype datatype,
                                            MPI_Op op,
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEAFULT_Reduce_scatter_block
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce_scatter_block(const void *sendbuf, void *recvbuf,
                                      ...
                                      MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        mpi_errno =
            MPIC_DEFAULT_Reduce_scatter_block_intra(sendbuf, recvbuf, recvcount, datatype, op,
                                                    comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {
        /* intercommunicator */
        mpi_errno =
            MPIC_DEFAULT_Reduce_scatter_block_inter(sendbuf, recvbuf, recvcount, datatype, op,
                                                    comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
