...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_scatter_noncomm
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce_scatter_noncomm(const void *sendbuf, void *recvbuf,
                                        ...
                                        MPIR_Errflag_t * errflag)
{
    ...
    for (i = 0; i < comm_size; ++i) {
        mpi_errno =
            MPIR_Localcopy((char *) (sendbuf ==
                           ...
                           (char *) tmp_buf0 +
                           (MPIU_Mirror_permutation(i, log2_comm_size) * true_extent * block_size),
                           block_size, datatype);
        if (mpi_errno)
    ...
    for (k = 0; k < log2_comm_size; ++k) {
        ...
        if (rank > peer) {
            ...
            mpi_errno = MPIR_Reduce_local_impl(incoming_data + recv_offset * true_extent,
            ...
            MPIR_Reduce_local_impl(outgoing_data + recv_offset * true_extent,
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_scatter_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)

/* not declared static because a machine-specific function may call this one in some cases */
int MPIC_DEFAULT_Reduce_scatter_intra(const void *sendbuf, void *recvbuf,
                                      const int recvcounts[], MPI_Datatype datatype,
                                      MPI_Op op, MPIR_Comm * comm_ptr,
                                      MPIR_Errflag_t * errflag)
{
    ...
    if ((is_commutative) && (nbytes < MPIR_CVAR_REDSCAT_COMMUTATIVE_LONG_MSG_SIZE)) {
        ...
        if (rank < 2 * rem) {
            if (rank % 2 == 0) {        /* even */
                ...
                mpi_errno =
                    MPIR_Reduce_local_impl(tmp_recvbuf, tmp_results, total_count, datatype, op);
...
*/
                ...
                if (recv_cnt) {
                    mpi_errno = MPIR_Reduce_local_impl((char *) tmp_recvbuf +
                                                       newdisps[recv_idx] * extent,
                                                       (char *) tmp_results +
                                                       newdisps[recv_idx] * extent, recv_cnt,
                                                       datatype, op);
                }
    ...
    if (is_commutative && (nbytes >= MPIR_CVAR_REDSCAT_COMMUTATIVE_LONG_MSG_SIZE)) {
        ...
        for (i = 1; i < comm_size; i++) {
            ...
            if (is_commutative || (src < rank)) {
                if (sendbuf != MPI_IN_PLACE) {
                    mpi_errno = MPIR_Reduce_local_impl(tmp_recvbuf, recvbuf, recvcounts[rank],
                                                       datatype, op);
                } else {
                    mpi_errno =
                        MPIR_Reduce_local_impl(tmp_recvbuf,
                ...
                if (sendbuf != MPI_IN_PLACE) {
                    mpi_errno =
                        MPIR_Reduce_local_impl(recvbuf, tmp_recvbuf, recvcounts[rank], datatype,
                                               op);
                    ...
                    mpi_errno = MPIR_Reduce_local_impl(((char *) recvbuf + disps[rank] * extent),
                                                       tmp_recvbuf, recvcounts[rank], datatype, op);
                    /* copy result back into recvbuf */
                    mpi_errno = MPIR_Localcopy(tmp_recvbuf, recvcounts[rank],
                                               datatype,
                                               ((char *) recvbuf +
                                                disps[rank] * extent), recvcounts[rank], datatype);
                    if (mpi_errno)
                        MPIR_ERR_POP(mpi_errno);
                }
            }
        ...
        if ((sendbuf == MPI_IN_PLACE) && (rank != 0)) {
            mpi_errno = MPIR_Localcopy(((char *) recvbuf +
                                        disps[rank] * extent),
                                       recvcounts[rank], datatype,
                                       recvbuf, recvcounts[rank], datatype);
            if (mpi_errno)
                MPIR_ERR_POP(mpi_errno);
        }
    }
    ...
    if (!is_commutative) {
        ...
        if (pof2 == comm_size && is_block_regular) {
            ...
            mpi_errno =
                MPIC_DEFAULT_Reduce_scatter_noncomm(sendbuf, recvbuf, recvcounts, datatype, op,
                                                    comm_ptr, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
            ...
            while (mask < comm_size) {
                ...
                if (received) {
                    if (is_commutative || (dst_tree_root < my_tree_root)) {
                        {
                            mpi_errno = MPIR_Reduce_local_impl(tmp_recvbuf, tmp_results, blklens[0],
                                                               datatype, op);
                            if (mpi_errno)
                                MPIR_ERR_POP(mpi_errno);
                        ...
                        {
                            mpi_errno = MPIR_Reduce_local_impl(tmp_results, tmp_recvbuf, blklens[0],
                                                               datatype, op);
                            if (mpi_errno)
                                MPIR_ERR_POP(mpi_errno);

                            mpi_errno = MPIR_Reduce_local_impl(((char *) tmp_results +
                                                                dis[1] * extent),
                                                               ((char *) tmp_recvbuf +
                                                                dis[1] * extent), blklens[1],
                                                               datatype, op);
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_scatter_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)

/* not declared static because a machine-specific function may call this one in some cases */
int MPIC_DEFAULT_Reduce_scatter_inter(const void *sendbuf, void *recvbuf,
                                      const int recvcounts[], MPI_Datatype datatype,
                                      MPI_Op op, MPIR_Comm * comm_ptr,
                                      MPIR_Errflag_t * errflag)
{
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEAFULT_Reduce_scatter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce_scatter(const void *sendbuf, void *recvbuf,
                                            const int recvcounts[], MPI_Datatype datatype,
                                            MPI_Op op, MPIR_Comm * comm_ptr,
                                            MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        mpi_errno = MPIC_DEFAULT_Reduce_scatter_intra(sendbuf, recvbuf, recvcounts,
                                                      datatype, op, comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {
        /* intercommunicator */
        mpi_errno = MPIC_DEFAULT_Reduce_scatter_inter(sendbuf, recvbuf, recvcounts,
                                                      datatype, op, comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
