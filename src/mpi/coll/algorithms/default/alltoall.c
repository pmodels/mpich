...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Alltoall_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Alltoall_intra(const void *sendbuf,
                                int sendcount,
...
/* not declared static because a machine-specific function may call this one in some cases */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Alltoall_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Alltoall_inter(const void *sendbuf,
                                int sendcount,
                                MPI_Datatype sendtype,
                                void *recvbuf,
                                int recvcount,
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Alltoall
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Alltoall(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                          ...
                          MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        mpi_errno = MPIC_DEFAULT_Alltoall_intra(sendbuf, sendcount, sendtype,
                                                recvbuf, recvcount, recvtype, comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {
        /* intercommunicator */
        mpi_errno = MPIC_DEFAULT_Alltoall_inter(sendbuf, sendcount, sendtype,
                                                recvbuf, recvcount, recvtype, comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
