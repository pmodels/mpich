...
static int sched_test_length(MPIR_Comm * comm, int tag, void *state)
{
    ...
    int recv_size;
    struct  MPIC_Ibcast_status *status = (struct MPIC_Ibcast_status*) state;
 ...
 * function. */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Ibcast_binomial
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Ibcast_binomial(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
    int mask;
    int comm_size, rank;
    ...
    int relative_rank;
    int src, dst;
    struct MPIC_Ibcast_status *status;
    void *tmp_buf = NULL;
    ...
    if (comm_ptr->is_hetero)
        is_homogeneous = 0;
#endif
    MPIR_SCHED_CHKPMEM_MALLOC(status, struct MPIC_Ibcast_status *,
                              sizeof(struct MPIC_Ibcast_status), mpi_errno, "MPI_Stauts");
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Iscatter_for_bcast
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Iscatter_for_bcast(void *tmp_buf, int root, MPIR_Comm *comm_ptr, int nbytes, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
 ...
 */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Ibcast_scatter_rec_dbl_allgather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Ibcast_scatter_rec_dbl_allgather(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    MPIR_SCHED_CHKPMEM_MALLOC(status, struct MPIC_Ibcast_status*,
                              sizeof(struct MPIC_Ibcast_status), mpi_errno, "MPI_Status");
    is_homogeneous = 1;
#ifdef MPID_HAS_HETERO
    if (comm_ptr->is_hetero)
        is_homogeneous = 0;
    ...
    mpi_errno = MPIC_DEFAULT_Iscatter_for_bcast(tmp_buf, root, comm_ptr, nbytes, s);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
...
*/
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Ibcast_scatter_ring_allgather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Ibcast_scatter_ring_allgather(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
    int comm_size, rank;
 ...
 * subcommunicator. */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Ibcast_SMP
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Ibcast_SMP(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
    int is_homogeneous;
    MPI_Aint type_size;
    ...
    {
        mpi_errno = MPIC_DEFAULT_Ibcast_sched(buffer, count, datatype,
    ...
    if (comm_ptr->node_comm != NULL)
    {
        mpi_errno = MPIC_DEFAULT_Ibcast_sched(buffer, count, datatype, 0, comm_ptr->node_comm, s);
        if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    }
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Ibcast_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Ibcast_intra(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    {
        mpi_errno = MPIC_DEFAULT_Ibcast_binomial(buffer, count, datatype, root, comm_ptr, s);
        if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    }
    else /* (nbytes >= MPIR_CVAR_BCAST_SHORT_MSG_SIZE) && (comm_size >= MPIR_CVAR_BCAST_MIN_PROCS) */
    {
        if ((nbytes < MPIR_CVAR_BCAST_LONG_MSG_SIZE) && (MPIU_is_pof2(comm_size, NULL))) {
            mpi_errno = MPIC_DEFAULT_Ibcast_scatter_rec_dbl_allgather(buffer, count, datatype, root, comm_ptr, s);
            if (mpi_errno) MPIR_ERR_POP(mpi_errno);
        }
        else {
            mpi_errno = MPIC_DEFAULT_Ibcast_scatter_ring_allgather(buffer, count, datatype, root, comm_ptr, s);
            if (mpi_errno) MPIR_ERR_POP(mpi_errno);
        }
    }
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Ibcast_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Ibcast_inter(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    else
    {
        ...
        /* now do the usual broadcast on this intracommunicator
           with rank 0 as root. */
        mpi_errno = MPIC_DEFAULT_Ibcast_sched(buffer, count, datatype, root, comm_ptr->local_comm, s);
        if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    }
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Ibcast_sched
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Ibcast_sched(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        if (comm_ptr->hierarchy_kind == MPIR_COMM_HIERARCHY_KIND__PARENT) {
            mpi_errno = MPIC_DEFAULT_Ibcast_SMP(buffer, count, datatype, root, comm_ptr, s);
        } else {
            mpi_errno = MPIC_DEFAULT_Ibcast_intra(buffer, count, datatype, root, comm_ptr, s);
        }
    } else {
        mpi_errno = MPIC_DEFAULT_Ibcast_inter(buffer, count, datatype, root, comm_ptr, s);
    }
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Ibcast
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Ibcast(void *buffer, int count, MPI_Datatype datatype, int root, MPIR_Comm *comm_ptr, MPI_Request *request)
{
    ...
    *request = MPI_REQUEST_NULL;
    ...
    mpi_errno = MPIC_DEFAULT_Ibcast_sched(buffer, count, datatype, root, comm_ptr, s);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
