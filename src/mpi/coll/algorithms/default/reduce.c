 ...
 */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_binomial
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce_binomial(const void *sendbuf,
                                             void *recvbuf,
                                             int count,
                                             ...
                                             MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (rank != root) {
        MPIR_CHKLMEM_MALLOC(recvbuf, void *,
                            count * (MPL_MAX(extent, true_extent)), mpi_errno, "receive buffer");
...
*/
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_redscat_gather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce_redscat_gather(const void *sendbuf,
                                       void *recvbuf,
                                       int count,
                                       MPI_Datatype datatype,
                                       MPI_Op op,
...
/* not declared static because a machine-specific function may call this one
   in some cases */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce_intra(const void *sendbuf,
                              void *recvbuf,
                              int count,
                              MPI_Datatype datatype,
                              MPI_Op op,
                              int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if ((count * type_size > MPIR_CVAR_REDUCE_SHORT_MSG_SIZE) &&
        ...
        mpi_errno =
            MPIC_DEFAULT_Reduce_redscat_gather(sendbuf, recvbuf, count, datatype, op, root,
                                               comm_ptr, errflag);
        if (mpi_errno) {
            /* for communication errors, just record the error but continue */
            *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
        ...
        mpi_errno =
            MPIC_DEFAULT_Reduce_binomial(sendbuf, recvbuf, count, datatype, op, root, comm_ptr,
                                         errflag);
        if (mpi_errno) {
            /* for communication errors, just record the error but continue */
            *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce_inter(const void *sendbuf,
                              void *recvbuf,
                              int count,
                              MPI_Datatype datatype,
                              MPI_Op op,
...
*/
    ...
    if (root == MPI_ROOT) {
        ...
        mpi_errno = MPIC_DEFAULT_Reduce_intra(sendbuf, tmp_buf, count, datatype,
...
/* MPIR_Reduce performs an reduce using point-to-point messages.
   ...
   used. */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Reduce
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Reduce(const void *sendbuf, void *recvbuf, int count,
                        ...
                        MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        mpi_errno = MPIC_DEFAULT_Reduce_intra(sendbuf, recvbuf, count, datatype,
                                              op, root, comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    } else {
        /* intercommunicator */
        mpi_errno = MPIC_DEFAULT_Reduce_inter(sendbuf, recvbuf, count, datatype,
                                              op, root, comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
