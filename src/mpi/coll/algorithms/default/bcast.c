...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Bcast_binomial
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Bcast_binomial(void *buffer,
                                int count,
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int scatter_for_bcast(void *buffer ATTRIBUTE((unused)),
...
*/
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Bcast_scatter_doubling_allgather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Bcast_scatter_doubling_allgather(void *buffer,
                                                  int count,
                                                  MPI_Datatype datatype,
                                                  int root,
                                                  MPIR_Comm * comm_ptr,
...
*/
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Bcast_scatter_ring_allgather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Bcast_scatter_ring_allgather(void *buffer,
                                              int count,
                                              MPI_Datatype datatype,
                                              int root,
                                              MPIR_Comm * comm_ptr,
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_SMP_Bcast
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_SMP_Bcast(void *buffer,
                           int count,
                           MPI_Datatype datatype,
                           int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if ((nbytes < MPIR_CVAR_BCAST_SHORT_MSG_SIZE) ||
        ...
        if (comm_ptr->node_roots_comm != NULL) {
            MPIR_Bcast_fn_or_override(MPIC_DEFAULT_Bcast_binomial, mpi_errno_ret,
        ...
        if (nbytes < MPIR_CVAR_BCAST_LONG_MSG_SIZE && MPIU_is_pof2(comm_ptr->local_size, NULL)) {
            ...
            if (comm_ptr->node_roots_comm != NULL) {
                if (MPIU_is_pof2(comm_ptr->node_roots_comm->local_size, NULL)) {
                    MPIR_Bcast_fn_or_override(MPIC_DEFAULT_Bcast_scatter_doubling_allgather,
                                              mpi_errno_ret, buffer, count, datatype,
                                              MPIR_Get_internode_rank(comm_ptr, root),
                                              comm_ptr->node_roots_comm, errflag);
                } else {
                    MPIR_Bcast_fn_or_override(MPIC_DEFAULT_Bcast_scatter_ring_allgather,
                                              mpi_errno_ret, buffer, count, datatype,
                                              MPIR_Get_internode_rank(comm_ptr, root),
                                              comm_ptr->node_roots_comm, errflag);
                }
             ...
             * communication. */
            mpi_errno =
                MPIC_DEFAULT_Bcast_scatter_ring_allgather(buffer, count, datatype, root, comm_ptr,
                                                          errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Bcast_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Bcast_intra(void *buffer,
                             int count,
                             MPI_Datatype datatype,
                             int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (MPIR_CVAR_ENABLE_SMP_COLLECTIVES && MPIR_CVAR_ENABLE_SMP_BCAST &&
        ...
        mpi_errno = MPIC_DEFAULT_SMP_Bcast(buffer, count, datatype, root, comm_ptr, errflag);
        if (mpi_errno) {
            /* for communication errors, just record the error but continue */
            *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
            MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
    ...
    if ((nbytes < MPIR_CVAR_BCAST_SHORT_MSG_SIZE) || (comm_size < MPIR_CVAR_BCAST_MIN_PROCS)) {
        ...
        if ((nbytes < MPIR_CVAR_BCAST_LONG_MSG_SIZE) && (MPIU_is_pof2(comm_size, NULL))) {
            mpi_errno =
                MPIC_DEFAULT_Bcast_scatter_doubling_allgather(buffer, count, datatype, root,
                                                              comm_ptr, errflag);
            if (mpi_errno) {
                /* for communication errors, just record the error but continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Bcast_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Bcast_inter(void *buffer,
                             int count,
                             MPI_Datatype datatype,
                             int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
...
*/
    ...
    if (root == MPI_PROC_NULL) {
         ...
         * with rank 0 as root. */
        mpi_errno = MPID_Bcast(buffer, count, datatype, 0, newcomm_ptr, errflag);
        if (mpi_errno) {
            /* for communication errors, just record the error but continue */
            *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
            MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Bcast
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Bcast(void *buffer, int count, MPI_Datatype datatype, int root,
                       MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        mpi_errno = MPIC_DEFAULT_Bcast_intra(buffer, count, datatype, root, comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);

    } else {
        /* intercommunicator */
        mpi_errno = MPIC_DEFAULT_Bcast_inter(buffer, count, datatype, root, comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
