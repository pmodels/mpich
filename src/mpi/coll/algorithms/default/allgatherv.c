...
/* not declared static because a machine-specific function may call this one
   in some cases */
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Allgatherv_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Allgatherv_intra(const void *sendbuf,
                                  int sendcount,
    ...
    if ((total_count * recvtype_size < MPIR_CVAR_ALLGATHER_LONG_MSG_SIZE) &&
        ...
        while (tosend || torecv) {      /* While we have data to send or receive */
            ...
            if (!sendnow && !recvnow) {
                ...
                mpi_errno =
                    MPIC_Send(sbuf, sendnow, recvtype, right, MPIR_ALLGATHERV_TAG, comm_ptr,
                              errflag);
                if (mpi_errno) {
                    /* for communication errors, just record the error but continue */
                    *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Allgatherv_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Allgatherv_inter(const void *sendbuf,
                                  int sendcount,
                                  MPI_Datatype sendtype,
                                  void *recvbuf,
                                  const int *recvcounts,
...
*/
    ...
    mpi_errno = MPIR_Bcast(recvbuf, 1, newtype, 0, newcomm_ptr, errflag);
    if (mpi_errno) {
        /* for communication errors, just record the error but continue */
        *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
        MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
...
#undef FUNCNAME
#define FUNCNAME MPIC_DEFAULT_Allgatherv
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIC_DEFAULT_Allgatherv(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                            ...
                            MPIR_Errflag_t * errflag)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        mpi_errno = MPIC_DEFAULT_Allgatherv_intra(sendbuf, sendcount, sendtype,
        ...
        /* intracommunicator */
        mpi_errno = MPIC_DEFAULT_Allgatherv_inter(sendbuf, sendcount, sendtype,
                                                  recvbuf, recvcounts, displs, recvtype,
                                                  comm_ptr, errflag);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
