 ...
 * https://dl.acm.org/doi/full/10.1145/3735139
 */
 ...
 *
 * The send and receive schedule refers to Bcast. If we reverse the schedule, we get Reduce. And
 * if we concatenate the schedules with each process as root, we get Allgather or Allreduce.
 * We assume the context of bcast in the explanation unless where it is noted.
 */
...
int MPII_circ_graph_create(MPII_circ_graph * cga, int p, int r)
{
    ...
    int *mem;
    mem = MPL_malloc((cga->q * 7 + 4) * sizeof(int), MPL_MEM_COLL);
...
#define SET_MEM(V, size) V = mem; mem += size
    ...
    gen_rsched(r, cga->R, p, q, cga->Skip, stack, next, prev);
    gen_ssched(r, cga->S, p, q, cga->Skip, stack, next, prev, extra);
