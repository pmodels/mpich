...
#undef FUNCNAME
#define FUNCNAME MPIR_Ineighbor_allgatherv_sched_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Ineighbor_allgatherv_sched_intra(const void *sendbuf, int sendcount,
...
#undef FUNCNAME
#define FUNCNAME MPIR_Ineighbor_allgatherv_sched_inter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Ineighbor_allgatherv_sched_inter(const void *sendbuf, int sendcount,
                                    MPI_Datatype sendtype, void *recvbuf,
                                    const int recvcounts[], const int displs[],
                                    MPI_Datatype recvtype, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        switch (MPIR_Ineighbor_allgatherv_intra_algo_choice) {
            ...
            case MPIR_INEIGHBOR_ALLGATHERV_INTRA_ALGO_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Ineighbor_allgatherv_sched_intra(sendbuf, sendcount, sendtype,
                                                                    recvbuf, recvcounts, displs, recvtype,
                                                                    comm_ptr, s);
                break;
        }
        ...
        switch (MPIR_Ineighbor_allgatherv_inter_algo_choice) {
            ...
            case MPIR_INEIGHBOR_ALLGATHERV_INTER_ALGO_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Ineighbor_allgatherv_sched_inter(sendbuf, sendcount, sendtype,
                                                                    recvbuf, recvcounts, displs, recvtype,
                                                                    comm_ptr, s);
                break;
        }
