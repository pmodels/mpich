...
cvars:
    ...
    - name        : MPIR_CVAR_INEIGHBOR_ALLGATHERV_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_ineighbor_allgatherv will allow the device to override the
        MPIR-level collective algorithms. The device still has the
        option to call the MPIR-level algorithms manually.
        If set to false, the device-level ineighbor_allgatherv function will not be
        called.
...
#undef FUNCNAME
#define FUNCNAME MPIR_Ineighbor_allgatherv_sched__intra__auto
#undef FCNAME
...
#undef FUNCNAME
#define FUNCNAME MPIR_Ineighbor_allgatherv_sched__inter__auto
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        switch (MPIR_Ineighbor_allgatherv_intra_algo_choice) {
            ...
            case MPIR_INEIGHBOR_ALLGATHERV_INTRA_ALGO_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Ineighbor_allgatherv_sched__intra__auto(sendbuf, sendcount, sendtype,
                                                                    recvbuf, recvcounts, displs, recvtype,
                                                                    comm_ptr, s);
                break;
        }
        ...
        switch (MPIR_Ineighbor_allgatherv_inter_algo_choice) {
            ...
            case MPIR_INEIGHBOR_ALLGATHERV_INTER_ALGO_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Ineighbor_allgatherv_sched__inter__auto(sendbuf, sendcount, sendtype,
                                                                    recvbuf, recvcounts, displs, recvtype,
                                                                    comm_ptr, s);
                break;
        }
                                    ...
                                    MPI_Datatype recvtype, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (MPIR_CVAR_BARRIER_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
        mpi_errno = MPID_Ineighbor_allgatherv_sched(sendbuf, sendcount, sendtype, recvbuf,
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    mpi_errno = MPIR_Ineighbor_allgatherv(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs, recvtype, comm_ptr, &request_ptr);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
