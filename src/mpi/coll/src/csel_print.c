...
void collect_rules(csel_node_s * node)
{
    ...
    if (node->type == CSEL_NODE_TYPE__CONTAINER) {
        ...
        Csel_decision_rule *new_rule = (Csel_decision_rule *) MPL_malloc(sizeof(Csel_decision_rule), MPL_MEM_OTHER);
        new_rule->size = nesting;
        new_rule->algorithm = alg_id;
        new_rule->path = (csel_node_s **) MPL_malloc(sizeof(csel_node_s*) * nesting, MPL_MEM_OTHER);
...
void Csel_print_rules(csel_node_s * node)
{
    ...
    memset(algorithms, 0, sizeof(Csel_decision_rule *) * MPII_CSEL_CONTAINER_TYPE__ALGORITHM__Algorithm_count);
