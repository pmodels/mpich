...
cvars:
    - name        : MPIR_CVAR_ALLTOALL_BRUCKS_KVAL
      ...
      description : >-
        radix (k) value for blocking alltoall brucks
 ...
 *       Cost = logk(p) * alpha + (n/k) * (k - 1) * logk(p) * beta
                                 ...
                                 MPI_Datatype recvtype, MPIR_Comm * comm, MPIR_Errflag_t * errflag)
{
    ...
    const void *senddata;
    int tag;
    int k = MPIR_CVAR_ALLTOALL_BRUCKS_KVAL;     /* default k = 2 */
    MPIR_Request **reqs;
    int num_reqs = 0;
    ...
    MPIR_CHKLMEM_MALLOC(tmp_buf, void *, recvcount * size * r_extent, mpi_errno, "tmp_buf",
                        MPL_MEM_COLL);

    if (is_inplace) {
        mpi_errno =
            MPIR_Localcopy(recvbuf, size * recvcount, recvtype, tmp_buf, size * recvcount,
                           recvtype);
        if (mpi_errno) {
            MPIR_ERR_POP(mpi_errno);
        }
    ...
    mpi_errno = MPIR_Localcopy((void *) ((char *) senddata + rank * sendcount * s_extent),
                               (size - rank) * sendcount, sendtype, recvbuf,
                               (size - rank) * recvcount, recvtype);
    if (mpi_errno) {
        MPIR_ERR_POP(mpi_errno);
    }
    mpi_errno = MPIR_Localcopy(senddata, rank * sendcount, sendtype,
    ...
    for (j = 0; j < k - 1; j++) {
        tmp_sbuf[j] = (void *) MPL_malloc(r_extent * recvcount * p_of_k, MPL_MEM_COLL);
        MPIR_ERR_CHKANDJUMP(!tmp_sbuf[j], mpi_errno, MPI_ERR_OTHER, "**nomem");
        tmp_rbuf[j] = (void *) MPL_malloc(r_extent * recvcount * p_of_k, MPL_MEM_COLL);
    ...
    for (i = 0; i < nphases; i++) {
        ...
        for (j = 1; j < k; j++) {       /* for every non-zero value of digitval */
            ...
            mpi_errno =
                brucks_sched_pup(1, recvbuf, tmp_sbuf[j - 1], recvtype, recvcount, delta, k, j,
                                 size, &packsize);
            if (mpi_errno) {
                MPIR_ERR_POP(mpi_errno);
            }
            ...
            mpi_errno =
                MPIC_Irecv(tmp_rbuf[j - 1], packsize, MPI_BYTE, src, tag, comm, &reqs[num_reqs++]);
            if (mpi_errno) {
                MPIR_ERR_POP(mpi_errno);
            }

            mpi_errno =
                MPIC_Isend(tmp_sbuf[j - 1], packsize, MPI_BYTE, dst, tag, comm, &reqs[num_reqs++],
                           errflag);
            if (mpi_errno) {
                MPIR_ERR_POP(mpi_errno);
            }
        ...
        for (j = 1; j < k; j++) {
            ...
            mpi_errno =
                brucks_sched_pup(0, recvbuf, tmp_rbuf[j - 1], recvtype, recvcount, delta, k, j,
                                 size, &packsize);
            if (mpi_errno) {
                MPIR_ERR_POP(mpi_errno);
            }
    ...
    mpi_errno = MPIR_Localcopy((void *) ((char *) recvbuf + (rank + 1) * recvcount * r_extent),
                               (size - rank - 1) * recvcount, recvtype, tmp_buf,
                               (size - rank - 1) * recvcount, recvtype);
    if (mpi_errno) {
        MPIR_ERR_POP(mpi_errno);
    }
    ...
    for (i = 0; i < size; i++) {
        mpi_errno = MPIR_Localcopy((char *) tmp_buf + i * recvcount * r_extent, recvcount, recvtype,
                                   (void *) ((char *) recvbuf +
