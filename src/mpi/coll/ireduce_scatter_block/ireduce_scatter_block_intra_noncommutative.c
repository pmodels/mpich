 ...
 * the power-of-2 case. */
#undef FUNCNAME
#define FUNCNAME MPIR_Ireduce_scatter_block_sched__intra__noncommutative
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Ireduce_scatter_block_sched__intra__noncommutative(const void *sendbuf, void *recvbuf, int recvcount, MPI_Datatype datatype, MPI_Op op, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    for (i = 0; i < comm_size; ++i) {
        mpi_errno = MPIR_Sched_copy(((char *)(sendbuf == MPI_IN_PLACE ? (const void *)recvbuf : sendbuf) + (i * true_extent * block_size)),
                                    block_size, datatype,
                                    ((char *)tmp_buf0 + (MPL_mirror_permutation(i, log2_comm_size) * true_extent * block_size)),
