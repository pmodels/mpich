...
#undef FUNCNAME
#define FUNCNAME MPIR_Gatherv__inter__linear
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Gatherv__inter__linear(const void *sendbuf,
                                int sendcount,
                                ...
                                int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
    ...
    if (root == MPI_ROOT) {
        ...
        if (sendcount) {
            /* we want local size in intercomm cases
             * because the size of the root's group (group A in the standard)
             * is irrelevant here. */
