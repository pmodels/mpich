...
#undef FUNCNAME
#define FUNCNAME MPIR_TSP_Ineighbor_alltoall_sched_allcomm_linear
#undef FCNAME
                                                     ...
                                                     MPIR_Comm * comm_ptr, MPIR_TSP_sched_t * sched)
{
    ...
    for (k = 0; k < outdegree; ++k) {
        ...
        MPIR_TSP_sched_isend(sb, sendcount, sendtype, dsts[k], comm_ptr, tag, sched, 0, NULL);
    }

    for (l = 0; l < indegree; ++l) {
        char *rb = ((char *) recvbuf) + l * recvcount * recvtype_extent;
        MPIR_TSP_sched_irecv(rb, recvcount, recvtype, srcs[l], comm_ptr, tag, sched, 0, NULL);
    }
