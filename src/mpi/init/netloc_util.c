                                ...
                                MPIR_Netloc_network_attributes * network_attr)
{
    ...
    if (start_node != NULL && network_attr->type != MPIR_NETLOC_NETWORK_TYPE__INVALID) {
        ...
        if (start_index == topology->num_nodes) {
            ...
            network_attr->u.torus.node_coordinates =
            ...
            temp_coordinate_map =
                (int **) MPL_calloc(topology->num_nodes, sizeof(int *), MPL_MEM_OTHER);
            /* Traverse path graphs to assign coordinates of each node and size of torus alone each dimension */
            for (i = 0; i < topology->num_nodes; i++) {
                ...
                for (j = 0; j < path_graph_insert; j++) {
                    ...
                    while (current_edge < (path_graph_count[j] + 1)) {
                        ...
                        if (vertex_in_second_set) {
                            temp_coordinate_map[vertex->__uid__][j] =
                                ceilf((float) current_edge / 2);
            ...
            /* Flatten computed coordinates into a long value for each node */
            for (i = 0; i < topology->num_nodes; i++) {
                int *coordinates = temp_coordinate_map[topology->nodes[i]->__uid__];
                long node_index = coordinates[0];
                for (j = 1; j < path_graph_insert; j++) {
                    node_index = (node_index * network_attr->u.torus.geometry[j]) + coordinates[j];
                }
                network_attr->u.torus.node_coordinates[topology->nodes[i]->__uid__] = index;
            }
...
#undef FUNCNAME
#define FUNCNAME MPIR_Netloc_get_torus_network_coordinates
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Netloc_get_torus_network_coordinates(MPIR_Netloc_network_attributes network_attributes,
                                              netloc_topology_t topology, netloc_node_t * node,
                                              long *index)
{
    int mpi_errno = MPI_SUCCESS;
    *index = network_attributes.u.torus.node_coordinates[node->__uid__];
