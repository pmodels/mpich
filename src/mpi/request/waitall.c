...
static inline int request_complete_fastpath(MPI_Request * request,
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int do_waitall_post(int count, int remaining,
                                   ...
                                   MPI_Status * array_of_statuses)
{
    ...
    mpi_errno =
        do_waitall_post(count, remaining, array_of_requests, request_ptrs, array_of_statuses);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
...
int MPI_Waitall(int count, MPI_Request array_of_requests[], MPI_Status array_of_statuses[])
{
    ...
    for (i = 0; i < count; i += MPIR_REQUEST_PTR_ARRAY_SIZE) {
        int batch_size =
            (count - i < MPIR_REQUEST_PTR_ARRAY_SIZE) ? count - i : MPIR_REQUEST_PTR_ARRAY_SIZE;
        mpi_errno =
            do_waitall_batch(batch_size, count - i, &array_of_requests[i],
                             array_of_statuses == MPI_STATUSES_IGNORE ?
                             MPI_STATUSES_IGNORE : &array_of_statuses[i]);
        if (mpi_errno)
            MPIR_ERR_POP(mpi_errno);
    }
