                                ...
                                MPI_Aint * dlsz_p, int *dldepth_p, int flag)
{
    ...
    if ((flag == DLOOP_DATALOOP_HOMOGENEOUS) || (flag == DLOOP_DATALOOP_ALL_BYTES)) {
        return DLOOP_Dataloop_create_flattened_struct(count,
                                                      blklens,
                                                      disps,
                                                      oldtypes, dlp_p, dlsz_p, dldepth_p, flag);
    }
    ...
    MPIR_Dataloop_struct_alloc((DLOOP_Count) nr_basics + nr_derived,
                               old_loop_sz, nr_basics, &curpos, &new_dlp, &new_loop_sz);
    ...
    for (i = 0, loop_idx = 0; i < count; i++) {
        ...
        if (is_builtin) {
            ...
            err = MPIR_Dataloop_create_contiguous(blklens[i],
                                                  oldtypes[i],
                                                  &dummy_dlp, &dummy_sz, &dummy_depth, flag);
            ...
            new_dlp->loop_params.s_t.blocksize_array[loop_idx] = (DLOOP_Count) blklens[i];
            new_dlp->loop_params.s_t.el_extent_array[loop_idx] = old_extent;
        }
        new_dlp->loop_params.s_t.offset_array[loop_idx] = (DLOOP_Offset) disps[i];
        loop_idx++;
    }
