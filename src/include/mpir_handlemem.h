...
. objmem - Pointer to object memory block.
  ...
  Threading: The 'MPID_THREAD_CS_ENTER/EXIT(EP_POBJ, MPIR_THREAD_POBJ_HANDLE_MUTEX)' enables both
  finer-grain
...
{
    void *ret;
    MPID_THREAD_CS_ENTER(EP_POBJ, MPIR_THREAD_POBJ_HANDLE_MUTEX);
    ret = MPIR_Handle_obj_alloc_unsafe(objmem);
    MPID_THREAD_CS_EXIT(EP_POBJ, MPIR_THREAD_POBJ_HANDLE_MUTEX);
    return ret;
}
