...
static inline void MPL_Memcpy_ntw(void *dest, const void *src, size_t n)
{
    ...
    /* Copy 256 bytes at a time by unrolling a series of 32-byte streaming (non-temporal) write. */
    while (n >= 256) {
...
static inline void MPL_Memcpy_ntr(void *dest, const void *src, size_t n)
{
    /* Anything less than 256 bytes is not worth optimizing */
    if (n <= 256) {
        memcpy(dest, src, n);
    ...
    /* Copy 256 bytes at a time by unrolling a series of 32-byte using non-temporal read from src.
     * It limits the cache pollution caused by src buffer. Note, we cannot use
     * __mm256_stream_load_si256 intrinic because it translates to VMOVNTDQA. It is deisgned to work
     * on Write-Combined memory (MMIO memory) and does not work on Write-Back (normal memory).
     * Instead, we need to use PREFETCHNTA with regular load to achieve non-temoral load. */
    while (n >= 256) {
...
static inline void MPL_Memcpy_ntr(void *dest, const void *src, size_t n)
{
    ...
    while (n >= 128) {
        ...
        __m128i xmm6 = _mm_loadu_si128((__m128i const *) (s + (16 * 6)));
        __m128i xmm7 = _mm_loadu_si128((__m128i const *) (s + (16 * 7)));
        _mm_storeu_si128((__m128i *) (d + (16 * 0)), xmm0);
        _mm_storeu_si128((__m128i *) (d + (16 * 1)), xmm1);
        _mm_storeu_si128((__m128i *) (d + (16 * 2)), xmm2);
        _mm_storeu_si128((__m128i *) (d + (16 * 3)), xmm3);
        _mm_storeu_si128((__m128i *) (d + (16 * 4)), xmm4);
        _mm_storeu_si128((__m128i *) (d + (16 * 5)), xmm5);
        _mm_storeu_si128((__m128i *) (d + (16 * 6)), xmm6);
        _mm_storeu_si128((__m128i *) (d + (16 * 7)), xmm7);
        d += 128;
        s += 128;
        n -= 128;
    }
