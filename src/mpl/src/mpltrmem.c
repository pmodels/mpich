...
void *MPL_trmalloc( unsigned int a, int lineno, const char fname[] )
{
    ...
    if (TRdebugLevel > 0) {
        ...
        MPL_snprintf( buf, 256,
                       ...
                       lineno, fname );
        if (MPL_trvalid( buf )) goto fn_exit;
    }
...
/* Check that nend is properly aligned */
    if ((sizeof(long) == 4 && ((long)nend & 0x3) != 0) ||
        ...
        MPL_trmem_error_printf("[%d] Block at address %lx is corrupted (invalid address or header)\n"
                               "called in %s at line %d\n",
    ...
    if (*nend != COOKIE_VALUE) {
        if (*nend == ALREADY_FREED) {
            ...
            if (TRidSet) {
                MPL_trmem_error_printf("[%d] Block [id=%d(%lu)] at address %s was already freed\n",
            ...
            if (TRidSet) {
                MPL_trmem_error_printf("[%d] Block [id=%d(%lu)] at address %s is corrupted (probably write past end)\n",
                                       world_rank, head->id, head->size, hexstring );
            }
            else {
                MPL_trmem_error_printf("[%d] Block at address %s is corrupted (probably write past end)\n",
                                       world_rank, hexstring );
            }
            head->fname[TR_FNAME_LEN-1]= 0;  /* Just in case */
...
int MPL_trvalid( const char str[] )
{
    ...
    while (head) {
        ...
        if (head->cookie != COOKIE_VALUE) {
            if (!errs) MPL_trmem_error_printf( "%s\n", str );
            errs++;
            addrToHex( head, hexstring );
            MPL_trmem_error_printf("[%d] Block at address %s is corrupted (invalid cookie in head)\n",
                                   world_rank, hexstring );
...
.   fname  - file name where used.  Use __FILE__ for this
    ...
    available.
 +*/
void *MPL_trcalloc( unsigned int nelem, unsigned int elsize, int lineno,
                     const char fname[] )
{
    void *p;
