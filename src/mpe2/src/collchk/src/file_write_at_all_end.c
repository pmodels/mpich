...
int MPI_File_write_at_all_end(MPI_File fh, MPICH2_CONST void *buff, MPI_Status *st)
{
    int g2g = 1;
    ...
    if(g2g) {
        ...
        if(COLLCHK_CALLED_BEGIN) {
            if(strcmp(CollChk_begin_str, "WRITE_AT_ALL") != 0) {
        ...
        /* make the call */
        return PMPI_File_write_at_all_end(fh, buff, st);
    }
    else {
        /* init not called */
