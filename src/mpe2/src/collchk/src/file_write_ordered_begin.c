...
int MPI_File_write_ordered_begin(MPI_File fh, MPICH2_CONST void *buff, int cnt,
                                 MPI_Datatype dtype)
{
    int g2g = 1;
    ...
    sprintf(call, "FILE_WRITE_ORDERED_BEGIN");
    ...
    if(g2g) {
        ...
        if(COLLCHK_CALLED_BEGIN) {
            ...
            CollChk_set_begin("WRITE_ORDERED");
        }

        /* make the call */
        return PMPI_File_write_ordered_begin(fh, buff, cnt, dtype);
    }
    else {
        /* init not called */
