...
int MPI_File_write_ordered_end(MPI_File fh, MPICH2_CONST void *buff, MPI_Status *st)
{
    int g2g = 1;
    ...
    sprintf(call, "FILE_WRITE_ORDERED_END");
    ...
    if(g2g) {
        ...
        if(COLLCHK_CALLED_BEGIN) {
            if(strcmp(CollChk_begin_str, "WRITE_ORDERED") != 0) {
        ...
        /* make the call */
        return PMPI_File_write_ordered_end(fh, buff, st);
    }
    else {
        /* init not called */
