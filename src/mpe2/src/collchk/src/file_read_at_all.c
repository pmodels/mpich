...
int MPI_File_read_at_all(MPI_File fh, MPI_Offset offset, void *buff, int cnt, 
                         MPI_Datatype dtype, MPI_Status *st)
{
    ...
    if(g2g) {
        ...
        /* make the call */
        return PMPI_File_read_at_all(fh, offset, buff, cnt, dtype, st);
    }
    else {
        /* init not called */
