...
void MPIX_Init_fortran(void)
{
    ...
    if (info == MPI_INFO_NULL) {
        ...
        int8_t logical_true = F77_TRUE_VALUE;
        int8_t logical_false = F77_FALSE_VALUE;
#elif MPI_LOGICAL_SIZE == 2
        int16_t logical_true = F77_TRUE_VALUE;
        int16_t logical_false = F77_FALSE_VALUE;
#elif MPI_LOGICAL_SIZE == 4
        int32_t logical_true = F77_TRUE_VALUE;
        int32_t logical_false = F77_FALSE_VALUE;
#elif MPI_LOGICAL_SIZE == 8
        int64_t logical_true = F77_TRUE_VALUE;
        int64_t logical_false = F77_FALSE_VALUE;
#else
#error Bad MPI_LOGICAL_SIZE!
#endif
        mpi_errno = MPI_Abi_set_fortran_booleans(MPI_LOGICAL_SIZE, &logical_true, &logical_false);
        assert(mpi_errno == MPI_SUCCESS);
    }
}
