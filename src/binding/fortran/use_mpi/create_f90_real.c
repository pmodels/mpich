...
int MPI_Type_create_f90_real(int precision, int range, MPI_Datatype * newtype)
{
    ...
    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Type_create_f90_real(context, 0, precision, range, newtype);

    fn_ptr = (QMPI_Type_create_f90_real_t *) MPIR_QMPI_first_fn_ptrs[MPI_TYPE_CREATE_F90_REAL_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_TYPE_CREATE_F90_REAL_T], precision,
                      range, newtype);
}
...
.N MPI_ERR_ARG
@*/
int QMPI_Type_create_f90_real(QMPI_Context context, int tool_id, int precision, int range,
