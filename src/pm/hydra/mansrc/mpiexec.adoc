...
-genv <name> <value>:: The '-genv' options have the same meaning as their corresponding '-env' version, except they apply to all executables, not just the current executable (in the case that the colon syntax is used to specify multiple executables).
