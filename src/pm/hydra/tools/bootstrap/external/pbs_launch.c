...
HYD_status HYDT_bscd_pbs_launch_procs(const char *rmk, struct HYD_node *node_list, char **args,
                                      int *control_fd)
{
    int node_count, i, args_count, err, hostid;
    ...
    HYDU_MALLOC_OR_JUMP(HYDT_bscd_pbs_sys->task_id, tm_task_id *, node_count * sizeof(tm_task_id),
                        status);
    HYDU_MALLOC_OR_JUMP(HYDT_bscd_pbs_sys->spawn_events, tm_event_t *,
                        node_count * sizeof(tm_event_t), status);
    ...
    for (i = 0, node = node_list; node; node = node->next, i++) {
        if (pbs_node_list) {
            status = find_pbs_node_id(node->hostname, &hostid);
