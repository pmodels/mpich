...
def dump_function_normal(func):
    ...
    if func['_need_CHKLMEM']:
        G.out.append("MPIR_CHKLMEM_DECL(10);")
    ...
    if '_datatype_in_list' in func:
        for p in func['_datatype_in_list']:
            if not p['length']:
                ...
                G.out.append("MPIR_CHKLMEM_MALLOC(%s_i, MPI_Datatype *, %s * sizeof(MPI_Datatype), mpi_errno, \"%s swap\", MPL_MEM_OTHER);" % (p['name'], n, p['name']))
...
def dump_type_create_swap(func):
    for p in func['_poly_in_arrays']:
        ...
        G.out.append("MPIR_CHKLMEM_MALLOC(%s, MPI_Aint *, %s * sizeof(MPI_Aint), mpi_errno, \"counts swap\", MPL_MEM_OTHER);" % (new_name, n))
        dump_for_open("i", n)
...
def dump_coll_v_swap(func):
    ...
    def allocate_tmp_array(n):
        ...
        G.out.append("MPIR_CHKLMEM_MALLOC(tmp_array, MPI_Aint *, %s * sizeof(MPI_Aint), mpi_errno, \"coll_v_swap\", MPL_MEM_OTHER);" % n)
