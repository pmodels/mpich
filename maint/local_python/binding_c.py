...
def dump_abi_wrappers(func, is_large):
    ...
    #
    if RE.match(r'..._(Comm|Datatype|Errhandler|Group|Info|Message|Op|Request|Session|Win|File)_(toint|c2f)\b', func_name):
        ...
        dump_if_close()
    elif RE.match(r'..._(Comm|Datatype|Errhandler|Group|Info|Message|Op|Request|Session|Win|File)_(fromint|f2c)\b', func_name):
        handle_in = func['c_parameters'][0]['name']
        dump_if_open("%s > 0 && %s < 4096" % (handle_in, handle_in))
        G.out.append("return (ABI_%s) (uintptr_t) %s;" % (RE.m.group(1), handle_in))
        dump_if_close()
