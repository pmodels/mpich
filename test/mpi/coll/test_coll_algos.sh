...
#test nb algorithms
testing_env+="env=MPIR_CVAR_ALLTOALLW_INTRA_ALGORITHM=nb "
testing_env+="env=MPIR_CVAR_IALLTOALLW_DEVICE_COLLECTIVE=0 "
...
for algo_name in ${algo_names}; do
    ...
    coll_algo_tests+="alltoallw1 10 ${env}${nl}"
    coll_algo_tests+="alltoallw2 10 ${env}${nl}"
    coll_algo_tests+="alltoallw_zeros 1 ${env}${nl}"
    coll_algo_tests+="alltoallw_zeros 2 ${env}${nl}"
    coll_algo_tests+="alltoallw_zeros 5 ${env}${nl}"
    coll_algo_tests+="alltoallw_zeros 8 ${env}${nl}"
done
