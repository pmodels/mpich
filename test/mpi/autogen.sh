    ...
    do
        ...
        do
            if [ $dir = "pt2pt" ] ; then # only send/recv comm can use types from different pools
                ...
                do
                    echo "${source}__BASIC__L${ln} $procs arg=-type=${type} arg=-sendcnt=${sendcount} arg=-recvcnt=${recvcount} $timelimit env=DTP_MIN_OBJ_ID=24 env=DTP_MAX_OBJ_ID=28" >> ${dir}/testlist.dtp
                ...
                echo "${source}__BASIC__L${ln} $procs arg=-type=${type} arg=-count=${sendcount} $timelimit env=DTP_MIN_OBJ_ID=24 env=DTP_MAX_OBJ_ID=28" >> ${dir}/testlist.dtp
            fi
        done
    done
