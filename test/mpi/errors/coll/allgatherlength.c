...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        ierr = MTest_Allgather(is_blocking, buf, 1, MPI_INT, recvbuf, 10, MPI_INT, MPI_COMM_WORLD);
    } else {
        ierr = MTest_Allgather(is_blocking, buf, 10, MPI_INT, recvbuf, 10, MPI_INT, MPI_COMM_WORLD);
    }
    if (ierr == MPI_SUCCESS) {
        if (rank != 0) {
