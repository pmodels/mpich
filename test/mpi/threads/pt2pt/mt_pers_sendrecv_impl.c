                         ...
                         MPI_Request * r, MPI_Status * s, int verify)
{
    ...
    if (rank == 0) {
        for (i = 0; i < count; i++)
            buff[i] = i + id;   /*always send buff[0] */
        MPI_Start(r);
        MPI_Wait(r, s);
    } else if (rank == 1) {
        MPI_Start(r);
...
int test_pers_sendrecv(int id, int *buff, MPI_Comm comm, int tag, int verify)
{
    int errs = 0, i, rank;
    ...
    /* Init objects */
    if (rank == 0)
        SENDINIT(buff, COUNT, MPI_INT, 1, tag, comm, &r);
    else
        MPI_Recv_init(buff, COUNT, MPI_INT, 0, tag, comm, &r);
...
MTEST_THREAD_RETURN_TYPE run_test(void *arg)
{
    ...
    p->result = test_pers_sendrecv(p->id, p->buff, p->comm, p->tag, p->verify);
