...
static inline int test(MPI_Comm comm, int rank, int orig, int target,
                       int i, int j, DTP_t send_dtp, DTP_t target_dtp)
{
    int errs = 0, err;
    int len;
    int disp_unit;
    ...
    count = send_dtp->DTP_type_signature.DTP_pool_basic.DTP_basic_type_count;
    origtype = send_dtp->DTP_obj_array[i].DTP_obj_type;
    targettype = target_dtp->DTP_obj_array[j].DTP_obj_type;
    origcount = send_dtp->DTP_obj_array[i].DTP_obj_count;
    targetcount = target_dtp->DTP_obj_array[j].DTP_obj_count;
    origbuf = send_dtp->DTP_obj_array[i].DTP_obj_buf;
    ...
    MTestPrintfMsg(1,
                   "Putting count = %ld of origtype %s - count = %ld receive type %s\n",
    ...
    if (rank == orig) {
        err = MPI_Put(origbuf, origcount, origtype, target, 0, targetcount, targettype, win);
        if (err) {
            errs++;
            if (errs < 10) {
                MTestPrintError(err);
