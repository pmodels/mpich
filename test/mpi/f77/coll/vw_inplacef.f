...
C
       ...
       integer sdispls(MAX_SIZE), scounts(MAX_SIZE), stypes(MAX_SIZE)
...
C
     ...
     $             ', should be', rank * size + i - 1
       ...
       do i=1,size
           ...
           enddo
       enddo
       call mpi_alltoallv( MPI_IN_PLACE, scounts, sdispls,
...
C          Alltoallw's displs[] are in bytes not in type extents.
     ...
     $                                        + 10 * (i-1) + j
           enddo
       enddo
       call mpi_alltoallw( MPI_IN_PLACE, scounts, sdispls, stypes, rbuf,
     $                     rcounts, rdispls, rtypes,
     $                     comm, ierr )
       do i=1,size
           do j=0,rcounts(i)-1
               iexpected = 100 * (i-1) + 10 * rank + j
