...
int main(int argc, char *argv[])
{
	...
	*/
	...
	for (;;)
	{
		...
		cur_proc = temp[0];
		MPI_Recv(in_grid_array, (temp[2] + 1 - temp[1]) * (temp[4] + 1 - temp[3]), MPI_INT, cur_proc, 201, MPI_COMM_WORLD, &status);
		/* draw data */
	...
	for (;;)
	{
		...
		MPI_Send(in_grid_array, (temp[2] + 1 - temp[1]) * (temp[4] + 1 - temp[3]), MPI_INT, 0, 201, MPI_COMM_WORLD);
